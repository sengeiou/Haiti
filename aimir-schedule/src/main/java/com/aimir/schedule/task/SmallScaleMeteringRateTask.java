 ...
 */
@Service
public class SmallScaleMeteringRateTask extends ScheduleTask {
	...
	public List<Map<String, Object>> execute(String searchTime) {
		...
		try {
			result = meterDao.getSmallScaleMeteringRate(searchTime);
		} catch (Exception e) {
			if (txstatus != null) {
	...
	public List<Map<String, Object>> execute2(String searchTime) {
		...
		try {
			result = meterDao.getSmallScaleMeteringRate_detail(searchTime);
		} catch (Exception e) {
			if (txstatus != null) {
				txmanager.rollback(txstatus);
			}
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			logger.info("Usage:");
			logger.info("SmallScaleMeteringRateTask -DsearchTime=SearchTime -DtableName=TableName -DtempTableName=TempTableName");
			return;
		}
		...
		logger.info("SmallScaleMeteringRateTask params. SearchTime={}, TableName={}, TempTableName={}", searchTime, tableName, tempTableName);

		try {
			ApplicationContext ctx = new ClassPathXmlApplicationContext(new String[] { "spring-MeteringRate.xml" });
			DataUtil.setApplicationContext(ctx);

			SmallScaleMeteringRateTask task = (SmallScaleMeteringRateTask) ctx.getBean(SmallScaleMeteringRateTask.class);
	...
	private static void makeExcel(List<Map<String, Object>> result, List<Map<String, Object>> result_detail, String searchTime) {
		...
		String fileName = "./report/SmallScaleMeteringReport_" + dayTime + ".xls";
		...
		try {
			...
			fontTitle.setFontName("굴림체");
			fontTitle.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);

			HSSFFont fontHeader = workbook.createFont();
			fontHeader.setFontHeightInPoints((short) 10);
			fontHeader.setFontName("굴림체");
			...
			sheet.setColumnWidth(colIdx++, 256 * 23);	// D 
			sheet.setColumnWidth(colIdx++, 256 * 15);	// E 
			sheet.setColumnWidth(colIdx++, 256 * 15);	// F 
			sheet.setColumnWidth(colIdx++, 256 * 15);	// G 
			sheet.setColumnWidth(colIdx++, 256 * 15);	// H
			sheet.setColumnWidth(colIdx++, 256 * 15);	// I 
			...
			for (Map<String, Object> map : result) {
				if (!dsoList.contains(map.get("DSO").toString())) {
					...
					dsoCalcMap.put(map.get("DSO").toString() + "_successMeter", 0);
					dsoCalcMap.put(map.get("DSO").toString() + "_failMeter", 0);
					dsoCalcMap.put(map.get("DSO").toString() + "_totalLP", 0);
					dsoCalcMap.put(map.get("DSO").toString() + "_collectedLP", 0);
			...
			cell.setCellValue("Success Meters");
			cell.setCellStyle(titleCellStyle);

			cell = row.createCell(4);
			cell.setCellValue("Fail Meters");
			cell.setCellStyle(titleCellStyle);
			
			cell = row.createCell(5);
			cell.setCellValue("Total LP");
			cell.setCellStyle(titleCellStyle);
			
			cell = row.createCell(6);
			cell.setCellValue("Collected LP");
			cell.setCellStyle(titleCellStyle);
			
			cell = row.createCell(7);
			cell.setCellValue("Remain LP");
			cell.setCellStyle(titleCellStyle);
            ...
            for (Map<String, Object> map : result) {
                ...
                String successMeter = (String.valueOf(map.get("SUCCESS_METER")) != null) ? String.valueOf(map.get("SUCCESS_METER")) : "0";
                String failMeter = (String.valueOf(map.get("FAIL_METER")) != null) ? String.valueOf(map.get("FAIL_METER")) : "0"; 
                String totalLP =  (String.valueOf(map.get("TOTAL_LP")) != null) ? String.valueOf(map.get("TOTAL_LP")) : "0"; 
                String collectedLP =  (String.valueOf(map.get("COLLECTED_LP")) != null) ? String.valueOf(map.get("COLLECTED_LP")) : "0";
                Double meteringRate = (Double.valueOf(collectedLP)/Double.valueOf(totalLP)) * 100;
                ...
                cell = row.createCell(3);
                cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                cell.setCellValue(Integer.valueOf(successMeter));
                cell.setCellStyle(currentRowStyle);
                
                cell = row.createCell(4);
                cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                cell.setCellValue(Integer.valueOf(failMeter));
                cell.setCellStyle(currentRowStyle);
                
                cell = row.createCell(5);
                cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                cell.setCellValue(Integer.valueOf(totalLP));
                cell.setCellStyle(currentRowStyle);
                
                cell = row.createCell(6);
                cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                cell.setCellValue(Integer.valueOf(collectedLP));
                cell.setCellStyle(currentRowStyle);
                
                cell = row.createCell(7);
                cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                cell.setCellValue(Integer.valueOf(totalLP) - Integer.valueOf(collectedLP));
                cell.setCellStyle(currentRowStyle);
                ...
                for (int i = 0; i < dsoList.size(); i++) {
                	...
                	int temp_successMeter = 0;
                	int temp_failMeter = 0;
                	int temp_totalLP = 0;
        			int temp_collectedLP = 0;
					...
					if (dsoName.equals(dsoList.get(i))) {
						...
						temp_successMeter = Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_successMeter").toString());
						temp_successMeter += Integer.valueOf(successMeter);

						temp_failMeter = Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_failMeter").toString());
						temp_failMeter += Integer.valueOf(failMeter);
						
						temp_totalLP = Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_totalLP").toString());
						temp_totalLP += Integer.valueOf(totalLP);
						
						temp_collectedLP = Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_collectedLP").toString());
						temp_collectedLP += Integer.valueOf(collectedLP);

						temp_meteringRate = (Double.valueOf(temp_collectedLP)/Double.valueOf(temp_totalLP)) * 100;
						...
						dsoCalcMap.put(map.get("DSO").toString() + "_successMeter", temp_successMeter);
						dsoCalcMap.put(map.get("DSO").toString() + "_failMeter", temp_failMeter);
						dsoCalcMap.put(map.get("DSO").toString() + "_totalLP", temp_totalLP);
						dsoCalcMap.put(map.get("DSO").toString() + "_collectedLP", temp_collectedLP);
        	...
        	int totalSuccessMeter = 0;
        	int totalFailMeter = 0;
        	int totalTotalLP = 0;
        	int totalCollectedLP = 0;
			...
			for (int i = 0; i < dsoList.size(); i++) {
	            ...
	            cell = row.createCell(3);
	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
	            cell.setCellValue(Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_successMeter").toString()));
	            cell.setCellStyle(currentRowStyle);
	            totalSuccessMeter += Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_successMeter").toString());

	            // Fail Meters
	            cell = row.createCell(4);
	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
	            cell.setCellValue(Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_failMeter").toString()));
	            cell.setCellStyle(currentRowStyle);
	            totalFailMeter += Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_failMeter").toString());
	            
	            // Total LP
	            cell = row.createCell(5);
	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
	            cell.setCellValue(Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_totalLP").toString()));
	            cell.setCellStyle(currentRowStyle);
	            totalTotalLP += Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_totalLP").toString());
	            
	            // Collected LP
	            cell = row.createCell(6);
	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
	            cell.setCellValue(Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_collectedLP").toString()));
	            cell.setCellStyle(currentRowStyle);
	            totalCollectedLP += Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_collectedLP").toString());
	            
	            // Remain LP
	            cell = row.createCell(7);
	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
	            cell.setCellValue(Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_totalLP").toString())
	            		- Integer.valueOf(dsoCalcMap.get(dsoList.get(i) + "_collectedLP").toString()));
	            cell.setCellStyle(currentRowStyle);
                ...
                // Total Rate
                if( i+1 == dsoList.size()){
                	Double totalMeteringRate = Double.parseDouble(String.format("%.2f", Double.valueOf(totalCollectedLP)/Double.valueOf(totalTotalLP) * 100));
    	            ...
    	            cell = row.createCell(3);
    	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
    	            cell.setCellValue(totalSuccessMeter);
    	            cell.setCellStyle(currentRowStyle);

    	            cell = row.createCell(4);
    	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
    	            cell.setCellValue(totalFailMeter);
    	            cell.setCellStyle(currentRowStyle);
    	            
    	            cell = row.createCell(5);
    	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
    	            cell.setCellValue(totalTotalLP);
    	            cell.setCellStyle(currentRowStyle);
    	            
    	            cell = row.createCell(6);
    	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
    	            cell.setCellValue(totalCollectedLP);
    	            cell.setCellStyle(currentRowStyle);
    	            
    	            cell = row.createCell(7);
    	            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
    	            cell.setCellValue(totalTotalLP - totalCollectedLP);
    	            cell.setCellStyle(currentRowStyle);
			...
			cell = row.createCell(8);
			cell.setCellValue("LP ( 00h -> 23h )");
			cell.setCellStyle(titleCellStyle);
			
			
			cell = row.createCell(9);
			cell.setCellStyle(titleCellStyle);
			sheet.addMergedRegion(new CellRangeAddress(cell.getRowIndex(), cell.getRowIndex(), cell.getColumnIndex()-1, cell.getColumnIndex()));
			...
			for (int i = 0; i < result_detail.size(); i++) {
	            ...
	            cell = row.createCell(3);
	            cell.setCellValue((result_detail.get(i).get("DEVICE_SERIAL") != null) ? result_detail.get(i).get("GS1").toString() : "");
	            cell.setCellStyle(currentRowStyle);
                ...
                cell = row.createCell(8);
                String tmpLP = "";
                if(result_detail.get(i).get("LP_24") != null) {
                	tmpLP = result_detail.get(i).get("LP_24").toString().replaceAll("1", "X").replaceAll("0", "_");
                }
	            cell.setCellValue(tmpLP.substring(1, 7) + " " + tmpLP.substring(7, 13) + " " + tmpLP.substring(13, 19) + " " + tmpLP.substring(19));
                cell.setCellStyle(currentRowStyle);
			...
			sheet.setAutoFilter(new CellRangeAddress(row_count2 + 2, row_count2 + 2,	0, 8));
//			sheet.setAutoFilter(new CellRangeAddress(row_count2 + 2, row_count2 + 2 + result_detail.size(),	0, 8));
			// 검침 상세 내역 (E)
