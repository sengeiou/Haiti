 ...
 */
@Service
public class MeteringRateTask extends ScheduleTask {
	private static Logger logger = LoggerFactory.getLogger(MeteringRateTask.class);
	...
	public List<Map<String, Object>> execute(String searchTime, String tableName) {
		...
		try {
			result = meterDao.getMeteringRate(searchTime, tableName);
		} catch (Exception e) {
			if (txstatus != null) {
	...
	public List<Map<String, Object>> execute2(String searchTime, String tableName, String tempTableName) {
		...
		try {
			result = meterDao.getMeteringRate_detail(searchTime, tableName, tempTableName);
		} catch (Exception e) {
			if (txstatus != null) {
				txmanager.rollback(txstatus);
			}
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			logger.info("Usage:");
			logger.info("MeteringRateTask -DsearchTime=SearchTime -DtableName=TableName -DtempTableName=TempTableName");
			return;
		}
		...
		logger.info("MeteringRateTask params. SearchTime={}, TableName={}, TempTableName={}", searchTime, tableName, tempTableName);

		try {
			ApplicationContext ctx = new ClassPathXmlApplicationContext(new String[] { "spring-MeteringRate.xml" });
			DataUtil.setApplicationContext(ctx);

			MeteringRateTask task = (MeteringRateTask) ctx.getBean(MeteringRateTask.class);
			...
			logger.error("MeteringRateTask excute error - " + e, e);
		} finally {
			logger.info("#### MeteringRateTask finished. ####");
			System.exit(0);
		}
	}
	...
	private static void makeExcel(List<Map<String, Object>> result, List<Map<String, Object>> result_detail, String searchTime) {
		...
		String fileName = "./report/MeteringReport_" + dayTime + ".xls";
		...
		try {
			...
			for (Map<String, Object> map : result) {
				if (!dsoList.contains(map.get("NAME").toString())) {
					...
					dsoCalcMap.put(map.get("NAME").toString() + "_relatedMeter", 0);
					dsoCalcMap.put(map.get("NAME").toString() + "_meteringCount", 0);
					dsoCalcMap.put(map.get("NAME").toString() + "_meteringRate", 0);
				}
			}
            ...
            for (Map<String, Object> map : result) {
                ...
                String dsoName = (String.valueOf(map.get("NAME")) != null) ? String.valueOf(map.get("NAME")) : "";
                ...
                for (int i = 0; i < dsoList.size(); i++) {
					...
					if (dsoName.equals(dsoList.get(i))) {
						...
						dsoCalcMap.put(map.get("NAME").toString() + "_relatedMeter", temp_relatedMeter);
						dsoCalcMap.put(map.get("NAME").toString() + "_meteringCount", temp_meteringCount);
						dsoCalcMap.put(map.get("NAME").toString() + "_meteringRate", temp_meteringRate);
                	}
                }
                // DSO별 통계 계산 로직 (E)
            }
			...
			for (int i = 0; i < result_detail.size(); i++) {
				...
				cell = row.createCell(1);
				cell.setCellValue((result_detail.get(i).get("MDS_ID") != null) ? result_detail.get(i).get("MDS_ID").toString() : "");
	            cell.setCellStyle(currentRowStyle);
	            ...
	            cell = row.createCell(5);
	            cell.setCellValue((result_detail.get(i).get("FW_REVISION") != null) ? result_detail.get(i).get("FW_REVISION").toString() : "");
	            cell.setCellStyle(currentRowStyle);
