...
@Service
public class ThresholdAuthenticationErrorTask extends ScheduleTask 
{
    private static Log log = LogFactory.getLog(ThresholdAuthenticationErrorTask.class);
    ...
    public static void main(String[] args) {
        ...
        ThresholdAuthenticationErrorTask task = ctx.getBean(ThresholdAuthenticationErrorTask.class);
        task.execute(null);
        System.exit(0);
    }
    
    private List<ThresholdWarning> getOverList() {
        Threshold threshold = thresholdDao.getThresholdByname(ThresholdName.AUTHENTICATION_ERROR.name());
        if (threshold==null) {
        	return null;
        }        
        	...
        	for (ThresholdWarning data : list) {
        		...
        		if (id==null ||"".equals(id)) {
	    			...
	    			String result  = cmdOperationUtil.cmdSendEvent("Threshold Warning", 
	    					target.name(),
	    					id,
	    					new String[][] {{"kind", ThresholdName.AUTHENTICATION_ERROR.getThresholdNameValue()},
