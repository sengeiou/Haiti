...
@Service
public class ThresholdInvalidPacketTask extends ScheduleTask 
{
    private static Log log = LogFactory.getLog(ThresholdInvalidPacketTask.class);
    ...
    public static void main(String[] args) {
        ...
        ThresholdInvalidPacketTask task = ctx.getBean(ThresholdInvalidPacketTask.class);
        task.execute(null);
        System.exit(0);
    }    
    
    private List<ThresholdWarning> getOverList() {
        Threshold threshold = thresholdDao.getThresholdByname(ThresholdName.INVALID_PACKET.name());
        if (threshold==null) {
        	return null;
        }
...
//        				"] thresholdValue[" + threshold.getLimit().toString() + "]");
        		if (id==null ||"".equals(id)) {
            		...
            		log.debug("ThresholdWarningID["+ data.getId() + "]");
            		log.debug("target[" + target.name() +"]");
            		log.debug("activatorId[" + id + "] ");
            		log.debug("observedValue[" + data.getValue().toString() + "]");
            		log.debug("thresholdValue[" + threshold.getLimit().toString() + "]");
            		
	                // send event
	    			String result  = cmdOperationUtil.cmdSendEvent("Threshold Warning", 
	    					target.name(),
	    					id,
	    					new String[][] {{"kind", ThresholdName.INVALID_PACKET.getThresholdNameValue()},
