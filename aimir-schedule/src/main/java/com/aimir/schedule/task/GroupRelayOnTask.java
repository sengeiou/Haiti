...
public class GroupRelayOnTask extends GroupTask {
    protected static Log log = LogFactory.getLog(GroupRelayOnTask.class);
    ...
    public void execute(JobExecutionContext context) {
        if(isNowRunning){
            log.info("########### GroupRelayOnTask is already running...");
            return;
        }
        isNowRunning = true;
        
        log.info("##########GroupRelayOnTask START #############");
        ...
        for (Iterator<String> i = groupDCU.keySet().iterator(); i.hasNext();) {
            mcuId = i.next();
            try {
                executor.execute(new RelayOnThread(mcuId, groupDCU.get(mcuId), WRONGGROUP));
            }
            catch (Exception e) {
                log.error(e, e);
            }
...
class RelayOnThread implements Runnable {
    ...
    RelayOnThread(String mcuId, List<Meter> meterList, String WRONGGROUP) {
        this.mcuId = mcuId;
    ...
    public void run() {
        ...
        for (Meter meter : meterList) {
        	if(!cannotConnectDCU) {
        		...
        		saveOperationLog(supplier, meterTypeCodeId, meter.getMdsId(), "groupRelayOn_Schedule", status.getCode(), "Can't connect to DCU");
        	}
        }
        log.info("########## End thread MCU[" + mcuId + "] Meter_size [" + meterList.size() + "] ##########");
    }
    
    public String relayOnThread(Meter meter) {
    	log.info("Relay ON START GroupMember[" + meter.getMdsId() + "]");
        ResultStatus status = ResultStatus.FAIL;
        TransactionStatus txStatus = null;
        MCU mcu = null;
        ...
        try{
            ...
            if(!WRONGGROUP.equals(mcu.getSysID())) {
                ...
                Map<String, Object> resultMap = cmdOperationUtil.relayValveOn(mcu.getSysID(), meter.getMdsId());
                ...
                for (Object o : values) {
                    ...
                    for (int i = 0; i < ja.size(); i++) {
                        if (ja.get(i).getAsJsonObject().get("name").getAsString().equals("Result") 
                        		|| ja.get(i).getAsJsonObject().get("value").getAsString().equals("Relays connected")
                        			|| ja.get(i).getAsJsonObject().get("value").getAsString().equals("SUCCESS : Send SMS Command(RelayOn)")) {
        ...
        try {
            txStatus = txManager.getTransaction(null);
            // Operation Log에 기록
            saveOperationLog(supplier, meterTypeCodeId, meter.getMdsId(), "groupRelayOn_Schedule", status.getCode(), mcuStatus != null ? mcuStatus : status.name());
            txManager.commit(txStatus);
            return mcuStatus;
        } catch(Exception e) {
            log.error("[DCU:" + mcu.getSysID() + " METER:" + meter.getMdsId() + "] Relay On Status [" + status + "]", e);       
