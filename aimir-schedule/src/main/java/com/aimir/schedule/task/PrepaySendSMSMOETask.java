 ...
 */
@Transactional
public class PrepaySendSMSMOETask extends ScheduleTask {

	private static Log log = LogFactory.getLog(PrepaySendSMSMOETask.class);
    ...
    public void execute(JobExecutionContext context) { 
		if(isNowRunning){
			log.info("########### PrepaySendSMSMOETask is already running...");
			return;
		}
		isNowRunning = true;
		
        log.info("########### 1. START PrepaySendSMSMOETask ###############");
		...
		/**	TEST
			Set<String> triggerSet = jobDataMap.keySet();
			for(String key : triggerSet){
				log.debug(triggerName + " Trigger - PrepaySendSMSMOETask ############ key = " + key + "  value = " + jobDataMap.get(key));
			}
		 */
		...
		if(jobDataMap != null && jobDataMap.containsKey("subJobData")){
			...
			try {
				for(int idx : messageKey){
					...
					log.debug(trigger.getName() + " Trigger - PrepaySendSMSMOETask - Message ====> " + temp + " ====>" + DateTimeUtil.getCurrentDateTimeByFormat("yyyy-MM-dd HH:mm:ss"));
				}
		 ...
		 */
 //      this.ContractInfo();   차후 IraqMOE 프로젝트 진행시 개발할것.


//		Contract contract = contractDao.get(777);
//		contract.setCurrentCredit(contract.getCurrentCredit() + 10);
//		contractDao.update(contract);



        log.info("########### 1. END PrepaySendSMSMOETask ############");
        isNowRunning = false;
    }
	...
	public void sendSMSTargetVal(List<Map<String, Object>> contractInfo) {
		...
		for (int i = 0; i < contractInfo.size(); i++) {
			try {
				...
				Double targetVal = Double.parseDouble(contractInfo.get(i).get("PREPAYMENTTHRESHOLD") == null ? "0" : contractInfo.get(i).get("PREPAYMENTTHRESHOLD").toString());
...
/*				String text = "\n Nom Du Client: " + contractInfo.get(i).get("CUSTOMERNAME")
                       // + "\n Metre no. compteur : " + mdsId
       					  + "\n Client ID : " + (customer.getLoginId().equals("") ? "-" : customer.getLoginId())  // 20141204 EDH 염부장 요청으로 변경함
                        // spasa 전용 문자 항목 ecg에서는 contract nr를 사용하지 않음
                        //+ "\n Contract nr : " + contract.getContractNumber()
		...
		log.info("########### 1.1 END prepaySMSSend_sendSMSTartgetVal (" + contractInfo.size() + ")건 ###############");
	}
...
//				String text = "\n Nom Du Client: " + contractInfo.get(i).get("CUSTOMERNAME")
//				     //+ "\n Metre no. compteur : " + mdsId
//					 //+ "\n Client ID. compteur : " + mdsId  // 20141203 EDH 염부장 요청으로 변경함
//					  + "\n Client ID : " + (customer.getLoginId().equals("") ? "-" : customer.getLoginId())  // 20141204 EDH 염부장 요청으로 변경함
...
//					  + "\n Courant Credit : " +  cdf.format(currentCredit);
				...
				if(msgKey == 1){
					//text = ("EDH pention vil office" + text);
					text = ("MOE office" + text);
				}
				messageMap.put(String.valueOf(msgKey), text);
...
//					urlConnection.disconnect();
		...
		log.info("########### 1.2 END prepaySMSSend_sendSMSPeriod (" + contractInfo.size() + ")건 ###############");
	}
	...
	private Contract notificationMultiSMS(Contract contract, boolean isUpdateCondract) throws Exception {
		...
		try {
			for(int idx : messageKey){
				...
				log.debug("[" + triggerName + "] Trigger - PrepaySendSMSMOETask - Message ID ===> [" + messageId + "] Message ====> ["+ text + "] : " + "Phone Number = " + mobileNo);
			}
		} catch (Exception e) {
			log.warn("WARNING : " + e);
		}
