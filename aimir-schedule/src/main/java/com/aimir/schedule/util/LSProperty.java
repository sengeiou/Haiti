...
public class LSProperty {
    private static Log log = LogFactory.getLog(LSProperty.class);
    private static final Properties properties;

    static {
        String it = "/config/schedule.properties";

        Properties result = new Properties();
        try {
            InputStream is = LSProperty.class.getResourceAsStream(it);
            result.load(is);
            is.close();
    ...
    public static URL getPropertyURL(String key) {
        ...
        if (url == null) {
            url = SAPProperty.class.getResource(val);
        }
