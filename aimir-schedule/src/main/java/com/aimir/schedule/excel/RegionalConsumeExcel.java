...
public class RegionalConsumeExcel {
	...
	public static void makeMonthlyReportExcel(String title,
	    ...
	    try {
            ...
            int dataStartRow = 3;
            PropertyMap[] columnProp = ReportColumn.getProperties(ReportType.Region, tariff);
            ...
            Set<String> districts = result.keySet();
            int rowNumber = dataStartRow + 1;
            
            for ( String district : districts ) {
            	if ( district.equals("total") ) {
            		continue;
            	}
            	...
            	Map<String, Object> record = result.get(district);
            	record.put(PropertyMap.district.getProp(), district);
            ...
            if(record != null) {
            	row = sheet.createRow(rowNumber);
            	record.put(PropertyMap.district.getProp(), "Total");
            	columnIndex = 0;
            	ExcelUtil.createDataRow(record, workbook, row, columnIndex, columnProp);
            }
            //End Data
