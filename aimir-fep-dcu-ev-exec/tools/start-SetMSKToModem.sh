...
count=`ps -eaf | grep pom-SetMSKToModem.xml | grep -v grep | wc -l`
if [ $count -gt 0 ]; then
...
/usr/bin/mvn --debug -e -f /home/aimir1/aimiramm.3.3/aimir-fep-exec/pom-SetMSKToModem.xml antrun:run -Dspring.instrument.path=/home/aimir1/.m2/repository/org/springframework/spring-instrument/4.2.5.RELEASE > ./SetMSKToModem.log &
