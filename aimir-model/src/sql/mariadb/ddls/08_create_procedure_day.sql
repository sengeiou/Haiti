DROP PROCEDURE IF EXISTS AIMIR.BATCH_DAY_EM;

DELIMITER //

CREATE PROCEDURE AIMIR.BATCH_DAY_EM
 BEGIN        
...
V_SQL := 'TRUNCATE TABLE AIMIR.DUMMY_LPEM_TO_DAYEM';        
EXECUTE IMMEDIATE V_SQL;        
INSERT INTO AIMIR.DUMMY_LPEM_TO_DAYEM SELECT /*+ INDEX(LPEM_TO_DAYEM IDX_LPEM_TO_DAYEM) */ MDEV_ID, YYYYMMDDHHMISS, CHANNEL, MDEV_TYPE, DST, WRITEDATE FROM AIMIR.LPEM_TO_DAYEM
 LIMIT 100000;      
      
V_SQL := 'ANALYZE TABLE AIMIR.DUMMY_LPEM_TO_DAYEM ESTIMATE STATISTICS';      
EXECUTE IMMEDIATE V_SQL;   
           
MERGE INTO AIMIR.DAY_EM T_TABLE USING       
 (SELECT A.MDEV_ID, A.YYYYMMDD, A.CHANNEL, A.MDEV_TYPE, A.DST, A.HH, METER.ID METER_ID, A.METERINGTYPE, SUBSTR(A.MDEV_ID, CHAR_LENGTH(A.MDEV_ID), 1) MDEV_ID_LAST, MODEM.ID MODEM_ID,       
  ...
  FROM (SELECT MDEV_ID, SUBSTR(YYYYMMDDHHMISS, 1, 8) YYYYMMDD, CHANNEL, MDEV_TYPE, DST, SUBSTR(YYYYMMDDHHMISS, 9, 2) HH, DEVICE_ID, DEVICE_TYPE, WRITEDATE, METERINGTYPE, INTERVAL_YN, VALUE FROM
        (SELECT /*+ RESULT_CACHE */ A.MDEV_ID, A.YYYYMMDDHHMISS, A.CHANNEL, A.MDEV_TYPE, A.DST, A.DEVICE_ID, A.DEVICE_TYPE, A.WRITEDATE, A.METERINGTYPE, A.INTERVAL_YN, A.VALUE VALUE, A.CONTRACT_ID
          FROM AIMIR.LP_EM A JOIN (SELECT /*+ INDEX_FFS(DUMMY_LPEM_TO_DAYEM, IDX_DUMMY_LPEM_TO_DAYEM) */ MDEV_ID, SUBSTR(YYYYMMDDHHMISS, 1, 8) YYYYMMDD, CHANNEL, MDEV_TYPE, DST FROM AIMIR.DUMMY_LPEM_TO_DAYEM GROUP BY MDEV_ID, SUBSTR(YYYYMMDDHHMISS, 1, 8), CHANNEL, MDEV_TYPE, DST) B     
          ON A.MDEV_ID = B.MDEV_ID AND A.YYYYMMDDHHMISS BETWEEN CONCAT(IFNULL(B.YYYYMMDD, ''),'000000') AND CONCAT(IFNULL(B.YYYYMMDD, ''),'235959') AND A.CHANNEL = B.CHANNEL AND A.MDEV_TYPE = B.MDEV_TYPE AND A.DST = B.DST) 
        ...
        LEFT JOIN (SELECT A.MDEV_ID, SUBSTR(A.YYYYMMDDHHMISS, 1, 8) YYYYMMDD, A.CHANNEL, A.MDEV_TYPE, A.DST, A.VALUE BASEVALUE FROM
                   (SELECT /*+ RESULT_CACHE */ A.MDEV_ID, A.YYYYMMDDHHMISS, A.CHANNEL, A.MDEV_TYPE, A.DST, A.DEVICE_ID, A.DEVICE_TYPE, A.WRITEDATE, A.METERINGTYPE, A.INTERVAL_YN, A.VALUE VALUE, A.CONTRACT_ID
                    FROM AIMIR.LP_EM A JOIN (SELECT /*+ INDEX_FFS(DUMMY_LPEM_TO_DAYEM, IDX_DUMMY_LPEM_TO_DAYEM) */ MDEV_ID, SUBSTR(YYYYMMDDHHMISS, 1, 8) YYYYMMDD, CHANNEL, MDEV_TYPE, DST FROM AIMIR.DUMMY_LPEM_TO_DAYEM GROUP BY MDEV_ID, SUBSTR(YYYYMMDDHHMISS, 1, 8), CHANNEL, MDEV_TYPE, DST) B     
                    ON A.MDEV_ID = B.MDEV_ID AND A.YYYYMMDDHHMISS BETWEEN CONCAT(IFNULL(B.YYYYMMDD, ''),'000000') AND CONCAT(IFNULL(B.YYYYMMDD, ''),'235959') AND A.CHANNEL = B.CHANNEL AND A.MDEV_TYPE = B.MDEV_TYPE AND A.DST = B.DST) A
                   JOIN
  ...
  GROUP BY A.MDEV_ID, A.YYYYMMDD, A.CHANNEL, A.MDEV_TYPE, A.DST, A.HH, METER.ID, A.METERINGTYPE, SUBSTR(A.MDEV_ID, CHAR_LENGTH(A.MDEV_ID), 1), MODEM.ID, A.DEVICE_ID, A.DEVICE_TYPE, M_BASEVALUE.BASEVALUE, SUPPLIER.ID, CH_METHOD, CONTRACT_VALUE.CONTRACT_ID, ENDDEVICE.ID) S_TABLE       
...
WHEN NOT MATCHED THEN INSERT SELECT (S_TABLE.MDEV_ID, S_TABLE.YYYYMMDD, S_TABLE.CHANNEL, S_TABLE.MDEV_TYPE, S_TABLE.DST, S_TABLE.HH, S_TABLE.METER_ID, S_TABLE.METERINGTYPE, S_TABLE.MDEV_ID_LAST, S_TABLE.MODEM_ID, S_TABLE.DEVICE_ID, S_TABLE.DEVICE_TYPE, S_TABLE.C_VALUE, S_TABLE.BASEVALUE, S_TABLE.VALUE, S_TABLE.WRITEDATE, S_TABLE.SUPPLIER_ID, S_TABLE.CH_METHOD, S_TABLE.CONTRACT_ID, S_TABLE.ENDDEVICE_ID);        
       
DELETE FROM AIMIR.LPEM_TO_DAYEM A WHERE EXISTS         
(SELECT 1 FROM AIMIR.DUMMY_LPEM_TO_DAYEM B WHERE A.MDEV_ID = B.MDEV_ID AND A.YYYYMMDDHHMISS = B.YYYYMMDDHHMISS AND A.CHANNEL = B.CHANNEL AND A.MDEV_TYPE = B.MDEV_TYPE AND A.DST = B.DST AND A.WRITEDATE = B.WRITEDATE);         
