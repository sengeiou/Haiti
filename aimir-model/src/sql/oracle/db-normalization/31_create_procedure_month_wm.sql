--DAY to MONTH 
CREATE OR REPLACE PROCEDURE AIMIR.BATCH_MONTH_WM IS
...
V_SQL := 'TRUNCATE TABLE AIMIR.DUMMY_DAYWM_TO_MONTHWM';
EXECUTE IMMEDIATE V_SQL;
INSERT INTO AIMIR.DUMMY_DAYWM_TO_MONTHWM SELECT /*+ INDEX(DAYWM_TO_MONTHWM IDX_DAYWM_TO_MONTHWM) */ MDEV_ID, YYYYMMDD, CHANNEL, MDEV_TYPE, DST, HH, WRITEDATE FROM AIMIR.DAYWM_TO_MONTHWM WHERE ROWNUM <= 100000;      
V_SQL := 'ANALYZE TABLE AIMIR.DUMMY_DAYWM_TO_MONTHWM ESTIMATE STATISTICS';      
EXECUTE IMMEDIATE V_SQL;


MERGE INTO AIMIR.MONTH_WM T_TABLE USING
 ...
 MIN(A.WRITEDATE) WRITEDATE, A.SUPPLIER_ID, A.CH_METHOD, CONTRACT_VALUE.CONTRACT_ID CONTRACT_ID, A.ENDDEVICE_ID FROM
  (SELECT MDEV_ID, SUBSTR(YYYYMMDD, 1, 6) YYYYMM, CHANNEL, MDEV_TYPE, DST, SUBSTR(YYYYMMDD, 7, 2) DD, METER_ID, METERINGTYPE, MDEV_ID_LAST, MODEM_ID, DEVICE_ID, DEVICE_TYPE, WRITEDATE, SUPPLIER_ID, CH_METHOD, ENDDEVICE_ID, C_VALUE, BASEVALUE, VALUE FROM
   ...
   FROM AIMIR.DAY_WM A JOIN (SELECT /*+ INDEX_FFS(DUMMY_DAYWM_TO_MONTHWM, IDX_DUMMY_DAYWM_TO_MONTHWM) */ MDEV_ID, SUBSTR(YYYYMMDD, 1, 6) YYYYMM, CHANNEL, MDEV_TYPE, DST FROM AIMIR.DUMMY_DAYWM_TO_MONTHWM GROUP BY MDEV_ID, SUBSTR(YYYYMMDD, 1, 6), CHANNEL, MDEV_TYPE, DST) B     
...
DELETE FROM AIMIR.DAYWM_TO_MONTHWM A WHERE EXISTS         
(SELECT 1 FROM AIMIR.DUMMY_DAYWM_TO_MONTHWM B WHERE A.MDEV_ID = B.MDEV_ID AND A.YYYYMMDD = B.YYYYMMDD AND A.CHANNEL = B.CHANNEL AND A.MDEV_TYPE = B.MDEV_TYPE AND A.DST = B.DST AND A.HH = B.HH AND A.WRITEDATE = B.WRITEDATE);
