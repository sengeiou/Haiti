 ...
 * 
 * <pre>
 * 검침데이터의 주기별 데이터 LoadProfile 데이터를 TimeInterval 별로 저장하는 클래스
 * LP테이블은 전기(EM) 가스(GM) 수도(WM) 로 테이블 명을 구분한다. 
 * 일 테이블은 해당일의 매 시간별 주기데이터 
 * 예를들어 검침주기가 15분 단위면 해당 시간의 value_00, value_15, value_30, value_45에 해당하는 단위 사용량 값이 채워지게 된다.
 * </pre>
 * 
 * @author goodjob
 *
 */
@MappedSuperclass
//@AdditionalCriteria("COLUMN('MDEV_ID_LAST', this) = SUBSTRING(:mdevId, LENGTH(:mdevId), 1)")
//@AdditionalCriteria("COLUMN('MDEV_ID_LAST', this) = SQL('(SELECT SUBSTR(?, LENGTH(?), 1) FROM DUAL)', :mdevId, :mdevId)")
public abstract class MeteringLP {
	...
	@ColumnInfo(name="검침값의 개수", descr="LP missing 처리를 위해 검침값의 개수를 파악하여 입력한다.")
...
/*	public void setDeviceType(Integer deviceType) {
	    for (DeviceType _deviceType : DeviceType.values()) {
	        if (_deviceType.getCode() == deviceType)
    ...
    public int getValueCnt() {
        if (valueCnt == null) return 0;
        else return valueCnt;
    }

    public void setValueCnt(Integer valueCnt) {
