...
public class IF4_1_2_BYPASS_SG {
    @Test
    public void testOnDemandByMeter(){
    	NioSocketConnector connector = new NioSocketConnector();
    	IF4_1_2_BYPASS_Handler_SG handler = new IF4_1_2_BYPASS_Handler_SG();
    	...
    	ConnectFuture future = connector.connect(new InetSocketAddress("187.1.10.58", 8900));
...
class IF4_1_2_BYPASS_Handler_SG implements IoHandler {
	private static Log log = LogFactory.getLog(IF4_1_2_BYPASS_Handler_SG.class);
	
	private String NAME_SPACE = "SG";
	...
	throws Exception
	{
        ...
        if (cmd.equals("cmdIdentifyDevice")) {
            ...
            CommandData cd = command("cmdMeterTimeSync", null);
            response(session, cd);
            session.write(new ControlDataFrame(ControlDataConstants.CODE_EOT));
        }
        else if (cmd.equals("cmdReadModemConfiguration")) {
            CommandData cd = command("cmdReadModemConfiguration", null);
            response(session, cd);
            session.write(new ControlDataFrame(ControlDataConstants.CODE_EOT));
        }
        else if (cmd.equals("cmdReadNMSInformation")) {
            CommandData cd = command("cmdReadNMSInformation", null);
            response(session, cd);
            session.write(new ControlDataFrame(ControlDataConstants.CODE_EOT));
        }
	}
