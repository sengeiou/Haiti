...
public class EV_SP_200_59_0_Action implements EV_Action {
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		try {
			...
			logger.debug("[EV_SP_200_59_0_Action] requestId={}", requestId);
			...
			logger.debug("[EV_SP_200_59_0_Action] UpgradeType={}, TargetClass={}", upgardeType, targetClass.name());
			...
			logger.debug("[EV_SP_200_59_0_Action] version={}", version);

			String targetModel = StringUtil.nullToBlank(event.getEventAttrValue("stringEntry.1")); // Thirdparty 방식일때만 값이 채워져서 올라옴.
			logger.debug("[EV_SP_200_59_0_Action] targetModel={}", targetModel);
