 ...
 */
@Component
public class EV_PH_200_64_0_Action implements EV_Action {
	private static Logger logger = LoggerFactory.getLogger(EV_PH_200_64_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		logger.debug("[EV_PH_200_64_0_Action][evtOTADownloadStart][{}] Execute start.", EVENT_MESSAGE);
		...
		try {
			...
			logger.debug("[EV_PH_200_64_0_Action][evtOTADownloadStart] DCU = {}({}), EventCode = {}", mcuId, trap.getIpAddr(), trap.getCode());

			String requestId = StringUtil.nullToBlank(event.getEventAttrValue("uintEntry"));
			logger.debug("[EV_PH_200_64_0_Action] requestId={}", requestId);

			String upgradeType = StringUtil.nullToBlank(event.getEventAttrValue("byteEntry"));
			logger.debug("[EV_PH_200_64_0_Action] upgradeType={}, TargetClass={}", OTA_UPGRADE_TYPE.getItem(upgradeType), OTA_UPGRADE_TYPE.getItem(upgradeType).getTargetClass().name());

			String targetId = StringUtil.nullToBlank(event.getEventAttrValue("stringEntry"));
			logger.debug("[EV_PH_200_64_0_Action] targetId={}", targetId);

			String targetModel = StringUtil.nullToBlank(event.getEventAttrValue("stringEntry.1")); // Thirdparty 방식일때만 값이 채워져서 올라옴.
			logger.debug("[EV_PH_200_64_0_Action] targetModel={}", targetModel);
			if (mcu != null) {
				...
				logger.debug("[EV_PH_200_64_0_Action][evtOTADownloadStart] DCU = {}({}) : Unknown DCU", trap.getMcuId(), trap.getIpAddr());
			}
		} catch (Exception e) {
			logger.error("[EV_PH_200_64_0_Action][evtOTADownloadStart] Error - ", e);
		}
		logger.debug("[EV_PH_200_64_0_Action][evtOTADownloadStart][{}] Execute stop.", EVENT_MESSAGE);
	}

	/**
	 * EV_PH_200_64_0 Event Make
	 * 
	 * @param activatorType
	 * @param activatorId
	 * @param targetType
	...
	public void makeEvent(TargetClass activatorType, String activatorId, TargetClass targetType, String openTime, String operatorType, Location location) {
		logger.debug("[EV_PH_200_64_0_Action][evtOTADownloadStart] MakeEvent.");
		...
		try {
			...
			logger.debug("[EV_PH_200_64_0_Action][openTime={}] evtOTADownloadStart - {}", openTime, resultValue);
		} catch (Exception e) {
