...
/**
 * 200.2 evtDcuStartup OID 3 True Normal DCU Startup
 */
@Component
public class EV_LK_200_2_0_Action implements EV_Action {
	private static Log log = LogFactory.getLog(EV_LK_200_2_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		log.debug("EventName[evtDcuStartup] " + " EventCode[" + trap.getCode() + "] DCU[" + trap.getMcuId() + "]");
		...
		String sysUptime = event.getEventAttrValue("sysUptime") == null ? "?" : event.getEventAttrValue("reason");  // Boot-up 이후 경과된 시간 (초)
		log.info("sysUptime [" + sysUptime + "]s");
		...
		try {
			...
			if (existMcu == null) {
				...
				existMcu.setSysUpTime(sysUptime);
			...
			event.append(EventUtil.makeEventAlertAttr("evtName", "java.lang.String", "DCU Startup - " + sysUptime + "s"));
		} catch (Exception e) {
			log.error("evtDcuStartup error - " + e.getMessage(), e);
		} finally {
			if (txstatus != null)
				txmanager.commit(txstatus);
		}

		log.debug("DCU Startup Action Complete");
