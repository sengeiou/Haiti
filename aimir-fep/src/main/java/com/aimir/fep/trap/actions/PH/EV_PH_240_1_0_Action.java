 ...
 * Event ID : 240.1.0 evtInstallModem
	31.5.1	moSPId	EUI64	8
	31.5.2	moSPResetTime	BYTE	1
	31.5.3	moSPNodeKind	STRING	20
	31.5.4	moSPFwVer	WORD	2
	31.5.5	moSPFwBuild	WORD	2
	31.5.6	moSPHwVer	WORD	2
	31.5.7	moSPStatus	BOOL	1
	31.5.8	moSPMode	BOOL	1
	31.5.9	moSPLastOnLine	TIMESTAMP	7
	31.5.10	moSPLastOffLine	TIMESTAMP	7
	31.5.11	moSPInstall	TIMESTAMP	7
    31.5.12	moSPBootloaderVer	WORD	2
 */
@Service
public class EV_PH_240_1_0_Action implements EV_Action {
	private static Log log = LogFactory.getLog(EV_PH_240_1_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		try {
			// DCU Codi
			if (nodeKind.equals("HDPLC-Master")) {
				try {
					...
					log.debug("DCU[" + mcu.getSysID() + "]  Codi[" + mcuc.getCodiID() + "] registration success.");

				} catch (Exception e) {
					log.error("DCU Codi registration error - " + e.getMessage(), e);
				}
			}
			// PLC Modem
			else if (nodeKind.startsWith("NAMR-C10")) {
				...
				if (modem == null) {
					...
					modem.setModemType(ModemType.PLCIU.name());
					...
					List<DeviceModel> models = deviceModelDao.getDeviceModelByName(mcu.getSupplierId(), nodeKind);
					...
					if (mcu.getMacAddr() != null && !"".equals(mcu.getMacAddr())) {
						((PLCIU) modem).setIpv6Address(Util.getIPv6(mcu.getIpv6Addr(), modemId));
					}
				...
				throw new Exception("Unknown Model Type modem. - " + nodeKind);
			}
		} catch (Exception e) {
			log.error("evtInstallModem save error - " + e.getMessage(), e);
			if (txStatus != null) {
