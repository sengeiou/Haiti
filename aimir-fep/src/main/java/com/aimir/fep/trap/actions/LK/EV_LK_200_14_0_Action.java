...
/*
	evtNetworkDown - Network status change(Down)
 ...
 */
@Component
public class EV_LK_200_14_0_Action implements EV_Action {
	private static Log log = LogFactory.getLog(EV_LK_200_14_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		log.debug("EventName[evtNetworkDown] " + " EventCode[" + trap.getCode() + "] DCU[" + trap.getMcuId() + "]");
...
//				existMcu.setIpAddr(mcu.getIpAddr());	evtIpChange 이벤트로만 ip변경하도록 수정.
			...
			log.error("evtNetworkDown error - " + e.getMessage(), e);
		} finally {
			if (txstatus != null)
				txmanager.commit(txstatus);
		}

		log.debug("evtNetworkDown Action Compelte");
	}
}
