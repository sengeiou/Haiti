...
/*
 * 200.23 evtPowerFail OID 3 True Major DCU Power Fail
 *
 */

@Component
public class EV_LK_200_23_0_Action implements EV_Action {
	private static Log log = LogFactory.getLog(EV_LK_200_23_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		log.debug("EventName[eventPowerFail] " + " EventCode[" + trap.getCode() + "] DCU[" + trap.getMcuId() + "]");

		// Get Attributes
		String mcuId = trap.getMcuId() == null ? "" : trap.getMcuId();
		...
		try {
			...
			if (existMcu == null) {
				...
				existMcu.setNetworkStatus(0);
				existMcu.setPowerState(1);
			...
			event.getEventAlert().setName("Power Fail");
		} catch (Exception e) {
			log.error("eventPowerFail error - " + e.getMessage(), e);
		} finally {
			if (txstatus != null)
