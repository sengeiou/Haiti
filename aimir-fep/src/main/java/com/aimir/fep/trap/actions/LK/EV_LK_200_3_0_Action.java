...
/*
 * 200.3	evtReboot	OID	3	True	Minor		DCU Reboot
     1.4	reason	BYTE	1				Reboot reason (1: LowBattery, 2: User, 3: Malfunction, 4: Recovery, 5: Scheduled, 255: Unknown)

 * @author sjhan
 */
public class EV_LK_200_3_0_Action implements EV_Action {
	private static Log log = LogFactory.getLog(EV_LK_200_3_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		log.debug("EventName[evtReboot] " + " EventCode[" + trap.getCode() + "] DCU[" + trap.getMcuId() + "]");
		...
		try {
			...
			log.error("evtReboot error - " + e.getMessage(), e);
		} finally {
			if (txstatus != null)
				txmanager.commit(txstatus);
		}

		log.debug("DCU Reboot Action Complete");
	}
}
