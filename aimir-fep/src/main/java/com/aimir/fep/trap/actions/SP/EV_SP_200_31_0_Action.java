 ...
 */
@Component
public class EV_SP_200_31_0_Action implements EV_Action {

	private static Logger logger = LoggerFactory.getLogger(EV_SP_200_31_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		logger.debug("[EV_SP_200_31_0_Action][evtIntergrityDeviation][{}] Execute.", EVENT_MESSAGE);
		...
		try {
			...
			logger.debug("[EV_SP_200_31_0_Action][evtIntergrityDeviation] DCU = {}({}), EventCode = {}", trap.getMcuId(), trap.getIpAddr(), trap.getCode());

			String requestId = StringUtil.nullToBlank(event.getEventAttrValue("uintEntry"));
			logger.debug("[EV_SP_200_31_0_Action] requestId={}", requestId);

			String upgradeType = StringUtil.nullToBlank(event.getEventAttrValue("byteEntry"));
			logger.debug("[EV_SP_200_31_0_Action] upgradeType={}, TargetClass={}", OTA_UPGRADE_TYPE.getItem(upgradeType), OTA_UPGRADE_TYPE.getItem(upgradeType).getTargetClass().name());

			String imageUrl = StringUtil.nullToBlank(event.getEventAttrValue("stringEntry"));
			logger.debug("[EV_SP_200_31_0_Action] imageUrl={}", imageUrl);
			...
			logger.debug("[EV_SP_200_31_0_Action] resultCode={}, result={}", resultCode, OTA_UPGRADE_RESULT_CODE.getItem(resultCode).getDesc());
			
			String targetModel = StringUtil.nullToBlank(event.getEventAttrValue("stringEntry.2"));  // Thirdparty 방식일때만 값이 채워져서 올라옴.
			logger.debug("[EV_SP_200_31_0_Action] targetModel={}", targetModel);
			...
			if (mcu != null) {
				...
				logger.debug("[EV_SP_200_31_0_Action][evtIntergrityDeviation] DCU = {}({}) : Unknown MCU", trap.getMcuId(), trap.getIpAddr());
			}
		} catch (Exception e) {
			logger.error("[EV_SP_200_31_0_Action][evtIntergrityDeviation] Error - ", e);
		}

	}

	/**
	 * EV_SP_200_31_0_Action Event make
	 * 
	 * @param activatorType
	 * @param activatorId
	 * @param targetType
	...
	public void makeEvent(TargetClass activatorType, String activatorId, TargetClass targetType, String openTime, OTA_UPGRADE_RESULT_CODE resultCode, String operatorType) {
		...
		try {
			...
			logger.debug("[EV_SP_200_31_0_Action][openTime={}] evtIntergrityDeviation - {}", openTime, resultValue);
		} catch (Exception e) {
