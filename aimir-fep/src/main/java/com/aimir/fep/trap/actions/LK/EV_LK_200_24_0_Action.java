...
/*
 * 200.24	evtPowerRestore	OID	3	True	Minor		DCU Power Restore
 *
 */

@Component
public class EV_LK_200_24_0_Action implements EV_Action{
    private static Log log = LogFactory.getLog(EV_LK_200_24_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		log.debug("EventName[eventPowerRestore] " + " EventCode[" + trap.getCode() + "] DCU[" + trap.getMcuId() + "]");
		...
		try {
			...
			if (existMcu == null) {
				...
				existMcu.setNetworkStatus(1);
				existMcu.setPowerState(0);
			...
			event.getEventAlert().setName("Power Restore");
		} catch (Exception e) {
			log.error("eventPowerRestore error - " + e.getMessage(), e);
		} finally {
			if (txstatus != null)
				txmanager.commit(txstatus);
		}

		log.debug("DCU Power Restore Action Compelte");
	}
}
