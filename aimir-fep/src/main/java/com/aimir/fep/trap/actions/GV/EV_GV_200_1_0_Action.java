...
public class EV_GV_200_1_0_Action implements EV_Action {
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		log.debug("EventName[eventModemInstall] modelName : " + modelName);
		...
		log.info("fwVer : " + fwVer);
		
		TransactionStatus txstatus = null;

		try {
			txstatus = txmanager.getTransaction(null);
			log.debug("txstatus : " + txstatus.toString());

			// get modem
			Supplier supplier = supplierDao.getSupplierByName(supplierName);
			if(supplier == null) supplier = supplierDao.getAll().get(0);
...
//				modem.setLastLinkTime(DateTimeUtil.getDST(supplier.getTimezone().getName(), commDate));
				modem.setLastLinkTime(DateTimeUtil.getDST(supplier.getTimezone().getName(), notTime));
...
//	    	 	}
				...
				if (meter != null) {
					...
					if (meterModel != null) {
						...
						log.info("Update meterModel=" + meterModel);
						
						if("LV3410CT-010M".equals(meterModel)) {
							meterModel = "LSKLV3410CT-010M";
							log.info("Update meterModel=" + meterModel);
						}
...
//					meter.setLpInterval(60);
					...
					if(needUpdate != null) {
						if(needUpdate.equals(Boolean.FALSE)) {
							meterDao.add(meter);
						}else if(needUpdate.equals(Boolean.TRUE)) {
