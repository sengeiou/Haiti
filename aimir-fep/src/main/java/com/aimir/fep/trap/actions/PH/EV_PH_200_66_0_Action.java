...
/*
 * Event ID : EV_PH_200_66_0 OTAResult event Processing Class
 *
 ...
 */
@Component
public class EV_PH_200_66_0_Action implements EV_Action {
	private static Logger logger = LoggerFactory.getLogger(EV_PH_200_66_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		logger.debug("[EV_PH_200_66_0_Action][evtOTADownloadResult][{}] Execute.", EVENT_MESSAGE);
		...
		try {
			...
			logger.debug("[EV_PH_200_66_0_Action][evtOTADownloadResult] DCU = {}({}), EventCode = {}", trap.getMcuId(), trap.getIpAddr(), trap.getCode());

			String requestId = StringUtil.nullToBlank(event.getEventAttrValue("uintEntry"));
			logger.debug("[EV_PH_200_66_0_Action] requestId={}", requestId);

			String upgradeType = StringUtil.nullToBlank(event.getEventAttrValue("byteEntry"));
			logger.debug("[EV_PH_200_66_0_Action] upgradeType={}, TargetClass={}", OTA_UPGRADE_TYPE.getItem(upgradeType), OTA_UPGRADE_TYPE.getItem(upgradeType).getTargetClass().name());

			String targetId = StringUtil.nullToBlank(event.getEventAttrValue("stringEntry"));
			logger.debug("[EV_PH_200_66_0_Action] targetId={}", targetId);
			...
			logger.debug("[EV_PH_200_66_0_Action] result={}", OTA_UPGRADE_RESULT_CODE.getItem(resultCode).getDesc());

			String targetModel = StringUtil.nullToBlank(event.getEventAttrValue("stringEntry.2")); // Thirdparty 방식일때만 값이 채워져서 올라옴.
			logger.debug("[EV_PH_200_66_0_Action] targetModel={}", targetModel);
			...
			if (mcu != null) {
				...
				logger.debug("[EV_PH_200_66_0_Action][evtOTADownloadResult] DCU = {}({}) : Unknown MCU", trap.getMcuId(), trap.getIpAddr());
			}
		} catch (Exception e) {
			logger.error("[EV_PH_200_66_0_Action][evtOTADownloadResult] Error - ", e);
		}
	}

	/**
	 * EV_PH_200_66_0 Event Make
	 * 
	 * @param activatorType
	 * @param activatorId
	 * @param targetType
	...
	public void makeEvent(TargetClass activatorType, String activatorId, TargetClass targetType, String openTime, OTA_UPGRADE_RESULT_CODE resultCode, String message, String operatorType, Location location) {
		logger.debug("[EV_PH_200_66_0_Action][activatorId={}][evtOTADownloadResult] MakeEvent.", activatorId);
		...
		try {
			...
			logger.debug("[EV_PH_200_66_0_Action][activatorId={}][openTime={}] evtOTADownloadResult - {}", activatorId, openTime, resultValue);
		} catch (Exception e) {
	...
	private void updateMeterFWVersion(String deviceId, DeviceType deviceType, OTA_UPGRADE_RESULT_CODE resultCode, String fwVersion) {
		...
		try {
			...
			txManager.commit(txStatus);
			logger.debug("[EV_PH_200_66_0_Action] updateMeterFWVersion F/W Version =[" + fwVersion + "]");
		} catch (Exception e) {
			logger.error("[EV_PH_200_66_0_Action] updateMeterFWVersion Error - " + e, e);
			if (txStatus != null) {
				txManager.rollback(txStatus);
			}
			//throw e;
