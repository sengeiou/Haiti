...
public class DFProcessor extends Processor
{
    ...
    private LinkedHashMap<String, ArrayList<String>> saveMeasurementData(MDData data)
    {
        ...
        try {
            oneEMDataListResult= mdsMain.save(mdHistoryData, false);
    ...
    public int processing(Object data) throws Exception
    {
        ...
        if (data instanceof DFData) {
            ...
            dataCnt = readData((String)data, null);  // 멀티프레임으로 오면 NameSpace가 안채워짐.
        }
        else {
    ...
    private int readData(String filename, String ns) throws Exception
    {
        ...
        try {
        	...
        	if (f.exists()) {
        		commTime = DateTimeUtil.getDateString(f.lastModified());
        	}
        	else {
        		commTime = DateTimeUtil.getDateString(new Date());
        	}
            ...
            dataCnt = readData(is, ns, commTime);
		} catch (Exception e) {
			log.error("readData Error - " + e.getMessage(), e);
		} 
        finally {
            if (is != null)
                is.close();
            ...
            try {
            	Files.delete(file);
            	log.debug("Uncompress Data file deleted. - " + filename);
            } catch (IOException x) {
            	log.warn("Deletion failed "+filename);
            } 
        }
    ...
    private int readData(InputStream is, String ns, String commTime) throws Exception
    {
        byte[] soh = new byte[1];
            ...
            Thread instream = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        log.warn("UnCompress Error - " + e.getMessage(), e);
                    }
                    finally {
                        try {
                            if (br != null)
            ...
            Thread errorstream = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        while ((line = br.readLine()) != null) {
                            log.debug("ErrorStrem line = " + line);
                        }
                    }
                    catch (IOException e) {
                        log.warn("ErrorStream process error - " + e.getMessage(), e);
                    }
                    finally {
                        try {
                            if (br != null)
        ...
        do {
            ...
            for (int i = 0; i < datacnt; i++) {
                ...
                if (kafkaEnable) {
                    ...
                    commLog.setData(bos.toByteArray());
                    commLog.setDataType(ProcessorHandler.SERVICE_MEASUREMENTDATA);
                    commLog.setSenderId(mcuId);
            		commLog.setStartDateTime(DateTimeUtil.getDateString(new Date()));//Start Time은 sessionOpen 시 지정
