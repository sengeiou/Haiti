...
public class SnmpManagerAdapter implements SnmpManagerAdapterMBean, MBeanRegistration {
    ...
    private static Log logger = LogFactory.getLog(SnmpManagerAdapter.class);
    ...
    /*
    	You have to configure iptables properly to forward all incoming UDP packets from port 162 to user defined port (>1024).

    	sudo iptables -t nat -A PREROUTING -i em1 -p udp --dport 162 -j REDIRECT --to-port 16200
    */
    ...
    public SnmpManagerAdapter(){
        ...
        int maxPoolSize = Integer.parseInt(FMPProperty.getProperty("executor.max.pool.size", defaultMaxPoolSize));
        System.setProperty("actors.maxPoolSize", String.valueOf(maxPoolSize));
    ...
    public void start() throws Exception {
        ...
        try {
            ...
            logger.error("objectName["+objectName+"] start failed");
            state = STOPPED;
            throw ex;
        }
    }
