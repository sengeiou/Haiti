...
public class GeneralFrameForMBB {
    private static Log log = LogFactory.getLog(GeneralFrameForMBB.class);
    ...
    public void setMonthValue(int monthVal) {
        this.month = monthVal;
    }
    ...
    public void setDayValue(int dayVal) {
        this.day = dayVal;
    }
...
//              value = Integer.parseInt(DataUtil.getBit(b[0])+DataUtil.getBit(b[1])+DataUtil.getBit(b[2])+DataUtil.getBit(b[3]));
            ...
            case "/6/n_m_s":
                ...
                if(data.length == 15) {
                    ...
                    b = new byte[1];
                    System.arraycopy(data, pos, b, 0, b.length);
                    int tempRssi=DataUtil.getIntToByte(b[0]);
                    if(tempRssi > 127)
                        rssi = (256 - tempRssi) * -1 ;
                    else{
                        if((tempRssi >= 0) && (tempRssi <= 31))
                            rssi = (-113) + (2*tempRssi);
                        else
                            rssi = 0; // unknown
                    }
