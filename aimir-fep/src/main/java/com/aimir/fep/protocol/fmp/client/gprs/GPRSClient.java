...
public class GPRSClient implements Client
{
    ...
    private Integer protocolType = 1; // new Integer(
    ...
    private synchronized void connect() throws Exception
    {
        ...
        for( ;; )
        {
            try
            {
                ...
                log.info(connector.getDefaultLocalAddress());
                ...
                session.getConfig().setIdleTime(IdleStatus.READER_IDLE, CONNECT_TIMEOUT);
    ...
    {
        log.debug("### GPRS Client Close Start. ###");
        ...
        log.debug("### GPRS Client Close Finished. ###");
    }
    ...
    { 
         ...
         log.debug("Compress mode used? = " + isUseCompress);
         if(isUseCompress){
         	frame.setAttrByte(GeneralDataConstants.ATTR_COMPRESS);
         }
