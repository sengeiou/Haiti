...
public class NiDtlsProtocolHandler extends NICommandActionHandlerAdaptor implements RawDataChannel
{
    ...
    public void receiveData(RawData rawData) {
    	if(isCmdAdapter) { //connect for HES to Device
    		SnowflakeGeneration.setSeq(sequenceLog);
    	} else { //connect for Device to HES
                    ...
                    if (node == null) {
                        ...
                        if (ns instanceof NetworkStatusSub1GhzForSORIA) {
                            ...
                            if (node.getMcu() == null || !mcu.getSysID().equals(node.getMcu().getSysID())) {
                                ...
                                try{
                                	SNRLogDao snrLogDao = DataUtil.getBean(SNRLogDao.class);
                                    SNRLog snrLog = new SNRLog();            
                                    snrLog.setDcuid(mcu.getSysID());
                                    snrLog.setDeviceId(node.getDeviceSerial());
                                    snrLog.setDeviceType(node.getModemType().name());
                                    snrLog.setYyyymmdd(DateTimeUtil.getCurrentDateTimeByFormat("yyyyMMdd"));
                                    snrLog.setHhmmss(DateTimeUtil.getCurrentDateTimeByFormat("HHmmss"));
