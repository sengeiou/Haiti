...
public class SORIAModemOTARunnable implements IBatchRunnable {
	private static Logger logger = LoggerFactory.getLogger(SORIAModemOTARunnable.class);
	...
	public SORIAModemOTARunnable(OTATargetType otaTargetType, Target target, Map<String, Object> params) {
		logger.debug("# SORIAModemOTARunnable = [{}][{}]", target.toString(), params.toString());
	...
	@Override
	public void run() {
		logger.debug("# SORIAModemOTARunnable Start.");
		logger.debug("Excute [START] OTATargetType={}, newFwVersion={}, Target={}", otaTargetType, newFwVersion, target);
		...
		try {
			...
			if (versionCheck) {
				...
				action.updateOTAHistory(
						  (otaTargetType == OTATargetType.MODEM ? target.getModemId() : target.getMeterId())
						, otaTargetDeviceType, openTime);
				...
				logger.debug("[SORIAModemOTARunnable] Excute START - TargetId={}, TargetType={}, ModemType={}, newFwVersion={}", target.getModemId(), otaTargetType, otaModemType, newFwVersion);
...
//					}
		...
		if (!executeResult) {
			...
			action3.updateOTAHistory(
					  (otaTargetType == OTATargetType.MODEM ? target.getModemId() : target.getMeterId())
					, otaTargetDeviceType, openTime, resultCode, errMsg);

			logger.debug("Execute Result fail. event message = {}", errMsg);
		}

		logger.debug("# SORIAModemOTARunnable End.");
	}
