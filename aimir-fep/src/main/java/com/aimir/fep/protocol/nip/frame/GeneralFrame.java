...
public class GeneralFrame {
	...
	public enum NIAttributeId {
        ...
        // SP-575 change start
        Bandwidth(new byte[] { (byte) 0xB7, (byte) 0x06 }),
        ...
        MeteringDataRequest(new byte[] { (byte) 0xC2, (byte) 0x04}),
		...
		// INSERT START SP-575
        RollbackImage(new byte[] { (byte) 0x00, (byte) 0x07 }),
        UploadMeteringData_UploadOnly(new byte[] { (byte) 0x00, (byte) 0x09 }),
        ModemDTLSHandshakeStatus(new byte[] { (byte) 0x10, (byte) 0x07 }),
        FWImageInformation(new byte[] { (byte) 0x10, (byte) 0x08 }),
        BootloaderInformation(new byte[] { (byte) 0x10, (byte) 0x09 }),
        ModemScheduleRun(new byte[] { (byte) 0x60, (byte) 0x01 }),
        CoordinatorEUI(new byte[] { (byte) 0xA0, (byte) 0x06 }),
        CoordinatorBroadcastConfiguration(new byte[] { (byte) 0xA0, (byte) 0x07 }),
        NetworkKey(new byte[] { (byte) 0xA0, (byte) 0x08 }),
        CoordinatorOne_TimeBroadcast(new byte[] { (byte) 0xA0, (byte) 0x09 }),
        Networkfilterrssivalue(new byte[] { (byte) 0xA0, (byte) 0x0A }),
        CertificateUpdate(new byte[] { (byte) 0xC0, (byte) 0x07 }),
        FactorySetting_Common(new byte[] { (byte) 0xB0, (byte) 0x01 }),
        Reset(new byte[] { (byte) 0xB0, (byte) 0x02 }),
        UtcTime(new byte[] { (byte) 0xB2, (byte) 0x01 }),
        TimeZone(new byte[] { (byte) 0xB2, (byte) 0x02 }),
        MeterSerialNumber(new byte[] { (byte) 0xB3, (byte) 0x01 }),
        MeterManufactureNumber(new byte[] { (byte) 0xB3, (byte) 0x02 }),
        CustomerNumber(new byte[] { (byte) 0xB3, (byte) 0x03 }),
        ModelName(new byte[] { (byte) 0xB3, (byte) 0x04 }),
        HWVersion(new byte[] { (byte) 0xB3, (byte) 0x05 }),
        SWVersion(new byte[] { (byte) 0xB3, (byte) 0x06 }),
        MeterStatus(new byte[] { (byte) 0xB3, (byte) 0x07 }),
        LastUpdateTime(new byte[] { (byte) 0xB3, (byte) 0x08 }),
        LastCommTime(new byte[] { (byte) 0xB3, (byte) 0x09 }),
        LPChannelCount(new byte[] { (byte) 0xB3, (byte) 0x0A }),
        LPInterval(new byte[] { (byte) 0xB3, (byte) 0x0B }),
        CumulativeActivePower(new byte[] { (byte) 0xB3, (byte) 0x0C }),
        CumulativeActivePowerTime(new byte[] { (byte) 0xB3, (byte) 0x0D }),
        CT(new byte[] { (byte) 0xB4, (byte) 0x01 }),
        PT(new byte[] { (byte) 0xB4, (byte) 0x02 }),
        TransformerRatio(new byte[] { (byte) 0xB4, (byte) 0x03 }),
        PhaseConfiguration(new byte[] { (byte) 0xB4, (byte) 0x04 }),
        SwitchStatus(new byte[] { (byte) 0xB4, (byte) 0x05 }),
        Frequency_Electric(new byte[] { (byte) 0xB4, (byte) 0x06 }),
        VA_SF(new byte[] { (byte) 0xB4, (byte) 0x07 }),
        VAH_SF(new byte[] { (byte) 0xB4, (byte) 0x08 }),
        DISP_SCALAR(new byte[] { (byte) 0xB4, (byte) 0x09 }),
        DISP_MULTIPLIER(new byte[] { (byte) 0xB4, (byte) 0x0A }),
        PrimaryPowerSourceType(new byte[] { (byte) 0xB5, (byte) 0x01 }),
        SecondaryPowerSourceType(new byte[] { (byte) 0xB5, (byte) 0x02 }),
        ResetCount(new byte[] { (byte) 0xB5, (byte) 0x03 }),
        ResetReason(new byte[] { (byte) 0xB5, (byte) 0x04 }),
        OperationTime(new byte[] { (byte) 0xB5, (byte) 0x05 }),
        ResetSchedule(new byte[] { (byte) 0xB5, (byte) 0x06 }),
        Type_Main(new byte[] { (byte) 0xB6, (byte) 0x01 }),
        Type_Type(new byte[] { (byte) 0xB6, (byte) 0x02 }),
        InterfaceIPv6Address(new byte[] { (byte) 0xB7, (byte) 0x01 }),
        IPv6Address(new byte[] { (byte) 0xB7, (byte) 0x02 }),
        InterfaceListenPort(new byte[] { (byte) 0xB7, (byte) 0x03 }),
        NetworkListenPort(new byte[] { (byte) 0xB7, (byte) 0x04 }),
        Frequency_6LoWPAN(new byte[] { (byte) 0xB7, (byte) 0x05 }),
        BaseStationAddress(new byte[] { (byte) 0xB7, (byte) 0x07 }),
        APPKey(new byte[] { (byte) 0xB7, (byte) 0x08 }),
        HopstoBaseStation(new byte[] { (byte) 0xB7, (byte) 0x09 }),
        EUI64(new byte[] { (byte) 0xB7, (byte) 0x0A }),
        ListenPort(new byte[] { (byte) 0xB7, (byte) 0x0B }),
        MaxHop(new byte[] { (byte) 0xB7, (byte) 0x0C }),
        Meteringschedule(new byte[] { (byte) 0xB8, (byte) 0x01 }),
        LPUploadschedule(new byte[] { (byte) 0xB8, (byte) 0x02 }),
        EventType(new byte[] { (byte) 0xB9, (byte) 0x01 }),
        NetworkChangeInformation(new byte[] { (byte) 0xC3, (byte) 0x06 }),
		// INSERT END SP-575		
        OndemandType(new byte[] { (byte) 0x10, (byte) 0x0A }),
    	//INSERT SP-919, SP-953
        ModemResetTimeRandomFactor(new byte[] { (byte) 0x20, (byte) 0x17 }),
		//SP-981, SP-982
		InitiateModuleUpgrade(new byte[] { (byte) 0x00, (byte) 0x0D }),
		MBBModuleInformation(new byte[] { (byte) 0x10, (byte) 0x0B });
	...
	public enum OndemandType {
		SMSMode((byte) 0x01), DirectMode((byte) 0x02);

		private byte code;

		OndemandType(byte code) {
			this.code = code;
		}
		...
		public static OndemandType getMode(byte value) {
			for (OndemandType ot : OndemandType.values()) {
				if (ot.code == value) {
					return ot;
				}
			}  
...
//			log.debug(Hex.decode(b));
    ...
    public byte[] txPowerFrame(byte[] tid) throws Exception {
        GeneralFrame frame = new GeneralFrame();
        frame._networkType = this._networkType;
        frame.fcPending = FrameControl_Pending.LastFrame;
        frame.fcAck = FrameControl_Ack.None;
        ...
        attr.newData(1);
        Data[] data = attr.getData();
        data[0].setId(NIAttributeId.ModemTXPower.getCode());
        
        int txPower = Integer.parseInt(FMPProperty.getProperty("mbb.txPower","168"));      
        data[0].setValue(new byte[] {(byte)txPower});
