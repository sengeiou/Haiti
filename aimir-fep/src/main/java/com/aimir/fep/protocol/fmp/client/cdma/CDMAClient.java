 ...
 */
public class CDMAClient implements Client
{
    private Log log = LogFactory.getLog(CDMAClient.class);
     ...
     */
    public CDMAClient() throws GeneralSecurityException, IOException
    {
    }
     ...
     * @throws GeneralSecurityException 
     */
    public CDMAClient(CDMATarget target) throws GeneralSecurityException, IOException
    {
        this.target = target;
    }
    ...
    private synchronized void connect() throws Exception
    {
        ...
        log.debug("CDMAClient Circuit Connection Mode["+connMode+"]");
        ...
        if(connMode == FMPClientResource.TERMINAL_SERVER)
        {
            ...
            log.debug("##### CDMAClient TS : Acquire ######");
            ...
            port = tsport.getPort();
            log.debug("CDMAClient TS : ipaddr["+ipaddr+"] port["+port+"]");
            log.debug("CDMAClient: "+resource.getActiveResourcesString());
            log.debug("CDMAClient: "+resource.getIdleResourcesString());
        }
        ...
        try
        {
            ...
            log.debug("CDMAClient initCircuit End");

            //handler.waitENQ();
        	handler.waitIF4Nego(session);
            log.debug("CDMAClient waitENQ End");
        }catch(Exception ex)
        {
            close();
            throw ex;
    ...
    {
        log.debug("CDMAClient::setLogProcessor");
        ...
        throws Exception
    {
        ...
        log.info("CDMAClient::sendCommand("+command+")");
        try {
            if(session == null || !session.isConnected())
                connect();
            ...
            log.debug("CDMAClient::sendCommand::start Wait Response");
            sd = handler.getResponse(session,command.getTid().getValue());
            log.debug("CDMAClient::sendCommand::end Wait Response");
    ...
    /**
     * close CDMAClient session and wait completed
     *
