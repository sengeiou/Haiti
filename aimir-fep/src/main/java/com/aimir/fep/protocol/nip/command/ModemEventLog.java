...
//        System.out.println(new String(Hex.decode(out.toByteArray())));
    ...
    public void decode2(byte[] bx) {
        ...
        for (int i = 0; i < logDatas.length; i++) {
            ...
            b = new byte[7];
            System.arraycopy(bx, pos, b, 0, b.length);
            String timeStr = CreateDateFormat(b);
            pos += b.length;
            logDatas[i].setTime(timeStr);
            b = new byte[2];
            System.arraycopy(bx, pos, b, 0, b.length);
            pos += b.length;
            logDatas[i].setCode(getHexDump(b));
    ...
    public String toString2() {
        ...
        for(int i=0; i< logDatas.length ;i++){
            ...
            rtn.append("Index: "+((LogData)logDatas[i]).getIdx()+", ");
            rtn.append("Time: "+((LogData)logDatas[i]).getTime()+", ");
            rtn.append("Code: "+eventName+"("+((LogData)logDatas[i]).getCode()+"), ");
            rtn.append("Data: "+descr+"("+((LogData)logDatas[i]).getValue()+")]");
            if(i != (logDatas.length-1)){
            	rtn.append(", \n");
            }
        }
        return "Event Log Count: "+logCount+", \n"+
        rtn.toString();	   
    }
