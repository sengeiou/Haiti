...
public class FMPProtocolHandler extends IoHandlerAdapter {
	...
	public void exceptionCaught(IoSession session, Throwable cause) {
		...
		try {
			if (cause.getMessage().contains("SSL handshake failed")) {
				...
				try {
					EventUtil.sendEvent("Security Alarm", TargetClass.Unknown, activatorId, new String[][] { { "message", "Uncertificated Access" } });
				} catch (Exception e) {
					log.error("[Security Alarm] Save error - " + e.getMessage(), e);
				}
			...
			log.error("Handler Exception error from [" + session.getRemoteAddress() + "] - " + cause.getMessage(), cause);
			// session.write(FrameUtil.getEOT());
			// FrameUtil.waitAfterSendFrame();
		} catch (Exception e) {
			log.error("Handler Exception process error" + e.getMessage(), e);
		}
		finally {
	...
	private void receivedControlDataFrame(IoSession session, ControlDataFrame cdf) throws Exception {
		...
		if (code == ControlDataConstants.CODE_ENQ) {
			...
			// NEG 버전이 1.2 인 경우 frame size와 window size를 session에 저장한다.
			...
			int sequence = FrameUtil.getAckSequence(cdf);
			log.debug(" Received ACK Seq =" + sequence);
			 ...
			 * Partial Frame인경우
			 */
			if(session.containsAttribute(CommonConstants.SessionKey.PARTIAL_KEY)){
				log.debug("Received Partial Frame ACK. ");
				FMPProtocolPartialKit partialKit = (FMPProtocolPartialKit) session.getAttribute(CommonConstants.SessionKey.PARTIAL_KEY);
				partialKit.excuteAck(sequence);
			}
		} else if (code == ControlDataConstants.CODE_NAK) {
			int seqs[] = FrameUtil.getNakSequence(cdf);
			for(int seq : seqs){
				log.debug("Received NAK. Seq = " + seq);				
			}

			
//			ArrayList framelist = (ArrayList) session.getAttribute("sendframes");
...
//			}
//			ControlDataFrame wck = (ControlDataFrame) session.getAttribute("wck");
//			if (wck != null) {
//				session.write(wck);
//			}
			 ...
			 * 기존방식. SingleFrame인경우
			 */
			if(session.containsAttribute("sendframes")){
			 ...
			 * PartialFrame인경우
			 */
			else if(session.containsAttribute(CommonConstants.SessionKey.PARTIAL_KEY)){
				log.debug("Received Partial Frame NAK. ");
				FMPProtocolPartialKit partialKit = (FMPProtocolPartialKit) session.getAttribute(CommonConstants.SessionKey.PARTIAL_KEY);
				partialKit.excuteNak(seqs[0]);
			}
			...
			// future.awaitUninterruptibly();  ==> DEAD Lock 발생. 
			session.closeNow();
			...
			if (sw == null) {
				session.write(FrameUtil.getACK(DataUtil.getByteToInt(endseq)));
				return;
			}
				...
				if (sw.isReceivedEndFrame()) {
					try {
						...
						if (data[7] == 'A') {
							...
							if (kafkaEnable) {
								sendCommLog(session, data.length, cdf.getLength().getValue(), ProcessorHandler.SERVICE_DATAFILEDATA, filename);
							} else {
						...
						log.error("Save MultiFrame data error - " + ex.getMessage(), ex);
					}
				}
	...
	private void receivedServiceDataFrame(IoSession session, ServiceDataFrame sdf) throws Exception {
		String nameSpace = (String) session.getAttribute("nameSpace");
       	log.debug(getProtoName() + " nameSpace :" + nameSpace);
		ServiceData sd = null;
		
    	InetSocketAddress isa = (InetSocketAddress)session.getRemoteAddress();
    	String ipaddr = isa.getAddress().getHostAddress();
		...
		if (sd != null) {
			...
			log.debug(getProtoName() + " ServiceData :" + sd.getType());
			...
			if (sd.getType().equals("ServiceData.CommandData")) {
				...
				if (responseServiceData != null) {
					...
					boolean reqCompressMode = ((GeneralDataFrame)sdf).isAttrByte(GeneralDataConstants.ATTR_COMPRESS);
					writeData(session, responseSdf, reqCompressMode);               // 여기 순서 바꿀것
				}
	...
	private void writeData(IoSession session, Object message, boolean reqCompressMode) {
		boolean isUseCompress = Boolean.parseBoolean(FMPProperty.getProperty("protocol.compress.use", "true"));
		log.debug("Compress mode used? = " + isUseCompress + ", Req Compress mode? = " + reqCompressMode);
		...
		try {
			if (message instanceof ServiceDataFrame) {
				...
				if (isUseCompress && reqCompressMode) {
					...
					log.debug("set UnCompress mode attr Hex=" + Hex.decode(new byte[] { frame.getAttr() }));
				}

				byte[] bx = frame.encodeWithCompress(); //zlib

				ArrayList<byte[]> framelist = null;
				...
				if(1 < framelist.size()){
					...
					FMPProtocolPartialKit partialKit = new FMPProtocolPartialKit(session, framelist);
					session.setAttribute(CommonConstants.SessionKey.PARTIAL_KEY, partialKit);
					partialKit.executeStart();
				}else{
					log.debug("===> [Single] FrameList size=" + framelist.size());
					session.setAttribute("sendframes", framelist);
					byte[] data = framelist.get(0);
					int seq = DataUtil.getIntToByte(data[1]);
					log.info("Sended : seq=[" + seq + "], target=[" + session.getRemoteAddress() + "], length=[" + data.length + "], Hex=[" + Hex.decode(data) + "]");

					session.write(data);
				}
			} // message instanceof ServiceDataFrame end
			else { // 기타 프레임은 encoder로 바로 전달
	...
	private Class<?> getActionClass(String clsName) throws Exception {
		try {
			...
			if (cl == null)
				cl = getClass().getClassLoader();//fallback
	...
	/**
	 * 요청으로 받은 커맨드 데이타의 OID에 따라 실행할 액션 코드 파일을 찾아서 execute 해주고 그 결과를 ServiceData
	 * 형태로 돌려 받는다.
	 * 
	 * @param session
	 * @param receiveSd
	...
	private ServiceData cmdExecute(IoSession session, ServiceData receiveSd) throws Exception {
		...
		if (cls != null) {
			...
			Method method = cls.getDeclaredMethod("execute", new Class[] { receiveSd.getClass() });
	...
	public void messageReceived(IoSession session, Object message) {
		try {
			...
			if (message instanceof PLCDataFrame) {
				...
				if (frame instanceof ServiceDataFrame) {
					if (kafkaEnable && frame.getSvc() != 'C') {
						...
						session.setAttributeIfAbsent(CommonConstants.SessionKey.SESSION_SUSTAINABLE, true);  // SP-901 
						log.debug("SET SessionKey => SESSION_SUSTAINABLE : true"); 
						receivedServiceDataFrame(session, (ServiceDataFrame) frame);
					}
				} else if (frame instanceof ControlDataFrame) {
					receivedControlDataFrame(session, (ControlDataFrame) frame);
				}
			} else {
			...
			log.error(getProtoName() + " FMPProtocolHandler::messageReceived " + " failed", ex);
			// INSERT START SP-193
			CheckThreshold.updateCount(session.getRemoteAddress().toString(), ThresholdName.INVALID_PACKET);
			// INSERT END SP-193
		}
	...
	public void messageSent(IoSession session, Object message) throws Exception {
		...
		try {
			...
			log.error(getProtoName() + " FMPProtocolHandler::MessageSent " + " failed", ex);
		}
	...
	public void sessionIdle(IoSession session, IdleStatus status) throws Exception {
		log.debug(getProtoName() + " IDLE COUNT : " + session.getIdleCount(IdleStatus.READER_IDLE));
		session.closeNow();
		/*
	...
	public void sessionOpened(IoSession session) {
		...
		log.info("sessionOpened : " + session.getRemoteAddress() + ", writeIdleTime : " + DateTimeUtil.getDateString(session.getLastWriterIdleTime()));
	 ...
	 *
	 * @param ack
	 *            <code>ControlDataFrame</code> ack
	 */
	 ...
	 *            <code>ProtocolSession</code> session
	 * @param sequence
	 *            <code>int</code> wait ack sequence
	 */
	public void waitAck(IoSession session, int sequence) throws Exception {
		int waitAckCnt = 0;
		while (session.isConnected()) {
			if (ack == null) {
				...
				if ((waitAckCnt / 2) > ackTimeout) {
					throw new FMPACKTimeoutException("ACK Wait Timeout[" + ackTimeout + "]");
				}
			} else {
				int ackseq = FrameUtil.getAckSequence(ack);
				log.debug("ㄷㄷㄷ => " + ackseq);
				if (sequence == ackseq) {
					setAck(null);
					break;
				} else {
	...
	private String saveSlideWindow(byte[] bx) {
        ...
        try {
        	log.debug("Compress Header + Header:" + Hex.decode(DataUtil.select(bx, 0, 5)) + "+" + Hex.decode(DataUtil.select(bx, 5, 8)));
        	...
        	/*
        	 * Raw GeneralFrame CRC Check.
        	 */
        	int rawFrameLength = bx.length - 5;
        	byte[] rawData = new byte[rawFrameLength]; 
        	System.arraycopy(bx, 5, rawData, 0, rawData.length);
        	boolean isCrcPass = FrameUtil.checkCRC((byte[])rawData);
        	log.debug("Raw GeneralFrame CRC Check(except Compress header) => " + isCrcPass);
        	...
        	if(isCrcPass){
        		...
        		log.error("### [For DEBUG] CRC Error Raw Data ==> [" + Hex.decode(bx) + "]");
        		throw new Exception("CRC Error");
        	}
        } catch (Exception ex) {
            log.error("SliedeWindow file save error - " + ex.getMessage(), ex);
        } finally {
			if (fos != null) {
				try {
					fos.close();
			...
			try {
				if (filename != null && !Boolean.parseBoolean(FMPProperty.getProperty("kafka.enable"))){
                	...
                	log.debug(">>>> real file is created? = " + file.exists());
                	
					putServiceData(serviceType, filename);
				}else{
					log.debug("file name["+ filename +"] is null ~!!");
				}
			} catch (Exception e) {
				 log.warn("PutServerData error - " + e.getMessage(), e);
			}
		}
		return filename;
	}
	 ...
	 * 
	 * @param protocolType
	 *            <code>Integer</code> Protocol Type
	 */
	...
	private void sendCommLog(IoSession session, ServiceDataFrame sdf) throws Exception {
		...
		if (sdf.getSvc() == GeneralDataConstants.SVC_M || sdf.getSvc() == GeneralDataConstants.SVC_S || sdf.getSvc() == GeneralDataConstants.SVC_G) {
			serviceType = ProcessorHandler.SERVICE_MEASUREMENTDATA;
			/*
			 *	SP-1078 불필요 로직 삭제.(주석처리)
			 */
...
//			}
	...
	private void sendCommLog(IoSession session, int rcvBytes, int sendBytes, String serviceType, String filename) throws Exception {
		String nameSpace = (String) session.getAttribute("nameSpace");
		String ipaddr = session.getRemoteAddress().toString();
		ipaddr = ipaddr.substring(ipaddr.indexOf("/") + 1, ipaddr.lastIndexOf(":"));
