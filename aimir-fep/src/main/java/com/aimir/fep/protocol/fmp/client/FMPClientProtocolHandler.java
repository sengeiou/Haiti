...
public class FMPClientProtocolHandler extends IoHandlerAdapter
{
    ...
    private int negRTimeout = Integer.parseInt(FMPProperty.getProperty("protocol.negr.timeout", "3"));
...
//            } 
        	 ...
        	 */
            byte[] args = cdf.getArg().getValue();
            String protoVersion = DataUtil.getIntToByte(args[0]) + "." + DataUtil.getIntToByte(args[1]);
            int frameMaxLen = DataUtil.getIntTo2Byte(new byte[] {args[3], args[2]});
            int frameWinSize = DataUtil.getIntToByte(args[4]);
            
            log.info("Received ENQ[" + Hex.decode(args) + "] Version=" + protoVersion + ", WindowSize=" + frameMaxLen + ", WindowsCount= " + frameWinSize);
            
            session.setAttribute("frameMaxLen", frameMaxLen);
            session.setAttribute("frameWinSize", frameWinSize);
            session.setAttribute(IF4_VER, protoVersion);
            ...
            byte[] args = cdf.getArg().getValue();
            log.debug("NEG_R Received ==> " + DataUtil.getIntToByte(args[0]));
            
            if(args[0] == ControlDataConstants.NEG_R_NOERROR){
                log.debug("NEG_R_NOERROR");
                
                synchronized(negRMonitor) 
                { 
                    isReceivedNEGR = true; 
                    negRMonitor.notify(); 
                } 
            	...
            	log.debug("NEG_R_UNSUPPORTED_VERSION");  // IF4ERR_INVALID_VERSION
                session.closeNow();
            	...
            	log.debug("NEG_R_INVALID_SIZE");  // IF4ERR_INVALID_WINDOW_SIZE
                session.closeNow();
            }else if(args[0] == ControlDataConstants.NEG_R_INVALID_COUNT){
            	log.debug("NEG_R_INVALID_COUNT");  // IF4ERR_INVALID_WINDOW_COUNT
                session.closeNow();
            }else{
    ...
    public void sessionClosed(IoSession session)
    {
        ...
        synchronized(negRMonitor) { negRMonitor.notify(); }
    ...
    public void waitIF4Nego(IoSession session) throws Exception{
        log.debug("Wait IF4 Negotiation.");
        
    	// 1. Wait ENQ Frame
    	waitENQ();
    	
    	// IF4 Protocol version check
        String protoVersion = null;
        if(session.containsAttribute(IF4_VER)){
        	protoVersion = (String)session.getAttribute(IF4_VER);
        }

        String nameSpace = null;
        if(session.containsAttribute("nameSpace")){
            nameSpace = (String)session.getAttribute("nameSpace");            	
        }
        
        log.debug("IF4 Version = " + protoVersion + ", NameSpace = " + nameSpace);

        
        /*
         * 2. Send NEG Frame 
         *  - 1.2 이상만 해당
         *  - ENQ로 받은 Protocol Version에 맞춰서 전송한다.        	
         */
        if(protoVersion != null && !protoVersion.equals("1.1")){
            if(nameSpace == null || nameSpace.equals("")){
            	log.error("Can not found namespace. please check namespace.");
            	throw new Exception(ErrorCode.getMessage(ErrorCode.IF4ERR_UNKNOWN_NAMESPACE));
            }
            
            session.write(FrameUtil.getNEG(nameSpace));
    ...
    public void waitNEGR() throws FMPException
    {
        ...
        int waitNegRCnt = 0;
        while(!isReceivedNEGR)
        {
            synchronized(negRMonitor)
            {
                try {negRMonitor.wait(500); }
                catch(Exception ex) {}
            }
            waitNegRCnt++;
            if((waitNegRCnt / 2) > negRTimeout)
            { 
                throw new FMPNEGRTimeoutException("NEG-R Wait Timeout[" + negRTimeout +"]");
            }
        }
