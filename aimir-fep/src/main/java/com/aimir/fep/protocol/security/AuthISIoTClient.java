...
//			"http://127.0.0.1/oac/api/");
    ...
    public StatusCode cinfoRequest(byte[] initToken ) {
    	...
    	if ( status != 0) {
    		 ...
    		 if(this.deviceSerial != null && this.deviceSerial.length > 0) {
    			 logger.error("Get PinCode Fail["+Hex.decode(this.deviceSerial)+"]");
    		 }else {
    	...
    	// dummy[i] = (byte) i ;
     	...
     	if ( status != IOT_SUCCESS ){
     		// error
     		if(this.deviceSerial != null && this.deviceSerial.length > 0) {
				logger.error("GET SignedDataPassword Fail["+Hex.decode(this.deviceSerial)+"]");
     		}else {
    ...
    public StatusCode certificateRequest(byte[] clientToken) throws Exception
    {
    	...
    	if ( isIotFunctionCalled(IoT_Function.IS_IoT_Make_Stoken) == false &&
     		...
     		if(this.deviceSerial != null && this.deviceSerial.length > 0) {
				logger.error("IS_Iot_Make_Stoken() was not called["+Hex.decode(this.deviceSerial)+"]");
     		}else {
    	...
    	if ( status != IOT_SUCCESS ){
    		// error 
     		...
     		if(this.deviceSerial != null && this.deviceSerial.length > 0) {
				logger.error("GET CRL Fail["+Hex.decode(this.deviceSerial)+"]");
     		}else {
    	...
    	if ( status != IOT_SUCCESS ){
    		// error
     		if(this.deviceSerial != null && this.deviceSerial.length > 0) {
				logger.error("GET Passwrod Fail["+Hex.decode(this.deviceSerial)+"]");
     		}else {
     		...
     		if ( CRLLen > 0 && cert_buffer_len > 0 ){
    	    	byte[] certBuffer = copyByteArray(cert_buffer, cert_buffer_len);
	     		try {
		     		...
		     		try{
                    	cert.checkValidity();
                    	logger.info("The certificate is Valid ");
                    }
                    catch (CertificateExpiredException cee) {
                		retVal = -361;
                       	
                 		if(this.deviceSerial != null && this.deviceSerial.length > 0) {
            				logger.error("The certificate is expired["+Hex.decode(this.deviceSerial)+"]");
                 		}else {
		     		...
		     		{
                 		if(this.deviceSerial != null && this.deviceSerial.length > 0) {
            				logger.error("CRL certificate is revoked["+Hex.decode(this.deviceSerial)+"]");
                 		}else {
    ...
    public HashMap <String, String>  getMeterSharedKey(String serialNo){
    	...
    	try {
    		...
    		if ( json == null || json.get("resul_tcode") == null ){
    			logger.error("get_sharedkey_and_pin_argument fail.["+serialNo+"]");
    		}
    		else {
	    		int result  = json.get("result_code").getAsInt();
	    		if ( result  != 0  ){
