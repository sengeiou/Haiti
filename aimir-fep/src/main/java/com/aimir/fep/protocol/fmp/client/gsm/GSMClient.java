...
{
    private Log log = LogFactory.getLog(GSMClient.class);
     ...
     * @throws GeneralSecurityException 
     */
    public GSMClient(GSMTarget target) throws GeneralSecurityException, IOException
    {
        this.target = target;
    }
	...
	private synchronized void connect() throws Exception
    {
        ...
        try
        {
            ...
            log.debug("GSMClient waitENQ End");
        }catch(Exception ex)
        {
            close();
            throw ex;
        ...
        throws Exception
    {
		...
		try {
			...
			FMPClientProtocolHandler handler = (FMPClientProtocolHandler) session
					.getHandler();
			ServiceData sd;
