...
public class CommandNIProxy implements CommandNIProxyMBean, MBeanRegistration {
	...
	public static synchronized NiClient getClient(Target target, GeneralFrame generalFrame) throws Exception {
		...
		if (target.getReceiverType().equals("MMIU") || target.getTargetType() == McuType.MMIU) {
			if (target.getProtocol() == Protocol.SMS || target.getProtocol() == Protocol.GPRS) { // MBB Modem / TCP
				/**
	...
	public static GeneralFrame setGeneralFrameOption(FrameOption_Type fOptionType, NIAttributeId attrId) {
 	...
 	// INSERT START SP-575
	public static GeneralFrame setGeneralFrameOption(String requestType) {
		GeneralFrame generalFrame = new GeneralFrame();
		generalFrame.setFrameOption_Type(FrameOption_Type.Command);
		...
		logger.debug("## FrameOption Type = {}", FrameOption_Type.Command);
	...
	public Object execute(Target target, NIAttributeId command, HashMap<String, Object> param, IoSession externalNISession) throws Exception {
		try {
 	...
 	// INSERT START SP-575
	public Object execute(Modem modem, String attrId, String param, String requestType) throws Exception {
		Target target = CmdUtil.getNullBypassTarget(modem);
		if (target == null) {
			throw new Exception("Can not found target. please check Meter & Modem information.");
		}
		return execute(target, attrId, param, requestType);
	}

	public Object execute(Target target, String attrId, String param, String requestType) throws Exception {
		try {
			GeneralFrame generalFrame = setGeneralFrameOption(requestType);
			client = getClient(target, generalFrame);

			byte[] sendData = makeGeneralDataFrame(generalFrame, attrId, param);
			generalFrame = client.sendCommand(target, generalFrame, sendData);
	...
	public static byte[] makeGeneralDataFrame(String netType,GeneralFrame gf, String attrId, String param) {
		if (netType.equals(NetworkType.MBB.name())) {
			gf.setNetworkType(NetworkType.MBB);
		} else if (netType.equals(NetworkType.Sub1Ghz_SORIA.name())) {
			gf.setNetworkType(NetworkType.Sub1Ghz_SORIA);
		} else if (netType.equals(NetworkType.Ethernet.name())) {
			gf.setNetworkType(NetworkType.Ethernet);
		} else {
			gf.setNetworkType(NetworkType.MBB);
		}
		
		return makeGeneralDataFrame(gf, attrId, param);
	}
	private static byte[] makeGeneralDataFrame(GeneralFrame command, String attrId, String param) {
        try {
        	byte[] hexid = Hex.encode(attrId);
            byte[] cmdArray1 = null;
            Command command2 = new Command();
            Command.Attribute attr = command2.newAttribute();
            Command.Attribute.Data[] datas = attr.newData(1);
            command2.setCommandFlow(CommandFlow.Request);
            switch (command._commandType) {
            case  Get:
            case  Trap:
                command2.setCommandType(CommandType.Get);
                break;
            case  Set:
                command2.setCommandType(CommandType.Set);
                break;
            }
            datas[0].setId(hexid);
            if (param.length() > 0){
    			byte[] hexparam = Hex.encode(param);
    			datas[0].setValue(hexparam);
            }
            attr.setData(datas);
            command2.setAttribute(attr);
            
            command.payload = command2;
            ...
            logger.debug("[NICL][GeneraDatalFrame|cmdArray1]"+Hex.decode(cmdArray1));
        	//GeneralFrame + Payload
        	byte[] cmdArray2 = command.encode(cmdArray1);
        	
        	logger.debug("[NICL][GeneraDatalFrame|cmdArray2]"+Hex.decode(cmdArray2));

        	return cmdArray2;
        } catch (Exception e) {
        	logger.error("[NICL][GeneraDatalFrame]", e);
			logger.error("CommandNIProxy Execute Error - {}", e);
        }
		 return null;
	}
