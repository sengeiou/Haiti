...
public class GatheringDataPoll extends AbstractCommand{
	...
	// SP-575 add start
    public DLMSMeter decode2(byte[] bx) {
        int pos = 0;
       ...
       // DLMSMeter dlmsMeter = new DLMSMeter();
        dlmsMeter = new DLMSMeter();
        dlmsMeter.obis = sb.toString();
        pos += b.length;
        ...
        b = new byte[2];
        System.arraycopy(bx, pos, b, 0, b.length);
        dlmsMeter.length= DataUtil.getIntTo2Byte(b);
        pos += b.length;
