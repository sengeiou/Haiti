...
public class SlideWindow
{
    ...
    public byte[] checkWindow(int start,int end)
    {
        ...
        log.info("FRAME_WINSIZE[" + FRAME_WINSIZE + "] startseq[" + startseq + "] endseq[" + endseq + "]");
...
//          al.add(new String(""+(i+startseq)));
        	...
        	// for debuging
        	boolean isCrcPass = false;
        	if(window[i] != null){
            	byte[] windowContent = (byte[]) window[i];
            	isCrcPass = FrameUtil.checkCRC((byte[])window[i]);
            	log.debug("i = " + i + ", CRC Pass = " + isCrcPass + ", length = " + windowContent.length + ", HEX = " + Hex.decode(windowContent));        		        		
        	}else{
        		log.debug("i = " + i + ", windows is null");
        	}

        	
		  	if(window[i] == null || !isCrcPass){
		  		al.add(new String(""+(i+startseq)));
		  		log.debug(" ==> " + (i+startseq));
		  	}
        } 
        ...
        {
            naklist[i] = DataUtil.getByteToInt(Integer.parseInt(strarr[i]));
