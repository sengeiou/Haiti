...
import com.aimir.fep.meter.parser.a3rlnqTable.ST55;
 ...
 */
public class A3RLNQ_Handler extends MeterProtocolHandler {

	private static Log log = LogFactory.getLog(A3RLNQ_Handler.class);
	...
	public CommandData execute(MRPClientProtocolHandler handler,
		...
		log.debug("==============A3RLNQ_Handler start cmd:" + cmd + "================");
		...
		if(cmd.equals("105.2.0") || cmd.equals("105.5.0")){
			...
			if (smiValue != null && smiValue.length >= 2) {
				...
				case A1830RLN_DataConstants.CMD_ALL:// all
					...
					ba.append(DataUtil.getReverseBytes(DataUtil.get2ByteToInt(mti.length)));
					ba.append(mti);
					
					//PB
					ba.append("PB".getBytes());
					ba.append(DataUtil.getReverseBytes(DataUtil.get2ByteToInt(tpb.length)));
					ba.append(tpb);
					
					//CB
					ba.append("CB".getBytes());
					ba.append(DataUtil.getReverseBytes(DataUtil.get2ByteToInt(tcb.length)));
					ba.append(tcb);
					
					//LD
					ba.append("LD".getBytes());
					ba.append(DataUtil.getReverseBytes(DataUtil.get2ByteToInt(lpd.length)));
					ba.append(lpd);
					
					//EL
					ba.append("EL".getBytes());
					ba.append(DataUtil.getReverseBytes(DataUtil.get2ByteToInt(eld.length)));
					ba.append(eld);
					
					//IS
					ba.append("IS".getBytes());
					ba.append(DataUtil.getReverseBytes(DataUtil.get2ByteToInt(ist.length)));
					ba.append(ist);
					
					//PQ
					ba.append("PQ".getBytes());
					ba.append(DataUtil.getReverseBytes(DataUtil.get2ByteToInt(pqm.length)));
					ba.append(pqm);
			...
			me.setMlpType(new BYTE(CommonConstants.ModemType.Converter_Ethernet
	...
	protected byte[] identRequest(IoSession session) throws MRPException {
		...
		try {
			...
			A3RLNQ_RequestDataFrame frame = new A3RLNQ_RequestDataFrame(
    ...
    throws Exception 
    {
        ...
        A3RLNQ_RequestDataFrame frame 
            = new A3RLNQ_RequestDataFrame(this.before_txd_control,kind,ANSI.WRITE,new OCTET(msg),4,26,true,this.meter_length);
...
//        session.write(buf);
	 ...
	 * @return
	 */
	protected byte[] read(IoSession session, A3RLNQ_RequestDataFrame frame,
	...
	public byte[] read(IoSession session, char tableid, int index, int count)
			throws MRPException {
		try {
			IoBuffer buf = A3RLNQ_RequestDataFrame.getRead(tableid, index,
					count);
