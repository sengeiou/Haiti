...
public class NICommandActionHandlerAdaptor {
	...
	public BypassCommandAction setBypassCommandAction(IoSession session, BypassCommandAction bypassCommandAction) throws Exception {
		if (bypassCommandAction == null) {
			throw new Exception("Unknown BypassCommandAction");
		}

		((IoSession) session).setAttribute(BYPASS_COMMAND_ACTION_KEY, bypassCommandAction);

		return (BypassCommandAction) session.getAttribute(BYPASS_COMMAND_ACTION_KEY);
	}
	 ...
	 * @return
	 */
	public BypassCommandAction getBypassCommandAction(IoSession session) {
		if (session.getAttribute("BYPASS_COMMAND_ACTION_KEY") != null) {
			((BypassCommandAction) session.getAttribute("BYPASS_COMMAND_ACTION_KEY")).printMultiSessionRemoteAddress();
		}
