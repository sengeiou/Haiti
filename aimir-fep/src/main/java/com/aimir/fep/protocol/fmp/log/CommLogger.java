...
public class CommLogger extends MessageLogger
{
    ...
    public void sendLog(final Serializable data) {
        ...
        try {
        	...
        	// OPF-1061
        	String supplierId = (commLog.getSuppliedId() != null) ? commLog.getSuppliedId() : supplierDao.getAll().get(0).getId().toString();
...
//            }
            ...
            if (protocolType != null)	commLog.setProtocolCode(CommonConstants.getProtocolByName(protocolType.name()));
            else						commLog.setProtocolCode(CommonConstants.getProtocolByName(Protocol.GPRS.name()));
            
            // Check commLog Object. If ReceiverTypeCode is null, check what receiver is and set proper code.
            if(commLog.getReceiverTypeCode() == null && commLog.getReceiverId() != null) { // OPF-1061
            	Code code = null;
            	if(mcuDao.get(commLog.getReceiverId()) != null) 
            		code = codeDao.getCodeIdByCodeObject("4.10.2"); // DCU
            	else {
            		Modem mo = modemDao.get(commLog.getReceiverId());
            		if(mo != null) {
                		if(mo.getModemType().equals(ModemType.MMIU))
                			code = codeDao.getCodeIdByCodeObject("4.10.15"); // MMIU
                		else
                			code = codeDao.getCodeIdByCodeObject("4.10.3"); // MODEM
            		} else if (meterDao.get(commLog.getReceiverId()) != null){
                		code = codeDao.getCodeIdByCodeObject("4.10.4"); // EnergyMeter
            		}
            	}
            	commLog.setReceiverTypeCode(code);
            	commLog.setReceiverTypeCodeId(code.getId());
            	log.debug("Set Receiver Type Code : " + code.getName()+"["+code.getCode()+"]");
            }
