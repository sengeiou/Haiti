...
public class GeneralFrame {
    ...
    public void setCoapResult(String resultData){
    ...
    //	         value = Integer.parseInt(DataUtil.getBit(b[0])+DataUtil.getBit(b[1])+DataUtil.getBit(b[2])+DataUtil.getBit(b[3]));
    		...
    		case "/6/n_m_s":
    	        ...
    	        parentNodeId = sb.toString();
    	        pos+=b.length;
    	        
    	        b = new byte[1];
    	        System.arraycopy(data, pos, b, 0, b.length);
    	        if(DataUtil.getIntToByte(b[0]) > 127)
    	        	rssi = (256 - DataUtil.getIntToByte(b[0])) * -1 ;
    	        else
    	        	rssi = DataUtil.getIntToByte(b[0]);
    	        pos+=b.length;
    	        ...
    	        b = new byte[2];
    	        System.arraycopy(data, pos, b, 0, b.length);
    	        if(DataUtil.getIntTo2Byte(b) >=100)
