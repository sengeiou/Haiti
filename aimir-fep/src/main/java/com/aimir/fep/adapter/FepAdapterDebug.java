...
//        registerNiPanaAdapter();
    ...
    /*
     * NiUdpDtlsAdapter and UdpDtlsAdpater are for NI protocol.
     * NiUdpDtlsAdapter is based on wolfssl.
     * other is based on scandium dtls
     * scandium dtls is more stable.
     */
    protected void registerNiUdpDtlsAdapter()
    throws Exception
    {
        ObjectName adapterName = null;
        NiUdpDtlsAdapter adapter = new NiUdpDtlsAdapter();
        ...
        adapter.setPort(Integer.parseInt(niUdpPort));
        adapterName = new ObjectName(
                ADAPTER_DOMAIN+":name=NiUdpDtlsAdapter, port="+niUdpPort);
        registerMBean(adapter, adapterName);
...
//        }
        ...
        FepAdapterDebug fep = new FepAdapterDebug();
		...
		logger.info("######## FepAdapterDebug Elapse Time : " + DateTimeUtil.getElapseTimeToString(endTime - startTime));
    }
}
