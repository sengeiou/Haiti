 ...
 *
 */
public class DlmsConstantsForIESCO {
	private static Logger logger = LoggerFactory.getLogger(DlmsConstantsForIESCO.class);

	public enum DlmsPiece {
		  //CLIENT_SYSTEM_TITLE("4C534B0000000001")   // Client System Title : LSK00001  // Iraq-MOE
		  //CLIENT_SYSTEM_TITLE("4E54430000000001")   // Client System Title : NTC00001	 // SORIA
		  CLIENT_SYSTEM_TITLE("0000000000000000")   // Client System Title : 00000000	 // IESCO - CLOU
		, C_TO_S("") // CtoS(challenge) : Random 8byte
		, DEDICATED_KEY("00000000000000000000000000000000");  // IESCO - CLOU
		...
		public byte[] getBytes(int length) {
			if(this == C_TO_S) {
				return DataUtil.readByteString(getValue(length));
			}
			return DataUtil.readByteString(getValue());
		}
		...
		public String getValue() {
			...
			logger.info("[DlmsPiece] {} = {}", this.name(), value);
			return value;
		}
		
		public String getValue(int length) {
			if(this == C_TO_S) {
				String value = Hex.decode(getSecureRandomByte(length));
	...
	public enum AARQ {
		...
		, APPLICATION_CONTEXT_NAME(1, "A109060760857405080103") // encoding of the value of the Object Identifier: LN referencing with ciphering (3) : Pakistan IESCO 사용
		, CALLED_AP_TITLE(2, "")
		, CALLED_AE_QUALIFIER(3, "")
		, CALLED_AP_INVOCATION_ID(4, "")
		, CALLED_AE_INVOCATION_ID(5, "")
		, CALLING_AP_TITLE(6, "A60A0408" + DlmsPiece.CLIENT_SYSTEM_TITLE.value) //  -- Client System Title  : 00000000
		...
		//, CALLING_AUTHENTICATION_VALUE(12, "AC0A8008" + DlmsPiece.C_TO_S.value) //   -- CtoS(challenge)
		, CALLING_AUTHENTICATION_VALUE(12, "") //   -- CtoS(challenge)
		, IMPLEMENTATION_INFORMATION(29, "")
		//, USER_INFORMATION(30, "BE10040E01000000065F1F0400007E1F00AC")
		//, USER_INFORMATION(30, "BE10040E01000000065F1F0400007E1F0000")  // IRAQ
		//, USER_INFORMATION(30, "BE10040E01000000065F1F040000181D0080")  // SORIA
		//, USER_INFORMATION(30, "BE10040E01000000065F1F0400007E1F0000") // SriLanka - CEB
		, USER_INFORMATION_TAG(30, "BE")  // IRAN, Pakistan - CLOU미터			
		, APPLICATION(97, "60")
		, AARQ_LLC(98, "E6E600");
		...
		public byte[] getValue() {
			if(this == CALLING_AUTHENTICATION_VALUE) {
				return getValue(DlmsPiece.C_TO_S.getBytes(8));
			}
			
			return Hex.encode(value);
		}
		
		public byte[] getValue(byte[] cToS) {
			if(this == CALLING_AUTHENTICATION_VALUE) {
				String str = "AC"   // AARQ-APDU [12]Authentication Value
				+ Hex.decode(new byte[] {DataUtil.getByteToInt(cToS.length + 2)}) // Authentication Value 의 값필드 길이
				+ "80"   // Authentication functional unit(0) : GraphicString
				+ Hex.decode(new byte[] {DataUtil.getByteToInt(cToS.length)})  // charstring 길이
				+ Hex.decode(cToS); // CtoS(challenge)
	...
	public enum AARE {
		...
		, RESPONDING_AE_QUALIFIER(0x05)
		, RESPONDING_AP_INVOCATION_ID(0x06)
		, RESPONDING_AE_INVOCATION_ID(0x07)
		, RESPONDER_ACSE_REQUIREMENTS(0x88)
		, MECHANISM_NAME(0x89)
		, RESPONDING_AUTHENTICATION_VALUE(0xAA)
		, IMPLEMENTATION_INFORMATION(0x1D)
	 ...
	 */
	public enum XDLMS_APDU_InitiateRequest {
		  DEDICATED_KEY(DlmsPiece.DEDICATED_KEY.value)
		, RESPONE_ALLOWED("00")
		, PROPOSED_QUALITY_OF_SERVICE("00")
		, PROPOSED_DLMS_VERSION_NUMBER("06")
		, PROPOSED_CONFORMANCE("5F1F0400007E1F")	  // Conformance ::= [APPLICATION 31] IMPLICIT BIT STRING 
		//, CLIENT_MAX_RECEIVE_PDU_SIZE("0000");
		, CLIENT_MAX_RECEIVE_PDU_SIZE("00AC"); // Paskistan - IESCO
		
		private String code;

		private XDLMS_APDU_InitiateRequest(String code) {
			this.code = code;
		}
		...
		public static XDLMS_APDU_InitiateRequest getItem(String code) {
			for (XDLMS_APDU_InitiateRequest fc : XDLMS_APDU_InitiateRequest.values()) {
	...
	public enum ActionRequest {
		...
		//, ACTION_REQUEST_INVOKE_ID_AND_PRIORITY("81")  // For Iran
		, ACTION_REQUEST_INVOKE_ID_AND_PRIORITY("C1") // Pakistan - IESCO
		, NORMAL("01"), NEXT_PBLOCK("02"), WITH_LIST("03"), WITH_FIRST_PBLOCK("04"), WITH_LIST_AND_FIRST_PBLOCK("05"), WITH_PBLOCK("06");
	...
	public enum ActionResult {
		SUCCESS(0x00), HARDWARE_FALT(0x01), TEMPORARY_FAIL(0x02), READ_WRITE_DENIED(0x03), OBJECT_UNDEFINED(0x04)
		, OBJECT_CLASS_INCONSISTENT(0x09), OBJECT_UNAVAILABLE(0x0B), TYPE_UNMATCHED(0x0C), SCOPE_OF_ACCESS_VIOLATED(0x0D)
		, DATA_BLOCK_UNAVAILABLE(0x0E), LONG_ACTION_ABORTED(0x0F), NO_LONG_ACTION_IN_PROGRESS(0x10), OTHER_REASON(0xFA);
	...
	public enum GetRequest {
		GET_REQUEST("C0"), GET_REQUEST_LLC("E6E600")
		, GET_REQUEST_INVOKE_ID_AND_PRIORITY("40")
		//, GET_REQUEST_INVOKE_ID_AND_PRIORITY("81")  // For Iran
		, NORMAL("01"), NEXT("02"), WITH_LIST("03");
	...
	/**
	 * Used Get Meter Key function for SORIA Kaifa Meter (maby back door)
	 */
	public enum KaifaCustomRequest {
		KAIFA_CUSTOM_LLC("E6E600"), GET_SORIA_METER_KEY_A("FF0755"), GET_SORIA_METER_KEY_B("FF020307"), GET_SORIA_METER_KEY_C("FF020306"),
		SET_SORIA_METER_SERIAL_A("FF0755"),
		SET_SORIA_METER_SERIAL_B("FF02020B"),
		SET_SORIA_METER_SERIAL_C("FF01020C"),
		SET_SORIA_METER_SERIAL_D("FF02020C"),
		SET_SORIA_METER_SERIAL_E("FF0710");

		private String code;

		private KaifaCustomRequest(String code) {
			this.code = code;
		}
		...
		public static KaifaCustomRequest getItem(String code) {
			for (KaifaCustomRequest fc : KaifaCustomRequest.values()) {
				if (fc.code.equals(code)) {
					return fc;
				}
			}
	...
	public enum DataAccessResult {
		SUCCESS(0x00), HARDWARE_FALT(0x01), TEMPORARY_FAIL(0x02), READ_WRITE_DENIED(0x03), OBJECT_UNDEFINED(0x04)
		, OBJECT_CLASS_INCONSISTENT(0x09), OBJECT_UNAVAILABLE(0x0B), TYPE_UNMATCHED(0x0C), SCOPE_OF_ACCESS_VIOLATED(0x0D)
		, DATA_BLOCK_UNAVAILABLE(0x0E), LONG_GET_ABORTED(0x0F), NO_LONG_GET_IN_PROGRESS(0x10), LONG_SET_ABORTED(0x11)
		, NO_LONG_SET_IN_PROGRESS(0x12), DATA_BLOCK_NUMBER_INVALID(0x13), OTHER_REASON(0xFA);
	...
	public enum SetRequest {
		  SET_REQUEST("C1"), SET_REQUEST_LLC("E6E600")
		, SET_REQUEST_INVOKE_ID_AND_PRIORITY("40")
		//, SET_REQUEST_INVOKE_ID_AND_PRIORITY("81")  // for iran
		, NORMAL("01")
		, WITH_FIRST_DATABLOCK("02"), WITH_DATABLOCK("03"), WITH_LIST("04"), WITH_LIST_AND_FIRST_DATABLOCK("05");
	...
	public enum ImageTransferAttributes {
		IMAGE_BLOCK_SIZE(0x02), IMAGE_TRANSFERRED_BLOCKS_STATUS(0x03), IMAGE_FIRST_NOT_TRANSFERRED_BLOCK_NUMBER(0x04)
		, IMAGE_TRANSFER_ENABLED(0x05), IMAGE_TRANSFER_STATUS(0x06), IMAGE_TO_ACTIVATE_INFO(0x07);
	...
	/*
	 * SORIA Meter F/W Attributes
	 * - class_id = 1 (0001) 
	 * - OBIS CODE = 1-1:0.2.0.255 (0100000200FF02)
	 *  
	 * @author simhanger
	 *
	 */
	public enum MeterFWInfoAttributes {
		...
		//, OBIS_CODE("0101000200FF")
		  , OBIS_CODE("0100000200FF")  // Pakistan - IESCO
		, FW_VERSION("02")
	...
	/*
	 * SORIA Meter RelayStatus Attributes
	 * - class_id = 70 (0046) 
	 * - OBIS CODE = 70-0:96.3.10.255 (000060030AFF)
	 *  
	 * @author simhanger
	 *
	 */
	public enum MeterRelayMethods {
		CLASS_ID("0046"), OBIS_CODE("000060030AFF"), REMOTE_DISCONNECT("01"), REMOTE_RECONNECT("02"), OPTION_USE("01"); // 00은 Option 사용 하지 않겠다는 뜻.

		private String code;

		private MeterRelayMethods(String code) {
			this.code = code;
		}
		...
		public static MeterRelayMethods getItem(String code) {
			for (MeterRelayMethods fc : MeterRelayMethods.values()) {
				if (fc.code.equals(code)) {
					return fc;
				}
			}
	...
	public enum DemandPeriodAttributes {
		CLASS_ID("0005"), OBIS_DEMAND_PLUS_A_PERIOD("0100010400FF"), OBIS_DEMAND_PLUS_A_NUMBER("0100010400FF")
		, OBIS_DEMAND_MINUS_A_PERIOD("0100020400FF"), OBIS_DEMAND_MINUS_A_NUMBER("0100020400FF")
		, OBIS_DEMAND_PLUS_R_PERIOD("0100030400FF"), OBIS_DEMAND_PLUS_R_NUMBER("0100030400FF")
		, OBIS_DEMAND_MINUS_R_PERIOD("0100040400FF"), OBIS_DEMAND_MINUS_R_NUMBER("0100040400FF")
		, OBIS_DEMAND_R_QI_PERIOD("0100050400FF"), OBIS_DEMAND_R_QI_NUMBER("0100050400FF")
		, OBIS_DEMAND_R_QIV_PERIOD("0100080400FF"), OBIS_DEMAND_R_QIV_NUMBER("0100080400FF")
		, OBIS_DEMAND_PLUS_PERIOD("0100090400FF"), OBIS_DEMAND_PLUS_NUMBER("0100090400FF")
		, OBIS_DEMAND_MINUS_PERIOD("01000A0400FF"), OBIS_DEMAND_MINUS_NUMBER("01000A0400FF")
		, PEROID("08"), NUMBER_OF_PERIODS("09"), OPTION_NOT_USE("00"); // 00은 Option 사용 하지 않겠다는 뜻.
	...
	/*
	 * SORIA M-Bus client Attributes (Channel 1)
	 * - class_id = 72 (0048) 
	 * - OBIS CODE = 0-1:24.1.0.255 (0001180100FF)
	 *  
	 * @author 
	 *
	 */
	public enum MBusClientMethods {
		CLASS_ID("0048"), OBIS_CODE("0001180100FF"), SLAVE_INSTALL("01"), SLAVE_DEINSTALL("02"), OPTION_USE("01");

		private String code;

		private MBusClientMethods(String code) {
			this.code = code;
		}
		...
		public static MBusClientMethods getItem(String code) {
			for (MBusClientMethods fc : MBusClientMethods.values()) {
				if (fc.code.equals(code)) {
					return fc;
				}
			}
	...
	public static Object getValueByDLMSCommonDataType(DLMSCommonDataType dataType, byte[] data) {
		...
		case CompactArray:
			...
			break;
		case INT8:
			value = new Integer(data[0]);
			break;
		case UINT8:
			value = new Integer(DataUtil.getIntToBytes(data));
			break;
		case INT16:
			value = new Integer(DataUtil.getShortTo2Byte(data));
			break;
		case UINT16:
			value = new Integer(DataUtil.getIntTo2Byte(data));
			break;	
		case INT32:
			value = new Long(DataUtil.getIntTo4Byte(data));
			break;
		case UINT32:
