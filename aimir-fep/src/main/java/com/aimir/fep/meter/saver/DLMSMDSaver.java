...
@Service
public class DLMSMDSaver extends AbstractMDSaver {
	private static Log log = LogFactory.getLog(DLMSMDSaver.class);

	@Override
	protected boolean save(IMeasurementData md) throws Exception {
		DLMS parser = (DLMS) md.getMeterDataParser();
		...
		double lpSum = 0;
		double basePulse = 0;
		double current = parser.getMeteringValue();
...
//			 Map<String, Object> lpMap = getLpMap(lplist, basePulse);
//			
//			 Set<String> keys = lpMap.keySet();
//			
//			 Iterator<String> lpMapIter = keys.iterator();
//			
//			 // log.debug("lpMap:" + lpMap);
//			 while (lpMapIter.hasNext()) {
//			 String yyyymmdd = (String) lpMapIter.next();
//			 if (!yyyymmdd.endsWith("_basePulse")) {
//			 Vector<LPData> lpVec = (Vector<LPData>) lpMap.get(yyyymmdd);
//			 LPData[] validlplist = new LPData[lpVec.size()];
//			 for (int i = 0; i < lpVec.size(); i++) {
//			 validlplist[i] = lpVec.get(i);
//			 }
//			 lpSave(md, validlplist, parser, (Double) lpMap.get(yyyymmdd
//			 + "_basePulse"));
//			 }
//			 }
	...
	private void saveEventLog(DLMS parser) {
	    try {
	...
	private void savePreBill(DLMS parser) {
	    Double value = 0.0;
	...
	private void saveCurrBill(DLMS parser) {
        Double value = 0.0;
	...
	private boolean lpSave(IMeasurementData md, LPData[] validlplist,
			DLMS parser, double base,double addBasePulse) throws Exception {
