...
public class ModbusEMnVLS extends MeterDataParser implements java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private static Logger log = LoggerFactory.getLogger(ModbusEMnVLS.class);
	...
	/**
	 * 옴니, 누리 둘다 현장에 ip5a 모델은 없고 현재 is7모델만 설치되어 있음.
	 */
	//LinkedHashMap<MODBUS_LS_IP5A_CODE, Map<String, Object>> result = new LinkedHashMap<MODBUS_LS_IP5A_CODE, Map<String, Object>>();
	LinkedHashMap<MODBUS_LS_IS7_CODE, Map<String, Object>> result = new LinkedHashMap<MODBUS_LS_IS7_CODE, Map<String, Object>>();
	...
	public void parse(byte[] data) throws Exception {
		...
		log.info("    ");
		log.info("    ");
		log.info("################ LS Inverter - IS7 로그확인 시작 ({}) #########################", parsingTime);
		log.info("ModbusEMnV-LS parse:[{}] [{}]", meteringDataType != null ? meteringDataType.name() : "", Hex.decode(data));
		...
		log.info("총 MODBUS_LS_CODE 갯수 ==> {}", dataCount);

		// 가짜 LP타임 저장
		parsingTime = new ModebusInverterCommonTable().resetLpTime(cal);
		//setMCodeResult(MODBUS_LS_IP5A_CODE.DATE, parsingTime);
		setMCodeResult(MODBUS_LS_IS7_CODE.DATE, parsingTime);
		...
		for (int i = 0; i < dataCount; i++) {
			...
			//MODBUS_LS_IP5A_CODE mCode = MODBUS_LS_IP5A_CODE.getItem(Hex.decode(address));
			MODBUS_LS_IS7_CODE mCode = MODBUS_LS_IS7_CODE.getItem(Hex.decode(address));
			...
			case TRIP_INFO:
				break;
			case INPUT_TERMINAL_INFO:
				value = Hex.decode(mData);
				break;
			case OUTPUT_TERMINAL_INFO:
				value = Hex.decode(mData);
				break;
			case V1:
				value = Hex.decode(mData);
			...
			log.info("[{}][PROTOCOL][METERING_DATA] MODBUS_LS_CODE[{}] ==> {}={}, HEX=[{}]", new Object[] { i, Hex.decode(address), mCode.name(), value, Hex.decode(mData) });
		}

		log.debug("### MODBUS_LS_CODE TOTAL ==> {}", result.toString());
		log.info("### Number_of_MODBUS={} Last POS={}", new Object[] { dataCount, pos });

		setInverterInfo();
	}

	//private void setMCodeResult(MODBUS_LS_IP5A_CODE mCode, Object value) {
	private void setMCodeResult(MODBUS_LS_IS7_CODE mCode, Object value) {
	...
	public void setLPData() {
		try {
			...
			// 출력 전류
			//mCodeMap = result.get(MODBUS_LS_IP5A_CODE.OUTPUT_CURRENT);
			mCodeMap = result.get(MODBUS_LS_IS7_CODE.OUTPUT_CURRENT);
			if (mCodeMap != null) {
			...
			// 출력 주파수
			//mCodeMap = result.get(MODBUS_LS_IP5A_CODE.OUTPUT_FREQUENCY);
			mCodeMap = result.get(MODBUS_LS_IS7_CODE.OUTPUT_FREQUENCY);
			if (mCodeMap != null) {
			...
			// 출력 전압
			//mCodeMap = result.get(MODBUS_LS_IP5A_CODE.OUTPUT_VOLTAGE);
			mCodeMap = result.get(MODBUS_LS_IS7_CODE.OUTPUT_VOLTAGE);
			if (mCodeMap != null) {
			...
			//lpEl = new LPData((result.get(MODBUS_LS_IP5A_CODE.DATE)).get("VALUE").toString(), lp, lpValue);
			lpEl = new LPData((result.get(MODBUS_LS_IS7_CODE.DATE)).get("VALUE").toString(), lp, lpValue);
