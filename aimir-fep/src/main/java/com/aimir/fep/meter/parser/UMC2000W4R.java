 ...
 */
@Service
public class UMC2000W4R extends ZEUPLS implements ModemParser, java.io.Serializable {
    ...
    private static Log log = LogFactory.getLog(UMC2000W4R.class);
    ...
    public void parse(byte[] data) throws Exception {
		...
		if (isOnDemand()) {
	        ...
	        serialNumber = DataUtil.getString(SERIAL_NUMBER).trim();
    ...
    public LinkedHashMap<String, String> getData() {
        ...
        res.put("meterStatus", CommonConstants.getWaterMeterStatus((2000+meterStatus)+"").getDescr());
    ...
    private String getWaterMeterAlarmStatus(byte status) {
        ...
        for (Enumeration<String> e = codes.keys(); e.hasMoreElements(); ) {
            ...
            buf.append("(");
            if ((Byte.parseByte((Integer.parseInt(key)%2000)+"") & status) != 0x00) {
                buf.append("Open");
            }
