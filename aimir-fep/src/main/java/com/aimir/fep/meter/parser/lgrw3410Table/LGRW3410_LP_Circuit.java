/* 
 * @(#)LGRW3410_LP_Circuit.java       1.0 08/05/29 *
 * 
 * Load Profile.
...
public class LGRW3410_LP_Circuit {
    ...
    public static final int MASK_LP_DATA = 0x3f;
	 ...
	 */
	public LGRW3410_LP_Circuit(byte[] rawData, int regK, double ke) {
    ...
    private LPData parseChannel(byte[] block) throws Exception{
        ...
        for(int i = 0; i < 2; i++){
            ...
            ch[i] =  new Double(dformat.format((double) (DataFormat.hex2dec( DataFormat.select(
                       block,OFS_PULSE+(i*LEN_PULSE),LEN_PULSE))) *ke*0.001));
        ...
        lpdata.setDatetime(getYyyymmddhhmmss(DataFormat.select(
                            block,OFS_LP_DATE, LEN_LP_DATE)));
        return lpdata;
    }
    ...
    private String getYyyymmddhhmmss(byte[] b) throws Exception {
		...
		int year = 2000+ DataFormat.hex2unsigned8(b[idx++]);
		int mm = DataFormat.hex2unsigned8((byte)(b[idx++]& 0x0F));
	...
	//	log.debug("YYYYMMDDHHMMSS :"+ret.toString());
		return ret.toString();
