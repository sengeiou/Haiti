/** 
 * @(#)MT6465.java       1.0 06/08/23 *
 * 
 * Instrumentation Profile Data Set I.
 ...
 */
public class MT6465 implements java.io.Serializable{
	...
	public MT6465(  byte[] dataset1, 
		...
		this.LEN_NBR_INTERVAL_RECORD_SET1 = nbr_blk_ints_set1*(1+nbr_chans_set1*5/2);
		...
		this.LEN_NBR_INTERVAL_RECORD_SET2 = nbr_blk_ints_set2*(1+nbr_chans_set2*5/2);
				...
				switch(melement){
					case 0x00:
						...
						if(!(dkvaa == null && dkvac == null)){
							dkvat =(dkvaa == null ? 0d : dkvaa) +(dkvac == null ? 0d : dkvac);
						}
						...
						if(!(dkwa == null && dkwc == null)){
							dkwt =(dkwa == null ? 0d : dkwa) +(dkwc == null ? 0d : dkwc);
						}
						

						dkvara = getCh2((byte[]) lst2.get(15), this.set2divisors[13]);
						dkvarc = getCh2((byte[]) lst2.get(17), this.set2divisors[15]);
						if(!(dkvara == null && dkvarc == null)){
							dkvart =(dkvara == null ? 0d : dkvara) +(dkvarc == null ? 0d : dkvarc);
						}	
						break;
					case 0x40:
						via    = getCh1((byte[]) lst1.get(2), this.set1divisors[0]);
						...
						dkvara = getCh2((byte[]) lst2.get(15), this.set2divisors[13]);
						dkvart = getCh2((byte[]) lst2.get(15), this.set2divisors[13]);	
						break;
					default :
						via   = getCh1((byte[]) lst1.get(2), this.set1divisors[0]);
						...
						if(!(dkvaa == null && dkvab == null && dkvac == null)){
							dkvat =(dkvaa == null ? 0d : dkvaa) +(dkvab == null ? 0d : dkvab)+(dkvac == null ? 0d : dkvac);
						}
						...
						if(!(dkwa == null && dkwb == null && dkwc == null)){
							dkwt =(dkwa == null ? 0d : dkwa) +(dkwb == null ? 0d : dkwb)+(dkwc == null ? 0d : dkwc);
						}

						dkvara = getCh2((byte[]) lst2.get(15), this.set2divisors[13]);
						dkvarb = getCh2((byte[]) lst2.get(16), this.set2divisors[14]);
						dkvarc = getCh2((byte[]) lst2.get(17), this.set2divisors[15]);
						
						if(!(dkvara == null && dkvarb == null && dkvarc == null)){
							dkvart =(dkvara == null ? 0d : dkvara) +(dkvarb == null ? 0d : dkvarb)+(dkvarc == null ? 0d : dkvarc);
						}
...
//		Double list = Double.parseDouble(df.format((double)d));
		
//		String capa = new DecimalFormat(".####").format(d);
//		Double list = Double.parseDouble(capa);
	...
	private void parseInstrumentI() {
		...
		int fst_blk_val_cnt = this.NBR_BLK_INTS_SET1;//NBR_VALID_INT_SET1;
		int empty_blk_cnt = 0;
		...
		try{
			if(fst_blk_val_cnt > 0){
				byte[] first_block 
					= new byte[this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT_SET1+fst_blk_val_cnt*(this.LEN_EXT_INTERVAL_STATUS_SET1+this.LEN_INTERVAL_DATA_SET1)];
				first_block 
					= DataFormat.select(
						dataset1,0,this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT_SET1+fst_blk_val_cnt*(this.LEN_EXT_INTERVAL_STATUS_SET1+this.LEN_INTERVAL_DATA_SET1));
				
				endtime = getQuaterYymmddhhmm(
									getYymmddhhmm(first_block,0,this.LEN_BLK_END_TIME));
								
				status = getBlkStatus(
									DataFormat.select(
										first_block,this.LEN_BLK_END_TIME,this.LEN_SIMPLE_INTVAL_STAT_SET1),
									fst_blk_val_cnt);
				int statusCnt = 0;
				for(int i=0; i < status.length() ; i++){
					if(status.substring(i, i+1).equals("1")) statusCnt++;
				}
				
				//마지막 시간 + 유효한 데이터 수
				int preDataSize = this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT_SET1;
				//데이터 상태 값 + 검침값
				int dataSize 	= this.LEN_EXT_INTERVAL_STATUS_SET1+this.LEN_INTERVAL_DATA_SET1;
				int interval = 15;
				String mm = getYymmddhhmm(first_block,0,this.LEN_BLK_END_TIME).substring(10, 12);
				...
				instI = DataFormat.select(first_block, preDataSize, fst_blk_val_cnt*dataSize);
			...
			for(int i = 1; i < block_count; i++){
				...
				if(blk[0] == (byte)0xFF){
					...
					endtime = getQuaterYymmddhhmm( getYymmddhhmm(blk,0,this.LEN_BLK_END_TIME) );
//					Util.getQuaterYymmddhhmm( getYymmddhhmm(blk,0,this.LEN_BLK_END_TIME),this.INT_TIME_SET1);

					status = getBlkStatus(
								DataFormat.select(
									blk,this.LEN_BLK_END_TIME,this.LEN_SIMPLE_INTVAL_STAT_SET1),this.NBR_BLK_INTS_SET1);
											
					instI = DataFormat.select(blk,
											this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT_SET1,
											this.NBR_BLK_INTS_SET1*(this.LEN_EXT_INTERVAL_STATUS_SET1+this.LEN_INTERVAL_DATA_SET1));
	...
	public void parseInstrumentII() {
			...
			int fst_blk_val_cnt = this.NBR_BLK_INTS_SET2;//NBR_VALID_INT_SET2;
			int empty_blk_cnt = 0;
			...
			try{
				if(fst_blk_val_cnt > 0){
					byte[] first_block 
						= new byte[this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT_SET2+fst_blk_val_cnt*(this.LEN_EXT_INTERVAL_STATUS_SET2+this.LEN_INTERVAL_DATA_SET2)];
					first_block 
						= DataFormat.select(
							dataset2,0,this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT_SET2+fst_blk_val_cnt*(this.LEN_EXT_INTERVAL_STATUS_SET2+this.LEN_INTERVAL_DATA_SET2));
					...
					status = getBlkStatus(
										DataFormat.select(
											first_block,this.LEN_BLK_END_TIME,this.LEN_SIMPLE_INTVAL_STAT_SET2),
										fst_blk_val_cnt);
					...
					//마지막 시간 + 유효한 데이터 수
					int preDataSize = this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT_SET2;
					//데이터 상태 값 + 검침값
					int dataSize 	= this.LEN_EXT_INTERVAL_STATUS_SET2+this.LEN_INTERVAL_DATA_SET2;
					int interval = 15;
					String mm = getYymmddhhmm(first_block,0,this.LEN_BLK_END_TIME).substring(10, 12);
					//마지막 시간이 lp주기 시간이 아닐경우 첫번째 데이터 누락								
					if(Integer.parseInt(mm)%interval != 0) {
					...
					instII = DataFormat.select(first_block, preDataSize, fst_blk_val_cnt*dataSize);
				...
				for(int i = 1; i < block_count; i++){
					...
					if(blk[0] == (byte)0xFF){
						...
						status = getBlkStatus(
									DataFormat.select(
										blk,this.LEN_BLK_END_TIME,this.LEN_SIMPLE_INTVAL_STAT_SET2),this.NBR_BLK_INTS_SET2);
											
						instII = DataFormat.select(blk,
												this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT_SET2,
												this.NBR_BLK_INTS_SET2*(this.LEN_EXT_INTERVAL_STATUS_SET2+this.LEN_INTERVAL_DATA_SET2));
...
//				ofs = (instcount.intValue()-1)*(this.LEN_EXT_INTERVAL_STATUS_SET1+this.LEN_INTERVAL_DATA_SET1);
//			}
			//int ofs = instIs.length - (this.LEN_EXT_INTERVAL_STATUS_SET1+this.LEN_INTERVAL_DATA_SET1);
			
			int idx = 0;
			for(int i = 0; i < instcount.intValue(); i++){
				String instItime = Util.addMinYymmdd(endtime,-this.INT_TIME_SET1*i);
				byte[] instIstat = DataFormat.select(instIs,ofs,this.LEN_EXT_INTERVAL_STATUS_SET1);
				
//				if(instIstat.equals(new byte[]{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00})) break;
				if(!Arrays.equals(instIstat, new byte[]{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00})) break;
				...
				for(int j = 0; j < this.NBR_CHANS_SET1; j++){
					byte[] ch = DataFormat.LSB2MSB(
									DataFormat.select(
										instIs,ofs,this.LEN_INTERVAL_DATA_SET1/this.NBR_CHANS_SET1));
...
//				ofs = instIs.length - (i+2)*(this.LEN_EXT_INTERVAL_STATUS_SET1+this.LEN_INTERVAL_DATA_SET1);
			}
...
//			interval = orderby(interval,emptycnt);
	...
	protected ArrayList[] parseInstII(ArrayList list) throws Exception {
		...
		try{
			...
			int ofs = 0;
			if(instcount.intValue() > 0){
				ofs = (instcount.intValue()-1)*(this.LEN_EXT_INTERVAL_STATUS_SET2+this.LEN_INTERVAL_DATA_SET2);
			}
			//int ofs = instIIs.length - (this.LEN_EXT_INTERVAL_STATUS_SET2+this.LEN_INTERVAL_DATA_SET2);
			
			int idx = 0;
			for(int i = 0; i < instcount.intValue(); i++){
				String instIItime = Util.addMinYymmdd(endtime,-this.INT_TIME_SET2*i);
				byte[] instIIstat = DataFormat.select(instIIs,ofs,this.LEN_EXT_INTERVAL_STATUS_SET2);
				...
				for(int j = 0; j < this.NBR_CHANS_SET2; j++){
					byte[] ch = DataFormat.LSB2MSB(
									DataFormat.select(
										instIIs,ofs,this.LEN_INTERVAL_DATA_SET2/this.NBR_CHANS_SET2));
					ofs += this.LEN_INTERVAL_DATA_SET2/this.NBR_CHANS_SET2;
					interval[idx].add(2+j,ch);
				}
				idx++;

				ofs = instIIs.length - (i+2)*(this.LEN_EXT_INTERVAL_STATUS_SET2+this.LEN_INTERVAL_DATA_SET2);
			}
...
//			newlists[i] = lists[size-i-1];
	...
	private String getBlkStatus(byte[] b, int blkcnt){
		...
		for(int i = 0; i < b.length; i++){
			...
			StringBuffer temp = new StringBuffer(
				Util.frontAppendNStr('0',Integer.toBinaryString(val),8));
			str += temp.reverse();
		}

		return str;//str.substring(0,blkcnt);		//blkcnt??????????  - INT_Nbr_Blks_Set1
	}
	...
	/*
	private byte[] getSet1Ch(byte[] ch, int divisor){

		float f;
		byte[] chan = {(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF};
		try {

			if(DataFormat.hex2signed16(ch) != -1){
				f = (float) (DataFormat.hex2signed16(ch) * divisor * this.ins_scale);
				chan = DataFormat.float32_2_hex(f);
			}
	...
	private byte[] getSet2Ch(byte[] ch, int divisor){
		...
		try {
			...
			if(DataFormat.hex2signed16(ch) != -1){
				String val = (float) 
					(DataFormat.hex2signed16(ch)*divisor*this.ins_scale)*0.001+"";

				chan = DataFormat.checkArrayBPost(
							val.getBytes(),14);
			}							
		} catch (Exception e) {
			logger.warn(e.getMessage());
