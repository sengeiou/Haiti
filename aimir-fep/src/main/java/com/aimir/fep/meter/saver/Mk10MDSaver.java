...
public class Mk10MDSaver extends AbstractMDSaver {
    ...
    protected boolean save(IMeasurementData md) throws Exception {
    	...
    	Mk10 parser = (Mk10)md.getMeterDataParser();    	
        ...
        if(parser.getSwVersion() != null && !"".equals(parser.getSwVersion())){
        ...
        if (meterEventLog != null && meterEventLog.length > 0) {
          	  saveMeterEventLog(parser.getMeter(), meterEventLog);
        }
        
        PowerAlarmLogData[] powerAlarm = parser.getPowerAlarmLogData();
        if(powerAlarm != null && powerAlarm.length > 0){
        	savePowerAlarmLog(parser.getMeter(), powerAlarm);
        }
        ...
        if(lplist != null && lplist.length > 0){
            ...
            double basePulse = parser.getLpValue() == null ? 0d : parser.getLpValue();
