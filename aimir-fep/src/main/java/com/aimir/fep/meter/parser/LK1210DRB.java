...
/**
 * parsing LK1210DRB, LK3410DRB Meter Data
 *
 * @author Kang, Soyi (ksoyi@nuritelecom.com)
 * @version $Rev: 1 $, $Date: 2008-03-28 $,
 */
public class LK1210DRB extends MeterDataParser implements java.io.Serializable {
    ...
    /**
     * parseing Energy Meter Data of LK1210DRB, LK3410DRB Meter
    ...
    public void parse(byte[] data) throws Exception
    {
        ...
        if (data.length < 53) {
            ...
            if(data.length == 38)
            {
                ...
                mdm = b;
                log.debug("[LK1210DRB_MDM] len=["+LEN_MDM_INFO+"] data=>"+Util.getHexString(mdm));
                nuri_mdm = new LK1210DRB_MDM(mdm);
                log.debug(nuri_mdm);
            }
            else
            {
                log.error("[LK1210DRB] Data total length[" + data.length + "] is invalid");
            }
        ...
        mtr = b;
        offset += LEN_MTR_INFO;
        log.debug("[LK1210DRB_MTR] len=["+LEN_MTR_INFO+"] data=>"+Util.getHexString(mtr));
        nuri_mtr = new LK1210DRB_MTR(mtr);
        log.debug(nuri_mtr);
        
        b = new byte[LEN_METER_STATUS];
        System.arraycopy(data,OFS_METER_STATUS,b,0,LEN_METER_STATUS);
        ev2 =b;
        log.debug("[LK1210DRB_METER_STATUS] len=["+LEN_METER_STATUS+"] data=>"+Util.getHexString(ev2));
	    ...
	    //Circuit LP
	    if(offset<totlen && data[offset]==(byte)0x81){
	    	len = totlen - offset;
	        if(len>=24)	{
	    	b = new byte[len];
		        System.arraycopy(data,offset,b,0,len);
        ...
        if(pb != null){
            nuri_pb = new LK1210DRB_PB(pb, demandResetDatetime, regK);
        }

        if(cb != null){
            nuri_cb = new LK1210DRB_CB(cb, meterTime, regK);
        }
        
        if(lpd != null){
            nuri_lp = new LK1210DRB_LP(lpd, regK, resolution, ct*vt);
        }
        
        if(ev != null || ev2 !=null ){
            nuri_ev = new LK1210DRB_EV(ev, ev2);
        }
    ...
    public String toString()
    {
        ...
        sb.append("NURI_LK1210DRB Meter DATA[");
          ...
          //  tou_block = getCurrBilling();
            ...
            if(nuri_mtr != null){          
              //  res.put("ProgramNameVer",nuri_mtr.getProgramNameVer().toString());
                ...
                res.put("Meter ID",nuri_mtr.getMeterManufacture().getMETER_SERIAL());
