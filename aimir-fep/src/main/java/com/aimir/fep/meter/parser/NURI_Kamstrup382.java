...
/**
 * parsing Kamstrup 382 meter data
 *
 * @author Y.K Park (goodjob@nuritelecom.com)
 * @version $Rev: 1 $, $Date: 2008-11-06 11:06:15 +0900 $,
 */
public class NURI_Kamstrup382 extends MeterDataParser implements java.io.Serializable {
    ...
    public void parse(byte[] data) throws Exception
    {
        ...
        pos += MDM.length;
        this.nuri_mdm = new KAMSTRUP_MDM(MDM);
        ...
        if(!nuri_mdm.isEventOnly())
        {
            ...
            if (period == 0) {
                ...
                for (int i = 0; i < lpcnt; i++) {                	
                     ...
                     */
					if(i==0) {
						previousLPData = getPreviewLPData(meterId, tempDate2.substring(0,8));
					}
   ...
   public String getSwName(){
    ...
    private LPData[] getPreviewLPData(String meterId, String yyyymmdd) {		
    	...
    	try{
    		...
    		//TODO Implement 상세 데이터 가져오는 부분
    		
    		//list를 lpData클래스로 변경해야 함
    	}catch(Exception e){
    		log.warn(e,e);
    	}
		...
		ArrayList<LPData> al = null;
		try {
			String sql = "select yyyymmdd,hhmmss,yyyymmddhhmmss, lp,ch1,v1,flag "
				+ "from current_em where meter_id=? and yyyymmddhhmmss>=? and yyyymmddhhmmss<=? "
				+ " and hhmmss like '%0000' order by yyyymmddhhmmss ";
			con = JDBCUtil.getConnection();
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, meterId);
			pstmt.setString(2, TimeUtil.getPreDay(yyyymmdd).substring(0,8));
			pstmt.setString(3, TimeUtil.getPreDay(yyyymmdd,-1).substring(0,8));
			rs = pstmt.executeQuery();
			if(rs!=null) {
				al = new ArrayList<LPData>();
				while(rs.next()){
					LPData temp = new LPData();
					temp.setDatetime(rs.getString("yyyymmddhhmmss"));
					if(temp.getDatetime().endsWith("000000")) {
						LPData temp2 = new LPData();
						temp2.setDatetime(TimeUtil.getPreHour(rs.getString("yyyymmddhhmmss")));
						temp2.setBasePulse(rs.getLong("LP"));
						temp2.setCh(new Double[]{new Double(rs.getLong("LP"))});
						temp2.setV(new Double[]{new Double(rs.getLong("LP"))});
						temp2.setFlag(rs.getInt("FLAG"));
						temp2.setPF(new Double(1));
						al.add(temp2);

						temp.setBasePulse(rs.getLong("CH1"));
						temp.setCh(new Double[]{new Double(rs.getLong("CH1"))});
						temp.setV(new Double[]{new Double(rs.getLong("CH1"))});
					} else {
						temp.setBasePulse(rs.getLong("CH1"));
						temp.setCh(new Double[]{new Double(rs.getLong("CH1"))});
						temp.setV(new Double[]{new Double(rs.getLong("CH1"))});
		...
		if( al != null && al.size()>0) {
			return (LPData[]) al.toArray(new LPData[0]);
		}
		return null;
		*/
