...
//        METER_MODEL("0000600107FF", "Meter Model"), //0-0:96.1.7.255  Meter Type -> Model
        ...
        METER_SERIAL_NUMBER("0000600100FF","Meter Serial No."),
        METER_TYPE("0000605B09FF","Meter Type"),
        CATEGOTY("0000605B0BFF","Category"),
        APP_FW_ID("0100000200FF","APP Firmware Identifier"),
        //DISCONNECT_CONTROL("000060030AFF","Disconnect control(control_state)"),
        METER_MODEL("0100000201FF","Meter model (Location information)"),
        LP_INTERVAL("0100630100FF","LP Interval"),
        //TRANFORMER_RATIO("01000B237FFF", "Transformer Ratio"),
        RELAY_STATUS("000060030AFF","Relay Status"),
        MONTHLY_BILLING_PROFILE("0100620100FF","Monthly billing profile"),
        VOLTAGE_RELATED_EVENTS("0000636200FF","Voltage related events"),
        CURRENT_RELATED_EVENTS("0000636201FF","Current related events"),
        POWER_RELATED_EVENTS("0000636202FF","Power related events"),
    	...
    	// -> UPDATE START 2018/04/05 #SP-929
    	//MBUS_CLIENT_SETUP_CHANNEL4("0004180100FF", "M-Bus Client Setup Channel 4"); // 0.4:24.1.0.255
    	MBUS_CLIENT_SETUP_CHANNEL4("0004180100FF", "M-Bus Client Setup Channel 4"), // 0.4:24.1.0.255
    	MBUS_DIGITAL_DATA("00015E1F06FF", "M-Bus Digital Data"), // 0-1:94.31.6.255
    	MBUS_CURRENT_ANALOG_VALUE("0100010900FF", "M-Bus Current Sample Analog Value"), // 1-0:1.9.0.255
    	MBUS_VOLTAGE_ANALOG_VALUE("0100010A00FF", "M-Bus Voltage Sample Analog Value"); // 1-0:1.10.0.255
    	// <- UPDATE END   2018/04/05 #SP-929
    ...
    public enum ENERGY_LOAD_PROFILE {
        ...
        IntervalActiveEnergyImport(4, "Interval Active Energy -Import"),
        IntervalActiveEnergyExport(5, "Interval Active Energy -Export"),
        AverageDemandValueImport(6, "Average Demand Value / Time of Active Power -Import"),
        AverageDemandValueExport(7, "Average Demand Value / Time of Active Power -Export");
...
//    }
    ...
    */
    ...
    public enum METER_DEVICE_MODEL {
    	UNKNOW(""),
    	CL710K22("1P"),
    	CL730S22("3P"),
    	CL730D22H("CT"),
    	...
    	METER_DEVICE_MODEL(String phase) {
    		this.phase = phase;
    	}
    ...
    public static METER_DEVICE_MODEL getMeterDeviceModel(String name) {
    	for(METER_DEVICE_MODEL mm : METER_DEVICE_MODEL.values()) {
    		if(mm.name().contentEquals(name)) {
    			return mm;
    		}
    	}
    	
    	return METER_DEVICE_MODEL.UNKNOW;
    }
    ...
    public static String getDataName(Meter meter, OBIS obis, int cnt, int structure) {
/*
 ...
 */
    	...
    	if (obis == OBIS.POWERFAILURE_LOG) {
  		...
  		if (obis == OBIS.ENERGY_LOAD_PROFILE) {
  			METER_DEVICE_MODEL meterModel = getMeterDeviceModel(meter.getModel().getName());
  			
  			switch(meterModel) {
  			case CL710K22:
  				for (ENERGY_LOAD_PROFILE profile : ENERGY_LOAD_PROFILE.values()) {
  	                if (profile.getCode() == (cnt % ENERGY_LOAD_PROFILE.values().length))
  	                    return profile.name();
  	            }
  				break;
  			case CL730S22:
    ...
    */
     ...
     * 
     */
    @XmlType(name = "RELAY_STATUS_CLOU")
    @XmlEnum
    public enum RELAY_STATUS_CLOU {
        ...
        public static RELAY_STATUS_CLOU getValue(int code) {
            for (RELAY_STATUS_CLOU a : RELAY_STATUS_CLOU.values()) {
                if (a.getCode() == code)
                    return a;
            }
            return null;
