...
import com.aimir.fep.protocol.nip.client.batch.excutor.RunnableMultiBatchExecutor;
...
import com.aimir.fep.protocol.nip.client.batch.job.SORIAMbbOTARunnable;
...
public class CommandGW implements CommandGWMBean {
	...
	public enum OnDemandOption {
		...
		WRITE_OPTION_TIMESYNC(8), // Timesync (Er000002 Non Clear) - iraq
									// support
		READ_EVENT_LOG(10), READ_IDENT_ONLY(11); // Meter Identification only -
													// iraq support
	...
	public void cmdMcuReset(String mcuId) throws FMPMcuException, Exception {
		...
		// INSERT START SP-112
		if (target.getNameSpace() != null && (target.getNameSpace().equals("SP") || target.getNameSpace().equals("PH"))) {
			datas = new Vector<SMIValue>();
	...
	public Map cmdOnDemandMeter(String mcuId, String meterId, int nOption) throws FMPMcuException, Exception {
		...
		try {
			...
			if (target.getNameSpace() != null && target.getNameSpace().equals("NG")) {
			...
			if (target.getNameSpace() != null && (target.getNameSpace().equals("SP") || target.getNameSpace().equals("PH"))) {
				...
				if (modem.getMcu() != null && modem.getMcu().getSysSwRevision() != null
						&& modem.getMcu().getSysSwRevision().compareTo(mcuRevision) >= 0) {
					smiValue = DataUtil.getSMIValueByObject("sensorID", modem.getDeviceSerial());
				} else {
					smiValue = DataUtil.getSMIValueByObject("sensorID", modem.getDeviceSerial());
			...
			if (smiValues != null && smiValues.length != 0) {
				...
				if (target.getNameSpace() != null && (target.getNameSpace().equals("SP") || target.getNameSpace().equals("PH"))) {
					...
					log.info("smiValues[1]  "+smiValues[1].toString());
					if (smiValues[1].getVariable() instanceof OPAQUE)
						log.info("smiValues[1] == OPAQUE");
					log.info("smiValues[2]  "+smiValues[2].toString());

					if ( smiValues.length==2 || smiValues.length==3){
						if ( Integer.parseInt(smiValue.getVariable().toString()) != ErrorCode.IF4ERR_NOERROR ) {
							...
							if (smiValue.getVariable() instanceof OPAQUE) {
								...
								if(smiValues.length==2) {
									...
									DLMSClou clou = (DLMSClou) edp;
									map = clou.getRelayStatus();
								}
	...
	public byte[] cmdWriteTable(String mcuId, String meterId, int tablenum, byte[] stream)
		...
		try {
			...
			if (meter.getModem().getMcu() != null && meter.getModem().getMcu().getSysSwRevision() != null
					&& meter.getModem().getMcu().getSysSwRevision().compareTo(mcuRevision) >= 0) {
			...
			if (obj instanceof Integer) {
				...
				if (ErrorCode.getCodeString(errcode).equals("UKNOWN ERROR CODE")) {
					...
					throw new FMPMcuException("UKNOWN ERROR CODE", errcode);
				} else {
	...
	public MeterData cmdOnDemandMeter(String mcuId, String meterId, String modemId, String nOption, String fromDate,
		...
		try {
			...
			// INSERT START SP-116
			else if ("SP".equals(target.getNameSpace()) || "PH".equals(target.getNameSpace())) {
			...
			// INSERT END SP-116
				...
				smiValue = DataUtil
						.getSMIValue(new INT(Integer.parseInt(nOption == null || nOption.equals("") ? "0" : nOption)));
				datas.add(smiValue);
				if ((modemType == ModemType.MMIU || modemType == ModemType.Converter_Ethernet)
	...
	public Hashtable cmdStdGet(String mcuId, String[] propNames) throws FMPMcuException, Exception {
		...
		*/
		Target target = CmdUtil.getTarget(mcuId);
		Vector<SMIValue> mibPropNames = new Vector<SMIValue>();
        ...
        try {
            for (String propName : propNames)
            {
                if (propName != null && !"".equals(propName)) {
                	...
                	OID oid = mibUtil.getOid(propName);  // OID
                	if(oid == null) {
                		throw new Exception(makeMessage("Can not find OID."));
                	}
                	
                    log.debug("mib property name="+ propName + ", OID = " + oid.value);
                    mibPropNames.add(DataUtil.getSMIValueByObject(target.getNameSpace(), "oidEntry", oid.value));
                }
            }			
		} catch (Exception e) {
            log.error("SMI Parsng error - " + e.getMessage(), e);
            throw new Exception(makeMessage(e.getMessage()));
		}
		...
		if (smiValues != null && smiValues.length != 0) {
			for (int i = 0; i < smiValues.length; i++) {
				...
				res.putAll(CmdUtil.convMCUSMItoMOP(target.getNameSpace(), smiValues[i]));
			}
		} else {
			log.debug("smiValues is null");
		}
	...
	public byte[] cmdMeterTimeSync(String mcuId, String meterId) throws FMPMcuException, Exception {
		...
		try {
			...
			meter = meterDao.get(meterId);
			modelCode = meter.getModel().getCode();
			Modem modem = meter.getModem();
	...
	public Map cmdOnDemandByMeter(String mcuId, String meterId, String modemId, int nOption)
		...
		try {
			...
			Map map = null;
			log.debug(smiValue.toString());
			if (smiValue.getVariable() instanceof BYTE && (edp instanceof DLMSNamjun)) {
				...
				case 3: // Relay switch table read
					...
					/*
					 * : 1-ON, 0-Off LSB 0 : L1 relay status 1 : L2 relay status
					 * 2 : L3 relay status 3 : External relay status 4 : L1
					 * relay error 5 : L2 relay error 6 : L3 relay error 7 :
					 * External relay error 8 : Open terminal cover 9 : Open
					 * terminal cover in power off 10: Open top cover 11: Open
					 * top cover in power off 12: Magnetic detection 1
					 * 13:Magnetic detection 2 14: Program 15: Factory status
					 * MSB
					 */
...
//	}
	...
	public Hashtable cmdReqNodeUpgrade(String mcuId, int upgradeType, int control,
        ...
        Object[] params = 
         ...
         Object obj = null;
         try {
        	 log.info("#### ====> [cmdReqNodeUpgrade] invoke [" + mcuId + "]");
             obj = invoke(params, types);
             log.info("#### <==== [cmdReqNodeUpgrade] invoke [" + mcuId + "]");
         } catch (Exception e) {
         	 log.error("cmdReqNodeUpgrade ERROR : " + e.getMessage(), e);
             throw new Exception(makeMessage(e.getMessage()));
         } 
	...
	public Hashtable cmdReqImagePropagate(String mcuId, int upgradeType, int control,
			String imageKey, String imageUrl, String upgradeCheckSum, 
			String imageVersion, String targetModel , int cloneCount, String add3Day, List<String> filterValue) throws FMPMcuException, Exception {
		...
		try {
			...
			if(imageKey != null && 0 < imageKey.indexOf(".")){
				String[] versionArray = imageKey.split("\\.");
				imageKey = (String.format("%02d", Integer.parseInt(versionArray[0])) + String.format("%02d", Integer.parseInt(versionArray[1])));
			}
			...
			}else if(imageVersion != null && 4 <= imageVersion.length()){
				imageVersion = imageVersion.substring(imageVersion.length() - 4, imageVersion.length());
			}
			
			byte[] hexVersion = Hex.encode(imageVersion);
			log.info("imageVersion hex = " + Hex.decode(hexVersion));
			datas.add(DataUtil.getSMIValue(new WORD(hexVersion)));
			datas.add(DataUtil.getSMIValueByObject("stringEntry", targetModel));
			datas.add(DataUtil.getSMIValue(new BYTE(cloneCount)));    // Clone count
			if(add3Day != null && !add3Day.equals("")) {
				datas.add(DataUtil.getSMIValue(new TIMESTAMP(add3Day)));  // SP-1100 Clone Operating time				
			}
			
			/** 0x07 (Third-party Modem) */
			if (upgradeType == 7 && filterValue != null) {
		        for(String entry : filterValue){
		        	datas.add(DataUtil.getSMIValue(new INT(2))); // Filter : 0x02 (FILTER_MODEMID)
		           	datas.add(DataUtil.getSMIValueByObject("stringEntry", entry)); 
		        }
			}
			log.info("cmdReqImagePropagate datas ==> " + datas.toString());
	        ...
	        Object[] params = 
        	 ...
        	 log.info("#### ====> [cmdReqImagePropagate] invoke [" + mcuId + "]");
             obj = invoke(params, types);
             log.info("#### <==== [cmdReqImagePropagate] invoke [" + mcuId + "]");
         } catch (Exception e) {
         	 log.error("cmdReqImagePropagate ERROR - " + e.getMessage(), e);
             throw new Exception(makeMessage(e.getMessage()));
         } 
	...
	public Hashtable cmdGetImagePropagateInfo(String mcuId, int upgradeType)  throws FMPMcuException, Exception {
        ...
        Object[] params = 
         ...
         Object obj = null;
         try {
        	 log.info("#### ====> [cmdGetImagePropagateInfo] invoke [" + mcuId + "]");
             obj = invoke(params, types);
             log.info("#### <==== [cmdGetImagePropagateInfo] invoke [" + mcuId + "]");
         } catch (Exception e) {
         	 log.error("cmdGetImagePropagateInfo ERROR : " + e.getMessage(), e);
             throw new Exception(makeMessage(e.getMessage()));
         } 
	...
	public String coapPing(java.lang.String ipAddress, java.lang.String device, java.lang.String type, java.lang.String protocol, java.lang.String fwVersion) throws Exception {
		Target target = new Target();
		...
		if(type.equals(ModemIFType.MBB.name())) {
			
        	if(protocol.equals(Protocol.SMS.name())) {
        		target.setProtocol(Protocol.SMS);
        	}else if(protocol.equals(Protocol.GPRS.name())) {
        		target.setProtocol(Protocol.GPRS);
        	}else {
        		target.setProtocol(Protocol.SMS);
        	}	
			target.setTargetType(McuType.MMIU);
		// INSERT START SP-828
		} else if (type.equals(ModemIFType.Ethernet.name())) {
			target.setProtocol(Protocol.IP);
			target.setTargetType(McuType.MMIU);
			
		// INSERT END SP-828
		}else {
			target.setFwVer(fwVersion); 
		}
		target.setPort(5683);
		SyncCoapCommand client = new SyncCoapCommand();
		result = client.sendCoapPing(target, device, type);
    ...
    public Map<String, String> coapGetInfo (
            ...
            java.lang.String protocol, java.lang.String fwVersion
            ) throws Exception {
        ...
        if( (ipv4 != null && !"".equals(ipv4)) || (ipv6 != null && !"".equals(ipv6))){ // IPv4, IPv6 둘 중 하나라도 주소 값이 있을경우 동작
	        if (ipv4 != null && !"".equals(ipv4)) {  // IPv4 주소 값이 있을경우
	            ...
	            if(type.startsWith(ModemIFType.RF.name())){
		            ...
		            if(TimeLocaleUtil.isThisDateValid(frmForRF.getYyyymmddhhmmss(), "yyyyMMddHHmmss")){
		            map.put("LastCommDate", frmForRF.getYyyymmddhhmmss()+"");
		            }else{
		            	map.put("LastCommDate", "");
		            }
		            ...
		            if(TimeLocaleUtil.isThisDateValid(frmForETHER.getYyyymmddhhmmss(), "yyyyMMddHHmmss")){
	            ...
	            }else if(type.equals(ModemIFType.MBB.name())){
	            	
	            	if(protocol.equals(Protocol.SMS.name())) {
	            		tag.setProtocol(Protocol.SMS);
	            	}else if(protocol.equals(Protocol.GPRS.name())) {
	            		tag.setProtocol(Protocol.GPRS);
	            	}else {
	            		tag.setProtocol(Protocol.SMS);
	            	}	            	
		            ...
		            if(TimeLocaleUtil.isThisDateValid(frmForMBB.getYyyymmddhhmmss(), "yyyyMMddHHmmss")){
    ...
    public Map<String, String> modemReset (
            java.lang.String ipv4 , java.lang.String ipv6, java.lang.String type, java.lang.String protocol, java.lang.String fwVersion
            ) throws Exception {
    ...
    public Map<String, String> coapBrowser (
            java.lang.String ipv4 , java.lang.String ipv6, java.lang.String uri, String query, String config, String type, java.lang.String protocol, java.lang.String fwVersion
            ) throws Exception {
        log.info("TARGET_NODE[" + ipv4 + " / " + ipv6 +"]");
...
//		log.debug("## NI command - Modem Event Log ["+mdsId+"],["+logCount+"]");
	...
	public Map<String, Object> 	setCloneOnOff(String modemId, String cloneCode, Integer count) throws Exception {
		return setCloneOnOffWithTarget(modemId, cloneCode, count, null, null, null);
	}
	
	// INSERT START SP-681
	public Map<String, Object> 	setCloneOnOffWithTarget(String modemId, String cloneCode, Integer count, String version, Integer euiCount, List<String> euiList) throws Exception {
		...
		try {
			...
			if(cloneCode == null || cloneCode.equals("") || count == null) {
			...
			if(euiList != null && 0 < euiList.size()) {
...
//			target.setProtocol(Protocol.IP);
			...
			} else if (target.getNameSpace().equals("SP") || target.getNameSpace().equals("PH")) { // SORIA,PKS
    ...
    public Map<String,Object> cmdMeterParamSet(String modemId, String param) throws Exception{
		...
		if(target != null){
			...
			} else if (target.getNameSpace().equals("SP") || target.getNameSpace().equals("PH")) { // SORIA
				Map<String, Object> params = new HashMap<String, Object>();
...
//				target.setProtocol(Protocol.IP);
				...
				if (target.getNameSpace().equals("NG")) { // IRAQ
				...
				} else if (target.getNameSpace().equals("SP")||target.getNameSpace().equals("PH")||target.getNameSpace().equals("PG")) { // SORIA,PKS(PLC),PKS(3G)
					...
					try  {
						...
						Object obj = bypassResult.getResultValue();
						if(obj instanceof HashMap) {
							resultMap = (HashMap<String, Object>)obj;
							log.debug("### cmdBypassGetMeterFWVersion result = " + String.valueOf(resultMap.get("resultValue")));	
						}else if(obj instanceof String) {
							resultMap.put("result", false);
							resultMap.put("resultValue",(String)obj);
						}
					} catch (Exception e) {
	...
	/**
	 * Command 실행 : cmdSORIASetMeterSerial
	 * 
	 * @param modemId
	 * @return
	 * @throws Exception
	 */
	public Map<String, Object> cmdSORIASetMeterSerial(String eui) throws Exception {
		Map<String,Object> resultMap = new HashMap<String,Object>();

		try {
			log.debug("cmdSORIASetMeterSerial(" + eui+ ")");

			Target target = CmdUtil.getNullBypassTarget(modemDao.get(eui));
			if(target != null){
				log.debug("## Target = " + target.toString());
				...
				if (target.getNameSpace().equals("NG")) { // IRAQ
					...
					try  {
						...
						resultMap = (HashMap<String, Object>)bypassResult.getResultValue();
						log.debug("### cmdSORIASetMeterSerial result = " + String.valueOf(resultMap.get("resultValue")));
			...
			log.error("cmdSORIASetMeterSerial error - " + e, e);
		}
			...
			for(String deviceId : deviceList){
				 ...
				 */
					...
					if (modem.getModemType() == ModemType.MMIU && modem.getProtocolType() == Protocol.SMS) { // MBB Modem
						useNullBypass = true;
					}else if (modem.getModemType() == ModemType.MMIU && modem.getProtocolType() == Protocol.GPRS) { // MBB GPRS(tcp mode) Modem
				...
				try {
					...
					for(String mcuSysId : mcuList){
						...
						if(otaTargetType == OTATargetType.DCU_KERNEL){
							...
							otaType = OTAType.DCU_COORDINATOR;
						}else{
							otaType = OTAType.DCU;
						}
...
//						}
	...
	/**
	 * MultiFirmwareOTA F/W file make.
	 * 
	 * @param firmwareId
	 * @param otaTargetType
	 * @param otaExecuteType
	 * @return
	 * @throws Exception
	 */
	private Map<String, Object> mfOTAmakeFWFile(String firmwareId, OTAExecuteType otaExecuteType) throws Exception{
		// 일단 true로 세팅하고 추후 확인할것.
		//SP-985
		boolean take_over = Boolean.parseBoolean(FMPProperty.getProperty("ota.firmware.meter.takeover.use","false"));		
		/*
		 * 1. Firmware 유무확인 및 준비
		 */
		log.debug("firmwareId=" + firmwareId + ", otaExecuteType=" + otaExecuteType);
		Firmware firmware = firmwareDao.get(Integer.parseInt(firmwareId));
		if(firmware == null){
			throw new Exception("Unknown Firmware");
		}
		...
		String fw_version = firmware.getFwVersion();
		String imageKey = firmware.getImageKey();
		String model = firmware.getEquipModel();
		String fwFileName = firmware.getFileName();
		
		log.debug("Firmware Info. fileURL=[" + fileURL + "], checksum=[" + checkSum + "], fw_crc=[" + fw_crc + "]"
				+ ", fw_version=[" + fw_version + "], imageKey=[" + imageKey + "], model=[" + model + "], fwFileName=[" + fwFileName + "]");
		...
		params.put("fwFileName", fwFileName);
		
		if(otaExecuteType == OTAExecuteType.EACH_BY_DCU || otaExecuteType == OTAExecuteType.CLONE_OTA){
			/**
			 * # mode=0 => http, mode=1 => https
			   #ota.firmware.download.mode=1
			   #ota.firmware.download.port=8081
			...
			if(IPUtil.checkIPv4(ipAddress))
				...
				log.error("Firmware download IP is invalid - " + ipAddress);
				throw new Exception("Firmware download IP is invalid - " + ipAddress);
			}
			...
			Path filePath = Paths.get((String)params.get("fw_path"));				
			byte[] fileArray = Files.readAllBytes(filePath);
			//byte[] fileArray = new byte[]{};  //for test code by SoapUI
...
//	 * @param mcuList
//	 * @param targetList
//	 * @param mbbTypeTargetList
//	 * @param invalidTargetList
//	 * @throws Exception 
//	 */
//	private void mfOTAmakeTarget(String issueDate, OTATargetType otaTargetType, OTAExecuteType otaExecuteType
//			, Map<String, List<String>> mcuListMap, List<String> mcuList, List<Target> targetList, List<Target> mbbTypeTargetList, List<String> invalidTargetList) throws Exception{
//		log.debug("issueDate=" + issueDate + ", OTATargetType=" + otaTargetType + ", OTAExecuteType=" + otaExecuteType + ", mcuListMap=" + mcuListMap + ", mcuList=" + mcuList
//				+ ", targetList=" + targetList + ", mbbTypeTargetList=" + mbbTypeTargetList + ", invalidTargetList=" + invalidTargetList);
//		
//		/*
//		 * 1. OTA Target 추출
//		 */
//		List<FirmwareIssueHistory> firmwareIssueHistoryList = firmwareIHDao.getRetryTargetList(issueDate);
...
//				String deviceId = "";
//				try {
//					deviceId = fih.getDeviceId();
//					/*
//					 * Clone OTA or DCU or DCU Kernel or DCU_COORDINATE OTA
//					 */
//					if(otaExecuteType == OTAExecuteType.CLONE_OTA 
//							|| otaTargetType == OTATargetType.DCU || otaTargetType == OTATargetType.DCU_KERNEL || otaTargetType == OTATargetType.DCU_COORDINATE){
//						if(mcuDao.get(deviceId) != null){
//							mcuList.add(deviceId);		
//						}else{
//							invalidTargetList.add(deviceId);
//							log.debug("UnRegistration DCU. Please check DCU List - " + deviceId);
//						}
//					}
//					
//					/*
...
//						Modem modem = meter.getModem();
//						MCU mcu = modem.getMcu();
//						log.debug("OTATargetType=["+otaTargetType+"], OTAExecuteType=["+otaExecuteType+"], Meter=[" + meter.getMdsId() + "], Modem=[" + modem.getDeviceSerial() + "], ModemType=" + modem.getModemType() + ", Protocol=" + modem.getProtocolType());
//						
//						if (modem.getModemType() == ModemType.MMIU && modem.getProtocolType() == Protocol.SMS) { // MBB Modem
//							Target target = CmdUtil.getNullBypassTarget(meter);
//							if(target == null){
//								invalidTargetList.add(deviceId);
//								log.debug("Meter=[" + meter.getMdsId() + "] This meter is invalid meter. please check Meter & Modem mapping");
//							}else{
//								mbbTypeTargetList.add(target);	
//								tempStr1.append(target.getMeterId());
//							}
//						} else if (modem.getModemType() == ModemType.MMIU && (modem.getProtocolType() == Protocol.IP || modem.getProtocolType() == Protocol.GPRS)) { // Ethernet Modem or gprs modem(ip mode)
//							Target target = CmdUtil.getNullBypassTarget(meter);
//							if(target == null){
//								invalidTargetList.add(deviceId);
//								log.debug("Meter=[" + meter.getMdsId() + "] This meter is invalid meter. please check Meter & Modem mapping");
...
//								&& (mcu != null && (otaExecuteType == OTAExecuteType.EACH_BY_DCU || otaExecuteType == OTAExecuteType.CLONE_OTA ))) { // DCU - RF Modem
//		            		if(mcuListMap.containsKey(mcu.getSysID())){
//		            			List<String> list = mcuListMap.get(mcu.getSysID());
//		            			list.add(meter.getMdsId());
//		            		}else{
...
//						Modem modem = modemDao.get(deviceId);
//						MCU mcu = modem.getMcu();
//						log.debug("OTATargetType=["+otaTargetType+"], OTAExecuteType=["+otaExecuteType+"], Modem=[" + modem.getDeviceSerial() + "], ModemType=" + modem.getModemType() + ", Protocol=" + modem.getProtocolType());
//						
//						if (modem.getModemType() == ModemType.MMIU && modem.getProtocolType() == Protocol.SMS) { // MBB Modem
//							Target target = CmdUtil.getNullBypassTarget(modem);
//							if(target == null){
//								invalidTargetList.add(deviceId);
//								log.debug("Modem=[" + modem.getDeviceSerial() + "] This modem is invalid meter. please check Meter & Modem mapping");
//							}else{
//								mbbTypeTargetList.add(target);					
//								tempStr1.append(target.getModemId());
//							}
//						} else if (modem.getModemType() == ModemType.MMIU && (modem.getProtocolType() == Protocol.IP || modem.getProtocolType() == Protocol.GPRS)) { // Ethernet Modem or gprs modem(ip mode)
//							Target target = CmdUtil.getNullBypassTarget(modem);
//							if(target == null){
...
//								targetList.add(target);							
//							}
//						} else if (modem.getModemType() == ModemType.SubGiga && modem.getProtocolType() == Protocol.IP && (mcu != null && (otaExecuteType == OTAExecuteType.EACH_BY_DCU || otaExecuteType == OTAExecuteType.CLONE_OTA ))) { // DCU - RF Modem
//		            		if(mcuListMap.containsKey(mcu.getSysID())){
//		            			List<String> list = mcuListMap.get(mcu.getSysID());
//		            			list.add(modem.getDeviceSerial());
//		            		}else{
...
//										
//				} catch (Exception e) {
//					log.error("[" + deviceId + "] This Device is Deleted in Target - " + e.getMessage(), e);
//				}
//			}
//			
//			
...
//			 * remove MBB Target
//			 */
//			for (Target mbbTarget : mbbTypeTargetList) {
//				if (targetList.contains(mbbTarget.getTargetId())) {
//					targetList.remove(mbbTarget.getTargetId());
//				}
//			}
//			
//			log.debug("MBB Target List = " + tempStr1.toString());
//			log.debug("Target List = " + targetList);
//		}else{
//			log.error("There is no OTA Target. Please check Group OTA History.");
//			throw new Exception("There is no OTA Target. Please check Group OTA History.");
//		}
//	}
	 ...
	 */
	private void mfOTAmakeTarget(String issueDate, OTATargetType otaTargetType, OTAExecuteType otaExecuteType
			, Map<String, List> mcuListMap, List<String> mcuList, List<Target> targetList, List<Target> mbbTypeTargetList, List<String> invalidTargetList) throws Exception{
		log.debug("issueDate=" + issueDate + ", OTATargetType=" + otaTargetType + ", OTAExecuteType=" + otaExecuteType + ", mcuListMap=" + mcuListMap + ", mcuList=" + mcuList
				+ ", targetList=" + targetList + ", mbbTypeTargetList=" + mbbTypeTargetList + ", invalidTargetList=" + invalidTargetList);
		...
		if (firmwareIssueHistoryList != null && 0 < firmwareIssueHistoryList.size()) {
					 ...
					 */
						...
						if (modem.getModemType() == ModemType.MMIU && modem.getProtocolType() == Protocol.SMS) { // MBB Modem
							...
							if(target == null){
			            		...
			            		if(mcuListMap.containsKey(mcu.getSysID())){
			            			List list = mcuListMap.get(mcu.getSysID());
			            			list.add(target);
			            		}else{
						...
						} else if (modem.getModemType() == ModemType.PLCIU && modem.getProtocolType() == Protocol.IP) { // PLC Modem
							Target target = CmdUtil.getNullBypassTarget(meter);
							if(target == null){
								invalidTargetList.add(deviceId);
								log.debug("Meter=[" + meter.getMdsId() + "] This meter is invalid meter. please check Meter & Modem mapping");
			 ...
			 */
			for (Target mbbTarget : mbbTypeTargetList) {
				if (targetList.contains(mbbTarget)) {
					targetList.remove(mbbTarget);
				}
			}
	...
	/**
	 *  Multi F/W OTA Execute - DCU, DCU_KERNEL, DCU_COORDINATE
	 *  
	 * @param result
	 */
//	private void mfOTAExecuteDCU(String issueDate, List<String> mcuList, OTATargetType otaTargetType, Map<String, Object> fwFileParams, Map<String, Object> result){
//		log.debug("mcuList= " + mcuList + ", OTATargetType=" + otaTargetType + ", fwFileParams=" + fwFileParams + ", result=" + result);
//		
//		try {
...
//				}
//				
//				SORIADcuOTARunnable runnable = new SORIADcuOTARunnable(this
//						, issueDate
//						, mcuSysId
//						, otaType 
//						, fwFileParams
...
//			}
//
//			RunnableMultiBatchExecutor rmbe = RunnableMultiBatchExecutor.getInstance("DCU_OTA");
//			log.debug("RunnableMultiBatchExcutor name   = " + rmbe.getName());
//			
//			boolean excuteResult = rmbe.execute(otaTargetType.name(), runnableList);
//			log.debug("RunnableMultiBatchExcutor result = " + excuteResult);
//			
//			if(excuteResult){
//				JSONObject jo = new JSONObject();
//				jo.put("RESULT", true);
//
//				log.info("TARGET Size=" + runnableList.size() + ", RESULT=" + true);
//				
//				result.put("OTA Excute Success", jo.toString());
//			}else{
//				result.put("OTA Excute Error", "DCU/DCU_KERNEL OTA Excute Fail");
//				log.error("DCU/DCU_KERNEL/DCU_COORDINATE OTABatch Error - DCU/DCU_KERNEL/DCU_COORDINATE OTA Excute Fail");
//			}
...
//	}
	private void mfOTAExecuteDCU(String issueDate, List<String> mcuList, OTATargetType otaTargetType, Map<String, Object> fwFileParams, Map<String, Object> result){
		...
		try {
			List<IBatchJob> jobList = new LinkedList<>();
			...
			for(String mcuSysId : mcuList){
				...
				IBatchJob runnable = new DCUOTABatchJob(mcuSysId, this
						, issueDate
						, mcuSysId
						, otaType 
						, fwFileParams
				...
				log.debug("[" + otaType + " by DCU OTA] DCU_ID = " + mcuSysId);
			}

			log.debug("### Batch Job List size = " + jobList.size());
			
			/*
			 * Execute OTA
			 */
			if(0 < jobList.size()){
				ExecutorManager em = ExecutorManager.getInstance("DCU_OTA_EM");
				log.debug("ExecutorManager name = " + em.getName());

				boolean excuteResult = em.execute(jobList);
				log.debug("ExecutorManager execution result = " + excuteResult);
				
				JSONObject jo = new JSONObject();
				if(excuteResult){
					jo.put("RESULT", true);
					log.info("TARGET Size=" + jobList.size() + ", RESULT=" + true);
					result.put("OTA Excute Success", jo.toString());
				}else{
					jo.put("RESULT", false);
					result.put("OTA Excute Error", otaTargetType.name() + " OTA Excute Fail");
					log.error(otaTargetType.name() + " OTABatch Error - " + otaTargetType.name() + " OTA Excute Fail");
				}	
			}
		} catch (Exception e) {
			result.put(otaTargetType.name() + " OTA Error", e.getMessage());
			log.error(otaTargetType.name() + " OTABatch Error -" + e, e);
		}
	}	
	
	
//	/**
//	 * Multi F/W OTA Execute - METER
//	 * @param result
//	 */
//	private void mfOTAExcuteMeter(String issueDate, OTATargetType otaTargetType, OTAExecuteType otaExecuteType, List<String> mcuList, Map<String, List<String>> mcuListMap, List<Target> targetList
//			, List<Target> mbbTypeTargetList, Map<String, Object> fwFileParams, boolean useAsyncChannel, Map<String, Object> result) {
//		log.debug("issueDate=" + issueDate + ", OTATargetType=" + otaTargetType + ", OTAExecuteType=" + otaExecuteType + ", mcuList=" + mcuList
//				+ ", mcuListMap=" + mcuListMap
//				+ ", targetList=" + targetList + ", mbbTypeTargetList=" + mbbTypeTargetList + ", fwFileParams=" + fwFileParams 
//				+ ", useAsyncChannel=" + useAsyncChannel + ", result=" + result);
//		
//		try {
//			List<IBatchRunnable> singleRunnableList = new LinkedList<>();
//			List<IBatchRunnable> multiRunnableList = new LinkedList<>();
//			
//			switch (otaExecuteType) {
////			case CLONE_OTA:  // RF만 해당
////				/*
////				 * Third-party Coordinator 방식만사용.
////				 * Third-party Modem 을 이용하는 방식이 필요할경우 추가 구현할것. 
////				 */
////				if(mcuListMap != null && 0 < mcuListMap.size()){
////					Iterator<String> it = mcuListMap.keySet().iterator();
////					while(it.hasNext()){
////						String mcuSysId = it.next();
////						
////						IBatchRunnable runnable = new SORIADcuOTARunnable(this
////								, issueDate
////								, mcuSysId
////								, OTAType.METER_RF_BY_THIRD_PARTY_COORDINATOR 
////								, fwFileParams
////								, mcuListMap.get(mcuSysId));
////						singleRunnableList.add(runnable);
////
////						log.debug("[METER OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId + ", ModemID=" + mcuListMap.get(mcuSysId).toString());
////					}
////				}
////				break;
//			case CLONE_OTA:  // RF만 해당. Coordinator가 OTA 진행
//				/*
//				 * Third-party Coordinator 방식만사용.
//				 * Third-party Modem 을 이용하는 방식이 필요할경우 추가 구현할것. 
//				 */
//				if(mcuList != null && 0 < mcuList.size()){
//					for(String mcuSysId : mcuList){
//						IBatchRunnable runnable = new SORIADcuOTARunnable(this
//								, issueDate
//								, mcuSysId
...
//						singleRunnableList.add(runnable);
//
//						log.debug("[METER OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId);
//					}
//				}
//				break;
//			case EACH_BY_DCU: // RF만 해당
//				if(mcuListMap != null && 0 < mcuListMap.size()){
//					Iterator<String> it = mcuListMap.keySet().iterator();
//					while(it.hasNext()){
//						String mcuSysId = it.next();
...
//						singleRunnableList.add(runnable);
//						
//						log.debug("[METER OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId + ", MeterId=" + mcuListMap.get(mcuSysId).toString());
//					}
//				}
//				break;
//			case EACH_BY_HES: // RF, MBB, Ethernet 모두 해당
//				/** RF Modem-Meter by HES OTA, Ethernet Modem-Meter OTA */
//				if(targetList != null && 0 < targetList.size()){
//					for(Target target : targetList){
//						IBatchRunnable runnable = new SORIAMeterOTARunnable(target, fwFileParams);
//						if(target.getReceiverType().equals("SubGiga")){  // RF 모뎀인경우
//							singleRunnableList.add(runnable);
//						}else{
//							multiRunnableList.add(runnable);							
//						}
//						
//						log.debug("[METER OTA] OTAExecuteType=" + otaExecuteType + ", MeterId=" + target.getMeterId());
//					}	
//				}
//				
//				/** MBB Modem-Meter OTA */
//				if(mbbTypeTargetList != null && 0 < mbbTypeTargetList.size()){
//					for(Target target : mbbTypeTargetList){
//						IBatchRunnable runnable = new SORIAMbbOTARunnable(this
//								, otaTargetType
//								, target
//								, fwFileParams
//								, useAsyncChannel);
//						multiRunnableList.add(runnable);
//						
//						log.debug("[METER-MBB OTA] OTAExecuteType=" + otaExecuteType + ", ModemID=" + target.getMeterId() + ", UseAsyncChannel=" + useAsyncChannel);
//					}	
//				}
//				break;
//			case EACH_BY_MODEM: // RF, MBB, Ethernet 모두 해당
//				/** RF Modem-Meter OTA by Modem, Ethernet Modem-Meter OTA by Modem */
//				if(targetList != null && 0 < targetList.size()){
//					for(Target target : targetList){
//						IBatchRunnable runnable = new SORIAModemOTARunnable(
//								  otaTargetType
//								, target
//								, fwFileParams);
//						if(target.getReceiverType().equals("SubGiga")){  // RF 모뎀인경우
...
//						}
//						
//						log.debug("[METER OTA by Modem] OTAExecuteType=" + otaExecuteType + ", ModemID=" + target.getModemId() + ", UseAsyncChannel=" + useAsyncChannel);
//					}						
//				}
//				
//				/** MBB Modem-Meter OTA by Modem */
//				if(mbbTypeTargetList != null && 0 < mbbTypeTargetList.size()){
//					for(Target target : mbbTypeTargetList){
//						IBatchRunnable runnable = new SORIAMbbOTARunnable(this
//								, otaTargetType
...
//						multiRunnableList.add(runnable);
//						
//						log.debug("[METER-MBB OTA by Modem] OTAExecuteType=" + otaExecuteType + ", ModemID=" + target.getModemId() + ", UseAsyncChannel=" + useAsyncChannel);
//					}	
//				}				
//				break;
//			default:
...
//			
//			
//			log.debug("### Multi OTA execute target size = " + multiRunnableList.size() + ", Single OTA execute target size = " + singleRunnableList.size());
//			
//			/*
//			 * Multi Execute OTA
//			 */
//			if(0 < multiRunnableList.size()){
//				RunnableMultiBatchExecutor rmbe = RunnableMultiBatchExecutor.getInstance("METER_OTA");
//				log.debug("RunnableMultiBatchExcutor name   = " + rmbe.getName());
//				
//				boolean excuteResult = rmbe.execute("METER_" + otaExecuteType.name(), multiRunnableList);
//				log.debug("RunnableMultiBatchExcutor result = " + excuteResult);
//				
//				if(excuteResult){
//					JSONObject jo = new JSONObject();
//					jo.put("RESULT", true);
//
//					log.info("TARGET Size=" + multiRunnableList.size() + ", RESULT=" + true);
//					
//					result.put("OTA Excute Success", jo.toString());
//				}else{
//					result.put("OTA Excute Error", "METER OTA Excute Fail");
//					log.error("METER OTABatch Error - METER OTA Excute Fail");
//				}	
//			}
//			
...
//			 * Single Execute OTA
//			 */
//			if(0 < singleRunnableList.size()){
//				RunnableSingleBatchExecutor rsbe = RunnableSingleBatchExecutor.getInstance("METER_OTA");
//				log.debug("RunnableBatchExcutor name   = " + rsbe.getName());
//				
//				boolean excuteResult = rsbe.execute("METER_" + otaExecuteType.name(), singleRunnableList);
//				log.debug("RunnableBatchExcutor result = " + excuteResult);
//				
//				if(excuteResult){
//					JSONObject jo = new JSONObject();
//					jo.put("RESULT", true);
//
//					log.info("TARGET Size=" + singleRunnableList.size() + ", RESULT=" + true);
//					
//					result.put("OTA Excute Success", jo.toString());
//				}else{
//					result.put("OTA Excute Error", "METER OTA Excute Fail");
...
//			}
//			
//			if(multiRunnableList.size() == 0 && singleRunnableList.size() == 0){
//				result.put("OTA Error", "Have no OTA Target.");	
//			}
//			
//
...
//	}	
	 ...
	 * @param result
	 */
	private void mfOTAExcuteMeter(String issueDate, OTATargetType otaTargetType, OTAExecuteType otaExecuteType, List<String> mcuList, Map<String, List> mcuListMap, List<Target> targetList
			, List<Target> mbbTypeTargetList, Map<String, Object> fwFileParams, boolean useAsyncChannel, Map<String, Object> result) {
		log.debug("issueDate=" + issueDate + ", OTATargetType=" + otaTargetType + ", OTAExecuteType=" + otaExecuteType + ", mcuList=" + mcuList
				+ ", mcuListMap=" + mcuListMap
				+ ", targetList=" + targetList + ", mbbTypeTargetList=" + mbbTypeTargetList + ", fwFileParams=" + fwFileParams 
		...
		try {
			...
			case CLONE_OTA:  // RF만 해당. Coordinator가 OTA 진행
				...
				if(mcuList != null && 0 < mcuList.size()){
					for(String mcuSysId : mcuList){
						...
						log.debug("[RF Modem-Meter by Clone OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId);
					}
				}
				break;
			case EACH_BY_DCU: // RF만 해당
				/** RF Modem-Meter by DCU OTA **/
				if(mcuListMap != null && 0 < mcuListMap.size()){
					Iterator<String> it = mcuListMap.keySet().iterator();
					while(it.hasNext()){
						String mcuSysId = it.next();
						...
						log.debug("[RF Modem-Meter by DCU OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId + ", MeterId=" + mcuListMap.get(mcuSysId).toString());
					}
				}
				break;
			case EACH_BY_HES: // RF, MBB, Ethernet 모두 해당
				/** RF Modem-Meter by HES OTA **/
				if(mcuListMap != null && 0 < mcuListMap.size()){
					Iterator<String> it = mcuListMap.keySet().iterator();
					while(it.hasNext()){
						String mcuSysId = it.next();
						
						List<Target> list = mcuListMap.get(mcuSysId);
						for(Target target : list) {
							jobList.add(new MeterOTABatchJob(mcuSysId, this, target, fwFileParams));
							log.debug("[RF Modem-Meter by HES OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId + ", MeterId=" + target.getMeterId() +", ModemId="+target.getModemId());
						}
					}
				}
				
				/** Ethernet Modem-Meter by HES OTA **/
				if(targetList != null && 0 < targetList.size()){
					for(Target target : targetList){
						jobList.add(new MeterOTABatchJob(target.getMeterId(), this, target, fwFileParams));
						log.debug("[Ethernet Modem-Meter by HES OTA] OTAExecuteType=" + otaExecuteType + ", MeterId=" + target.getMeterId() + ", ModemId="+target.getModemId());						
					}
				}
				
				/** MBB Modem-Meter by HES OTA **/
				if(mbbTypeTargetList != null && 0 < mbbTypeTargetList.size()){
					for(Target target : mbbTypeTargetList){
						IBatchJob runnable = new MBBOTABatchJob(target.getMeterId(), this
								, otaTargetType
								, target
								, fwFileParams
								, useAsyncChannel);
						...
						log.debug("[MBB Modem-Meter by HES OTA] OTAExecuteType=" + otaExecuteType + ", MeterId=" + target.getMeterId() + ", ModemID=" + target.getModemId() + ", UseAsyncChannel=" + useAsyncChannel);
					}	
				}
				break;
			case EACH_BY_MODEM: // RF, MBB, Ethernet 모두 해당
				/** RF Modem-Meter by Modem OTA **/
				if(mcuListMap != null && 0 < mcuListMap.size()){
					Iterator<String> it = mcuListMap.keySet().iterator();
					while(it.hasNext()){
						String mcuSysId = it.next();
						...
						List<Target> list = mcuListMap.get(mcuSysId);
						for(Target target : list) {
							jobList.add(new ModemOTABatchJob(mcuSysId, otaTargetType, target, fwFileParams));
							log.debug("[RF Modem-Meter by Modem OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId + ", MeterId=" + target.getMeterId() +", ModemId="+target.getModemId());
						}
					}
				}
				
				/** Ethernet Modem-Meter by Modem OTA **/
				if(targetList != null && 0 < targetList.size()){
					for(Target target : targetList){
						jobList.add(new ModemOTABatchJob(target.getMeterId(), otaTargetType, target, fwFileParams));
						log.debug("[Ethernet Modem-Meter by Modem OTA] OTAExecuteType=" + otaExecuteType + ", MeterId=" + target.getMeterId() + ", ModemId="+target.getModemId());
					}						
				}
				
				/** MBB Modem-Meter by Modem OTA **/
				if(mbbTypeTargetList != null && 0 < mbbTypeTargetList.size()){
					for(Target target : mbbTypeTargetList){
						IBatchJob runnable = new MBBOTABatchJob(target.getMeterId(), this
								, otaTargetType
						...
						log.debug("[MBB Modem-Meter by Modem OTA] OTAExecuteType=" + otaExecuteType + ", MeterId=" + target.getMeterId() + ", ModemID=" + target.getModemId() + ", UseAsyncChannel=" + useAsyncChannel);
					}	
				}				
				break;
			default:
			 ...
			 */
			if(0 < jobList.size()){
				ExecutorManager em = ExecutorManager.getInstance("METER_OTA_EM");
				log.debug("ExecutorManager name = " + em.getName());
...
//	/**
//	 * Multi F/W OTA Execute - MODEM
//	 * @param result
//	 */
//	private void mfOTAExcuteModem(String issueDate, OTATargetType otaTargetType, OTAExecuteType otaExecuteType, List<String> mcuList, Map<String, List<String>> mcuListMap, List<Target> targetList
//			, List<Target> mbbTypeTargetList, Map<String, Object> fwFileParams, boolean useAsyncChannel, Map<String, Object> result) {
//		log.debug("issueDate=" + issueDate + ", OTATargetType=" + otaTargetType + ", OTAExecuteType=" + otaExecuteType + ", mcuList=" + mcuList + ", mcuListMap=" + mcuListMap 
//				+ ", targetList=" + targetList + ", mbbTypeTargetList=" + mbbTypeTargetList + ", fwFileParams=" + fwFileParams 
//				+ ", useAsyncChannel=" + useAsyncChannel + ", result=" + result);
//		
//		try {
...
//			switch (otaExecuteType) {
//			
////			case CLONE_OTA:  // Third-party Modem 을 이용하는 방식이 필요할경우 추가 구현시 참고할것.
////				/*
////				 * Third-party Coordinator 방식만사용.
////				 * Third-party Modem 을 이용하는 방식이 필요할경우 추가 구현할것. 
////				 */
...
////						IBatchRunnable runnable = new SORIADcuOTARunnable(this
////								, mcuSysId
////								, OTAType.MODEM_RF_BY_THIRD_PARTY_COORDINATOR 
////								, fwFileParams
////								, mcuListMap.get(mcuSysId));
////						singleRunnableList.add(runnable);
////
////						log.debug("[MODEM OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId + ", ModemID=" + mcuListMap.get(mcuSysId).toString());
////					}
////				}
////				break;
//			case CLONE_OTA:  // RF만 해당. Coordinator가 OTA 진행
...
//						singleRunnableList.add(runnable);
//
//						log.debug("[MODEM OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId);
//					}
//				}
//				break;
//			case EACH_BY_DCU: // RF만 해당
...
//				break;
//			case EACH_BY_HES: // RF, MBB, Ethernet 모두 해당
//				/** RF Modem by HES OTA, Ethernet Modem OTA */
//				if(targetList != null && 0 < targetList.size()){
//					for(Target target : targetList){
//						IBatchRunnable runnable = new SORIAModemOTARunnable(otaTargetType, target, fwFileParams);
//						if(target.getReceiverType().equals("SubGiga")){  // RF 모뎀인경우
//							singleRunnableList.add(runnable);
//						}else{
//							multiRunnableList.add(runnable);							
//						}
//						
//						log.debug("[MODEM OTA] OTAExecuteType=" + otaExecuteType + ", ModemID=" + target.getModemId() + ", UseAsyncChannel=" + useAsyncChannel);
//					}						
//				}
//				
//				/** MBB Modem OTA */
...
//						multiRunnableList.add(runnable);
//						
//						log.debug("[MODEM-MBB OTA] OTAExecuteType=" + otaExecuteType + ", ModemID=" + target.getModemId() + ", UseAsyncChannel=" + useAsyncChannel);
//					}	
//				}
//				break;
//			default:
...
//			 */
//			if(0 < multiRunnableList.size()){
//				RunnableMultiBatchExecutor rmbe = RunnableMultiBatchExecutor.getInstance("MODEM_OTA");
//				log.debug("RunnableMultiBatchExcutor name   = " + rmbe.getName());
//				
//				boolean excuteResult = rmbe.execute("MODEM_" + otaExecuteType.name(), multiRunnableList);
//				log.debug("RunnableMultiBatchExcutor result = " + excuteResult);
//				
//				if(excuteResult){
//					JSONObject jo = new JSONObject();
...
//			
//			if(0 < singleRunnableList.size()){
//				RunnableSingleBatchExecutor rsbe = RunnableSingleBatchExecutor.getInstance("MODEM_OTA");
//				log.debug("RunnableSingleBatchExcutor name   = " + rsbe.getName());
//				
//				boolean excuteResult = rsbe.execute("MODEM_" + otaExecuteType.name(), singleRunnableList);
//				log.debug("RunnableSingleBatchExcutor result = " + excuteResult);
//				
//				if(excuteResult){
//					JSONObject jo = new JSONObject();
//					jo.put("RESULT", true);
...
//					result.put("OTA Excute Success", jo.toString());
//				}else{
//					result.put("OTA Excute Error", "MODEM OTA Excute Fail");
...
//	}
	 ...
	 * @param result
	 */
	private void mfOTAExcuteModem(String issueDate, OTATargetType otaTargetType, OTAExecuteType otaExecuteType, List<String> mcuList, Map<String, List> mcuListMap, List<Target> targetList
			, List<Target> mbbTypeTargetList, Map<String, Object> fwFileParams, boolean useAsyncChannel, Map<String, Object> result) {
		log.debug("issueDate=" + issueDate + ", OTATargetType=" + otaTargetType + ", OTAExecuteType=" + otaExecuteType + ", mcuList=" + mcuList + ", mcuListMap=" + mcuListMap 
				+ ", targetList=" + targetList + ", mbbTypeTargetList=" + mbbTypeTargetList + ", fwFileParams=" + fwFileParams 
				+ ", useAsyncChannel=" + useAsyncChannel + ", result=" + result);
		...
		try {
			...
			case CLONE_OTA:  // RF만 해당. Coordinator가 OTA 진행
				...
				if(mcuList != null && 0 < mcuList.size()){
					for(String mcuSysId : mcuList){
						...
						log.debug("[RF Modem by Clone OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId);
					}
				}
				break;
			case EACH_BY_DCU: // RF만 해당
				...
				if(mcuListMap != null && 0 < mcuListMap.size()){
					...
					while(it.hasNext()){
						...
						log.debug("[RF Modem by DCU OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId + ", ModemID=" + mcuListMap.get(mcuSysId).toString());
					}
				}
				break;
			case EACH_BY_HES: // RF, MBB, Ethernet 모두 해당
				...
				if(mcuListMap != null && 0 < mcuListMap.size()){
					...
					while(it.hasNext()){
						...
						for(Target target : list) {
							jobList.add(new ModemOTABatchJob(mcuSysId, otaTargetType, target, fwFileParams));
							log.debug("[RF Modem by HES OTA] OTAExecuteType=" + otaExecuteType + ", DCU_ID=" + mcuSysId + ", MeterId=" + target.getMeterId() +", ModemId="+target.getModemId());
						}
					}
				}
				
				/** Ethernet Modem by HES OTA **/
				if(targetList != null && 0 < targetList.size()){
					for(Target target : targetList){
						jobList.add(new ModemOTABatchJob(target.getModemId(), otaTargetType, target, fwFileParams));
						log.debug("[Ethernet Modem by HES OTA] OTAExecuteType=" + otaExecuteType + ", MeterId=" + target.getMeterId() + ", ModemId="+target.getModemId());						
					}
				}
				
				/** MBB Modem by HES OTA **/
				if(mbbTypeTargetList != null && 0 < mbbTypeTargetList.size()){
					for(Target target : mbbTypeTargetList){
						IBatchJob runnable = new MBBOTABatchJob(target.getModemId(), this
								, otaTargetType
								, target
								, fwFileParams
								, useAsyncChannel);
						...
						log.debug("[MBB Modem by HES OTA] OTAExecuteType=" + otaExecuteType + ", MeterId=" + target.getMeterId() + ", ModemID=" + target.getModemId() + ", UseAsyncChannel=" + useAsyncChannel);
					}	
				}
				break;
			default:
			 ...
			 */
			if(0 < jobList.size()){
				ExecutorManager em = ExecutorManager.getInstance("MODEM_OTA_EM");
				log.debug("ExecutorManager name = " + em.getName());
	...
	/**
	 * Multi Firmware OTA Improvement
	 * 
	 * @param otaTargetType
	 * @param firmwareId
	 * @param issueDate
	 * @param otaExecuteType
	 * @param useAsyncChannel
	 * @return
	 * @throws Exception
	 */
	public Map<String, Object> cmdMultiFirmwareOTAImprov(OTATargetType otaTargetType, String firmwareId, String issueDate, OTAExecuteType otaExecuteType, boolean useAsyncChannel) throws Exception {
		long startTime = System.currentTimeMillis();
		log.debug("Received params. OTATargetType=["+ otaTargetType + "]" + ", firmwareId=[" + firmwareId + "], issueDate=[" + issueDate + "], OTAExecuteType=[" + otaExecuteType + "], UseAsyncChannel=[" +useAsyncChannel+ "]");
		Map<String, Object> result = new HashMap<String, Object>();
		
		/*
		 * 1. Firmware 유무확인 및 준비
		 */
		Map<String, Object> fwFileParams;
		try {
			log.debug("### Make FW File ###");
			fwFileParams = mfOTAmakeFWFile(firmwareId, otaExecuteType);			
		} catch (Exception e) {
			log.error("MultFirmware OTA Make firmware file error - " + e.getMessage(), e);
			result.put("MultiFirmwareOTA. Make firmware file error", e.getMessage());
			return result;
		}
		...
		try {
			 ...
			 * 2. OTA Target 추출
			 */
			Map<String, List> mcuListMap = new HashMap<String, List>();
			List<Target> targetList = new ArrayList<Target>();
			List<String> mcuList = new ArrayList<String>();
	        List<String> invalidTargetList = new ArrayList<String>();
	        List<Target> mbbTypeTargetList = new ArrayList<Target>();

	        log.debug("### Make Target ###");
	        mfOTAmakeTarget(issueDate, otaTargetType, otaExecuteType, mcuListMap, mcuList, targetList, mbbTypeTargetList, invalidTargetList);
	        
			/*
			 * 3. OTA 진행
			 */
	        /** DCU / DCU Kernel / DCU Coordi  **/
	        if((mcuList != null && 0 < mcuList.size()) && (otaTargetType == OTATargetType.DCU || otaTargetType == OTATargetType.DCU_KERNEL || otaTargetType == OTATargetType.DCU_COORDINATE)){
	        	log.debug("### Execute DCU OTA ###");
	        	mfOTAExecuteDCU(issueDate, mcuList, otaTargetType, fwFileParams, result);
	        }
	        /** METER **/
	        else if(otaTargetType == OTATargetType.METER){
	        	log.debug("### Execute Meter OTA ###");
	        	mfOTAExcuteMeter(issueDate, otaTargetType, otaExecuteType, mcuList, mcuListMap, targetList, mbbTypeTargetList, fwFileParams, useAsyncChannel, result);
	        }
	        /** MODEM **/
	        else if(otaTargetType == OTATargetType.MODEM){
	        	log.debug("### Execute Modem OTA ###");
	        	mfOTAExcuteModem(issueDate, otaTargetType, otaExecuteType, mcuList, mcuListMap, targetList, mbbTypeTargetList, fwFileParams, useAsyncChannel, result);
	        }
	        else{
	        	throw new FMPException("Empty device");
	        }
			...
			for(String device : invalidTargetList){
				...
				log.debug("Invalid Target list Device=[" + device + "] => " + jo.toString());
				result.put(device, jo.toString());	
			}
			...
			result.put("OTA IssueDate = []" + issueDate, jo.toString());	
		}
		
		log.info("#### cmdMultiFirmwareOTAImprov process finished - Elapse Time : " + DateTimeUtil.getElapseTimeToString(System.currentTimeMillis() - startTime) + " ###");
		return result;
	}
	
	/**
	 * Executor Manager status check
	 * @param callName
	 * @return
	 * @throws Exception
	 */
	public String cmdExecutorManagerInfo(String callName) throws Exception{
		return ExecutorManager.getInstance(callName).toStringExecutorManagerInfo();
	}
	 ...
	 */
	@Override
	public String cmdForceUpload(String mcuId, String serverName, int dataCount
			, String fromDate, String toDate) throws FMPMcuException, Exception {
		log.debug("cmdForceUpload MCU[" + mcuId + "], ServerName[" + serverName + "]"
				+ ", DataCount[" + dataCount + "]"
				+ ", fromDate[" + fromDate + "]"
				+ ", ToDate[" + toDate + "]");
		...
		Vector<SMIValue> datas = new Vector<SMIValue>();
		if (target.getNameSpace() != null && target.getNameSpace().equals("SP")) {
			datas.add(DataUtil.getSMIValueByObject(target.getNameSpace(), "stringEntry", serverName));
			datas.add(DataUtil.getSMIValue(target.getNameSpace(), new INT(dataCount)));
			datas.add(DataUtil.getSMIValue(target.getNameSpace(), new INT(0x52)));// FILTER_DATA_AFTER_TIME
			datas.add(DataUtil.getSMIValueByObject(target.getNameSpace(), "stringEntry", fromDate.substring(0, 8)));
			datas.add(DataUtil.getSMIValue(target.getNameSpace(), new INT(0x53)));// FILTER_DATA_BEFORE_TIME
			datas.add(DataUtil.getSMIValueByObject(target.getNameSpace(), "stringEntry", toDate.substring(0, 8)));
		} else {
			log.error("Null or Unsupported Namespace");
			throw new Exception("cmdForceUpload supports only namespace SP.");
		}

		Object[] params = new Object[] { target, "cmdUploadMeteringDataForced", datas };
	 ...
	 */
	@SuppressWarnings("unchecked")
    static public String meterParamMapToJSON(Map map) {  // Add static SP-737
									...
									if(targetModemType != null) {
										if (targetModemType.equals(ModemType.SubGiga)) {
											...
											if (mmiuModem != null) {
												...
												//SP-991, no parent modem in the MBB modem
												//try {
	...
	public Map<String, Object> cmdMcuSetProperty(String mcuId, String[] key, String[] keyValue) throws FMPMcuException, Exception {
		...
		if(key.length == 0 || keyValue.length == 0) {
			...
			result.put("cmdResult", "FAIL(Argument has no data)");
			return result;
		}
	...
	public MeterData cmdGetMeteringData(String mcuId, String meterId, String modemId, String nOption, String fromDate,
		...
		try {
			...
			if ("SP".equals(target.getNameSpace()) || "PH".equals(target.getNameSpace())) {
	...
	@Override
	public MeterData cmdGetMeteringDataToNeighborDCU(String mcuId, String meterId, String modemId, String nOption, String fromDate,
			String toDate, String[] modemArray) throws FMPMcuException, Exception {
...
//				result = kaifa.getRelayStatus();
	...
	public Map<String, String> cmdModemMode(String modemId, String requestType, int mode ) throws Exception {
		...
		Modem modem = modemDao.get(Integer.parseInt(modemId));
		byte modemMode = DataUtil.getByteToInt(mode);
		...
		try {
			...
			if(requestType.equals("GET")){
				...
				//SP-1026 start
				if ( modemMode == ModemMode.Mode.Push.getCode()) {
					resultObj.setMode(ModemMode.Mode.Push);
				}
				else if ( modemMode == ModemMode.Mode.Polling.getCode()){
					resultObj.setMode(ModemMode.Mode.Polling);
				}
				//SP-1026 end
			}
		} catch (Exception e) {
...
//				stream.write(DataUtil.get2ByteToInt(offsetCount[1]));				// Poll Data - Count
    ...
    // INSERT END SP-476 
 	...
 	public void saveMeteringDataByQueue(String mcuId, String meterId, String modemId, byte[] data)  
 			throws Exception {
		try {
			boolean kafkaEnable = Boolean.parseBoolean(FMPProperty.getProperty("kafka.enable", "false"));// SP-1075,SP-882
			Meter meter = meterDao.get(meterId);
 			Modem modem = meter.getModem();	
 			DeviceModel model = meter.getModel();
			...
			if (kafkaEnable) {
				Message commLog = new Message();
				//commLog.setNameSpace(ns);
				commLog.setData(ba.toByteArray());
				commLog.setDataType(ProcessorHandler.SERVICE_MEASUREMENTDATA);
	...
	public Map<String, String> cmdExecDmdNiCommand(String modemId, String requestType, String attributeId, String param ) throws Exception {
		...
		List<String> resultList = new ArrayList<String>();
 		Target target = null;
 		// INSERT START SP-575
 		GeneralNiLog resultObj = new GeneralNiLog();
 		// INSERT END SP-575
...
//							jo.put("Value", Hex.decode(attrDataVal));
							resultObj.setBx(attrDataVal);
							jo.put("Value", resultObj.createResponseStr(attributeId, requestType));
							// UPDATE END SP-575
 	...
 	// INSERT START SP-575
	public Map<String, String> cmdGeneralNiCommand(String modemId, String requestType, String attributeId, String param ) throws Exception {
		log.debug("cmdGeneralNiCommand ["+modemId+"],["+requestType+"],["+ attributeId +"],[" + param +"]");
		Map map = new HashMap<String, String>();

		GeneralNiLog resultObj = null;
 		try {
 			Modem modem = modemDao.get(Integer.parseInt(modemId));
 			CommandNIProxy niProxy = new CommandNIProxy();
 			resultObj = (GeneralNiLog)niProxy.execute(modem, attributeId, param, requestType);
 		} catch (Exception e) {
 			log.error(e, e);
 		}
        ...
        String cmdResult = "";
		if(resultObj!=null){
			if(resultObj.getBx() == null){
				cmdResult = "Reponse of GeneralNiLog-Command is invalid";
			}else {
				cmdData = resultObj.createResponseStr(attributeId, requestType);
				cmdResult = "Execution OK";
			}
		}else{
			cmdResult = "Failed to sending a command in CommandNIProxy.";
		}
		JSONObject jo = new JSONObject();
		jo.put("AttributeId", attributeId);
		jo.put("Value", cmdData);
		map.put("AttributeData", jo.toString());
		map.put("cmdResult", cmdResult);
 ...
 //						modemDao.update(modem);	
	...
	public void cmdMcuExecuteSchedule(String mcuId, String scheduleName) throws FMPMcuException, Exception {
		...
		datas = new Vector<SMIValue>();
		datas.add(DataUtil.getSMIValueByObject(target.getNameSpace(), "stringEntry", scheduleName));
		Object[] params = new Object[] { target, "cmdForceSchedule", datas };
		String[] types = new String[] { TARGET_SRC, "java.lang.String", "java.util.Vector" };
		Object obj = null;
		try {
			obj = invoke(params, types);
	...
	/*
	 * Group Command - MCU Group Get Schedule
	 * SP-779
	 * */
	@Override
	public List<ScheduleData> cmdMcuGroupGetSchedule(Integer groupId) throws FMPMcuException, Exception {

		List<ScheduleData> result = new ArrayList<ScheduleData>();
		List<String> mcuList = new ArrayList<String>();

		mcuList = mcuDao.getGroupMcuList(groupId);
		int total = mcuList.size();

		try {
			if(mcuList.size()>0) {
				ScheduleGetRunner schGetRunner = new ScheduleGetRunner();
				result = schGetRunner.startRun(mcuList);
				log.debug("Runnable Execute Result => " + result);
}
	...
	/**
	 *  Group Command - MCU Group Set Schedule
	 *  SP-779
	 */

	@Override
	public List<McuScheduleResult> cmdMcuGroupSetSchedule(Integer groupId, String[][] args) throws FMPMcuException, Exception {
		log.debug("cmdMcuGroupSetSchedule(" + groupId + ") args.length(" + args.length + ")");

		List<McuScheduleResult> result = new ArrayList<McuScheduleResult>();
		List<String> mcuList = new ArrayList<String>();
		mcuList = mcuDao.getGroupMcuList(groupId);
		
		try {
			if(mcuList.size()>0) {
				ScheduleSetRunner schSetRunner = new ScheduleSetRunner();
				result = schSetRunner.startRun(mcuList, args);
				log.debug("Runnable Execute Result => " + result);
			}
		}catch (Exception e) {
			log.error(e, e);
	...
	/**
	 *  Group Command - MCU Group Set Schedule 결과값 가져오기 위해 만듦.
	 *  SP-779
	 */
	@Override
	public String cmdMcuGroupSetSchedule_(String mcuId, String[][] args) throws FMPMcuException, Exception {
		log.debug("cmdMcuSetSchedule(" + mcuId + ") args.length(" + args.length + ")");
		...
		if(mcu.getMcuVar()==null) {
			mcuVar = mcuVarDao.add(mcuVar);
			mcuVarId = mcuVar.getId();
		...
		String originReset = null;
		String newReset = null;
		String originUpload = null;
		...
		for (int i = 0; i < args.length; i++) {
			...
			if(args[i][0].toUpperCase().equals("RESET")) {
				newReset = args[i][2];
			}else if(args[i][0].toUpperCase().equals("UPLOAD")){
				newUpload = args[i][2];
			}
		...
		try {
			...
			if(mcuVarId != null) {
				mcuVar = mcuVarDao.get(mcuVarId);
				originReset = mcuVar.getVarAutoResetTime();
				originUpload = mcuVar.getVarUploadTime();
				
				if(originReset != null && originReset.equals(newReset)) {
					/* mcuVar Reset 값 유지 */
				}else {
					mcuVar.setVarAutoResetTime(newReset);
				}
				
				if(originUpload != null && originUpload.equals(newUpload)) {
					/* mcuVar Upload 값 유지 */
				}else {
					mcuVar.setVarUploadTime(newUpload);
				}
	...
	public List<McuPropertyResult> cmdMcuGroupSetProperty(Integer groupId, String[] key, String[] keyValue) throws FMPMcuException, Exception {
		log.debug("cmdMcuGroupSetProperty(" + groupId + ") args.length(" + key.length + ")");
		
		List<McuPropertyResult> result = new ArrayList<McuPropertyResult>();
		List<String> mcuList = new ArrayList<String>();
		...
		try {
			if(mcuList.size()>0) {
				PropertySetRunner proSetRunner = new PropertySetRunner();
				result = proSetRunner.startRun(mcuList, key, keyValue);
				log.debug("Runnable Execute Result => " + result);
			}
		}catch (Exception e) {
			log.error(e, e);
	...
	// INSERT START SP-802
	// Metering Data Request (Attribute ID 0xC204)
	@Override
	public Map<String, Object> cmdMeteringDataRequest(String mcuId, String[] modemIds, String fromDate, String toDate) throws FMPMcuException, Exception {
		Map<String,Object> resultMap = new HashMap<String,Object>();
		Map<String,Object> result = new HashMap<String,Object>();
		Target target = null;
 		Meter meter = null;
		...
		log.debug("cmdMeteringDataRequest(mcuId[" + mcuId + "]modems["+ modemsBuff.toString() + "] from[" +  fromDate + "] to[" + toDate + "]");

		// get first modem and check namespace, get interval
		Modem modem = modemDao.get(modemIds[0]);
		target = CmdUtil.getTarget(modem);		
		if (!"SP".equals(target.getNameSpace())) {
			log.error(target.getNameSpace() + " is not supported.");
        ...
        if (modem != null) {
			...
			throw new Exception("Modem[" + modemIds[0] + "] does not Exist!!");
		}					
		...
		if (modem.getModemType() == ModemType.MMIU && modem.getProtocolType() == Protocol.SMS) { // MBB Modem		
			...
			try {
				...
				for (int i=0; i < sendCount; i++ ) {
					...
					CommandNIProxy niProxy = new CommandNIProxy();
					resultObj = (MeteringDataRequest)niProxy.execute(modem, NIAttributeId.MeteringDataRequest, niParam);
...
//						log.debug("MeteringData[" + Hex.decode(metringData) + "]");
			...
			int responseTimeout = Integer.valueOf(FMPProperty.getProperty("protocol.dtls.response.timeout","180"));						
	 		...
	 		try {
	 			...
	 			SMIValue smiValue = null;
	 			ByteArrayOutputStream stream = null;
	 			String modems[] = new String[modemIds.length];
	
				int index = dateList.size() - 1;
	            for (int i=0; i < sendCount; i++ ) {
	            	log.debug("cmdExecDmdNiCommand [" + (i+1) + "/" + sendCount + "]");
		    		//Set Parameters
					stream = new ByteArrayOutputStream();
					stream.reset();
					stream.write(DataUtil.getByteToInt(1));								// Header Type  0x01
					stream.write(DataUtil.getByteToInt(1));								// Metering Data Type 0x01
	
					if (i+1 == sendCount) {
						stream.write(DataUtil.getByteToInt(index+1));					// Param Count					
						while (index >= 0) {
							String date = dateList.get(index);
//							System.out.print(date + "\n");
							stream.write(DataUtil.get2ByteToInt(Integer.parseInt(date.substring(0,4))));	// YYYY
							stream.write(DataUtil.getByteToInt(Integer.parseInt(date.substring(4,6))));		// MM
							stream.write(DataUtil.getByteToInt(Integer.parseInt(date.substring(6,8))));		// DD
							stream.write(DataUtil.getByteToInt(Integer.parseInt(date.substring(8,10))));	// hh
							stream.write(DataUtil.getByteToInt(Integer.parseInt(date.substring(10,12))));	// mm
							index--;
						}
					}
					else {
						stream.write(DataUtil.getByteToInt(24));						// Param Count	24	
						for (int j=0; j < 24; j++) {
							String date = dateList.get(index);
//							System.out.print(date + "\n");
							stream.write(DataUtil.get2ByteToInt(Integer.parseInt(date.substring(0,4))));	// YYYY
					...
					//System.out.print(Hex.decode(stream.toByteArray()));					
					result = cmdExecDmdNiCommandMulti("GET", "C204", Hex.decode(stream.toByteArray()),modemIds);					
					
					result.get("RESULT");
					result.get("cmdDesc");
					
					
			        for(String modemSerial : modemIds){

			        	log.info("Target :" + modemSerial + "###");
	
			        	meter = null;
 						Modem resultmodem = modemDao.get(modemSerial);
 						
 						if (resultmodem != null && !resultmodem.getMeter().isEmpty()) {
 							Set<Meter> meters = resultmodem.getMeter();
			        	...
			        	if(map.containsKey("AttributeData#1")){
					        ...
					        System.arraycopy(bx, 0, status, 0, status.length);
					        pos += status.length;						        

					        byte[] type = new byte[2];
					        System.arraycopy(bx, pos, type, 0, type.length);
					        pos += type.length;						        
					        
					        byte[] metringData = new byte[bx.length - pos];
						    System.arraycopy(bx, pos, metringData, 0, metringData.length);							
//							log.debug("MeteringData[" + Hex.decode(metringData) + "]");						    
	...
	// INSERT START SP-766
	@Override
	public Hashtable cmdGetCoordinatorConfigure(String mcuId) throws FMPMcuException, Exception {
		log.debug("cmdGetCoordinatorConfigure(" + mcuId + ")");

		Target target = CmdUtil.getTarget(mcuId);
		String command = "cmdGetCoordinatorConfigure";


		Object[] params = new Object[] { target, command, null };
	...
	@Override
	public Hashtable cmdSetCoordinatorConfigure(String mcuId, int configurations, int modemMode, 
			int resetTime, int metringInterval, int transmitFrequency, int cloneTerminate) 
			throws FMPMcuException, Exception {
		log.debug("cmdSetCoordinatorConfigure(" + mcuId + "," 
			+ configurations  + ","
			+ modemMode  + ","
			+ resetTime  + ","
			+ metringInterval  + ","
		...
		Target target = CmdUtil.getTarget(mcuId);
		String command = "cmdSetCoordinatorConfigure";

		Vector<SMIValue> datas = new Vector<SMIValue>();
		SMIValue smiValue = null;
		
		// Enable Configuration
		// Bit:0(LSB) Modem mode ON/OFF
		// Bit:1      Reset time ON/OFF
		// Bit:2      Metering interval ON/OFF
		// Bit:3      Transmit frequency ON/OFF
		// Bit:4      Clone terminate ON/OFF
		// Bit:5 - Bit7(MSB) Reserved		
		smiValue = DataUtil.getSMIValue(target.getNameSpace(), new BYTE(configurations));
		datas.add(smiValue);
		
		// Modem mode 0x00:Push, 0x01:Poll
		smiValue = DataUtil.getSMIValue(target.getNameSpace(), new BYTE(modemMode));
		datas.add(smiValue);

		// Reset time (sec)
		smiValue = DataUtil.getSMIValue(target.getNameSpace(), new BYTE(resetTime));
		datas.add(smiValue);
		
		// Metereing interval (sec)
		smiValue = DataUtil.getSMIValue(target.getNameSpace(), new WORD(DataUtil.get2ByteToInt(metringInterval))); 				
		datas.add(smiValue);
				
		// Transmit frequency  (sec)
		smiValue = DataUtil.getSMIValue(target.getNameSpace(), new WORD(DataUtil.get2ByteToInt(transmitFrequency))); 				
		datas.add(smiValue);
		
		// Clone terminate 1:stop clone
		smiValue = DataUtil.getSMIValue(target.getNameSpace(), new BYTE(cloneTerminate));
		datas.add(smiValue);		
	...
	// INSERT END SP-766	
	public Map<String, Object> cmdCloneOnOffViaSchedule(OTATargetType otaTargetType, OTAExecuteType otaExecuteType, String issueDate, boolean propagation, ModemCommandType modemCommandType, int cloningTime) throws Exception {
		long startTime = System.currentTimeMillis();
		String openTime = DateTimeUtil.getCurrentDateTimeByFormat(null);
		String elapseTime = "";
		
		log.info("Received params. OTATargetType=["+otaTargetType+"], OTAExecuteType=["+ otaExecuteType + "]" + ", issueDate=[" + issueDate + "], Propagation=[" + propagation + "], ModemCommandType=[" +modemCommandType+ "], CloningTime=["+cloningTime+"]");
		Map<String, Object> result = new HashMap<String, Object>();
		JSONObject resultMsgList = new JSONObject();
		
		DeviceType otaTargetDeviceType = null;
		TargetClass eventTargetClass = TargetClass.Unknown;
		String cloneCode = "";
		int cloneCount = 0;   // ModemCommandType.CLONE_OFF = 0
		
		if(otaTargetType == OTATargetType.MODEM) {
			eventTargetClass = TargetClass.SubGiga;
			...
			eventTargetClass = TargetClass.EnergyMeter;
		...
		if(modemCommandType == ModemCommandType.CLONE_ON) {
			cloneCount = cloningTime * 60 / 15;
			
			if(cloneCount < 20 || 96 < cloneCount) {
				log.error("Invalid Clone count range. please check clone count range. (0 or 20 ~ 96)");
				
				JSONObject jo = new JSONObject();
				jo.put("RESULT", false);
				jo.put("RESULT_VALUE", "Invalid Clone count range. please check clone count range. (0 or 20 ~ 96)");
				
				result.put("IssueDate = " + issueDate, jo.toString());
				return result;
			}
		}
		...
		// SP-1100
		String add3Day = "";
		int cloneOperatingTime = Integer.parseInt(FMPProperty.getProperty("ota.firmware.coordinator.clone.cloneOperatingTime", "3"));  // 최대 3일까지만 허용
		...
		byte[] cloneOperatingTimeData = DataUtil.getTimeStamp7Bytes(add3Day);
        byte[] crcData = CRCUtil.Calculate_ZigBee_Crc(cloneOperatingTimeData,(char)0x0000);
        DataUtil.convertEndian(crcData);
        log.info("[CloneParams] cloneCode=" + cloneCode +", cloneCount=" + cloneCount + ", yyyymmddhh="+openTime+", cloneOperatingTime="+add3Day+", Hex="+Hex.decode(cloneOperatingTimeData)+", size="+cloneOperatingTimeData.length+", crc="+Hex.decode(crcData));
        
		try {
			List<FirmwareIssueHistory> firmwareIssueHistoryList = firmwareIHDao.getRetryTargetList(issueDate);
			if (firmwareIssueHistoryList != null && 0 < firmwareIssueHistoryList.size()) {
				Map<String, String> entryResult = new HashMap<String, String>();
				
				String cmdResultType = "fail";
				String eventMsg = "";
				OTA_UPGRADE_RESULT_CODE resultCode = OTA_UPGRADE_RESULT_CODE.OTAERR_EXECUTE_FAIL;
				
				for(FirmwareIssueHistory target : firmwareIssueHistoryList) {
					...
					try {
						...
						if(otaTargetType == OTATargetType.METER) {
							Meter meter = meterDao.get(target.getDeviceId());
							if(meter == null) {
								throw new Exception("Can not found meter - " + target.getDeviceId());										
							}
							modem = meter.getModem();
							if(modem == null) {
								throw new Exception("Can not found modem - " +  target.getDeviceId());
							}
							modemSerial = modem.getDeviceSerial();
							location = meter.getLocation();
						}else {
							modem = modemDao.get(target.getDeviceId());
							modemSerial = target.getDeviceId();
							location = modem.getLocation();
						}
						
						// 이벤트 저장.
						// Event	            code	    Step
						// evtOTADownloadStart	200.64.0	Started writing FW
						EV_SP_200_64_0_Action action = new EV_SP_200_64_0_Action();
						action.makeEvent(eventTargetClass, target.getDeviceId(), eventTargetClass, openTime, otaExecuteType.name(), location);
						action.updateOTAHistory(target.getDeviceId(), otaTargetDeviceType, openTime);
						
						/*
						 * SP-1100
						 * DCU 코디 1.4 이상, RF모뎀 1.3 이상인 모뎀은 Clone operation time, CRC 도 전송.
						 */
						dcu = mcuDao.get(modem.getMcuId());
						float modemVersion = Float.parseFloat(modem.getFwVer());
						float dcuCodiVersion = Float.parseFloat(dcu.getMcuCodi().getCodiFwVer());
						switch (otaExecuteType) {
						case EACH_BY_HES:
							HashMap<String, Object> niParams = new HashMap<>();
							if(1.4 <= dcuCodiVersion && 1.3 <= modemVersion) {
								niParams.put("code", cloneCode);
								niParams.put("count", cloneCount);
								niParams.put("cloneOperatingTime", Hex.decode(cloneOperatingTimeData));
								niParams.put("crc", Hex.decode(crcData));
							}else {
								niParams.put("code", cloneCode);
								niParams.put("count", cloneCount);
							}
							
							log.info("[EACH_BY_HES] DeviceId="+target.getDeviceId()+", Target MODEM_ID="+modemSerial+", DCU CodiFWVer="+dcuCodiVersion+", ModemFWVer="+modemVersion+", Param=" + StringUtil.objectToJsonString(niParams));
							entryResult = executeClonOnOffByHES(modemSerial, niParams);
							
//							Map temp = new HashMap<String, Object>();
//							temp.put("[EACH_BY_HES]TestKey", "ok");
//							entryResult = temp;
							
							
							if(entryResult.containsKey("cmdResultType")) {
								cmdResultType = entryResult.get("cmdResultType");
							}
							
							if(entryResult.containsKey("cmdResult")) {
								eventMsg = entryResult.get("cmdResult");
							}
							
							break;
						case EACH_BY_DCU:
							byte[] commandParams = null;
							if(1.4 <= dcuCodiVersion && 1.3 <= modemVersion) {
								commandParams = DataUtil.append(cloneCodeData, cloneCountData);
								commandParams = DataUtil.append(commandParams, cloneOperatingTimeData);
								commandParams = DataUtil.append(commandParams, crcData);
							}else {
								commandParams = DataUtil.append(cloneCodeData, cloneCountData);
							}
							
							log.info("[EACH_BY_DCU] DeviceId="+target.getDeviceId()+", Target MODEM_ID="+modemSerial+", DCU CodiFWVer="+dcuCodiVersion+", ModemFWVer="+modemVersion+", Param=" + Hex.decode(commandParams));
							entryResult = executeClonOnOffByDCU(modemSerial, commandParams);
							
//							Map temp1 = new HashMap<String, Object>();
//							temp1.put("[EACH_BY_DCU]TestKey", "ok");
//							entryResult = temp1;

							for (java.util.Map.Entry<String, ?> e : entryResult.entrySet()) {
								log.info("[MODEM_ID:"+modemSerial+", REQUEST_TYPE:"+NIAttributeId.CloneOnOff.name()+", KEY["+e.getKey()+"], VALUE["+e.getValue()+"]");
							}
							...
							if(entryResult.containsKey("cmdResult")) {
								eventMsg = (String) entryResult.get("cmdResult");
							}
							break;
						default:
							throw new Exception("Unknown execute type. please check execute type.");
						}
						
						log.debug("cmdResultType=" + cmdResultType + ", eventMsg=" + eventMsg);
						
						elapseTime = DateTimeUtil.getElapseTimeToString(System.currentTimeMillis() - targetStartTime);
						eventMsg = "Msg=" + eventMsg + ", ElapseTime = [" + elapseTime + "]";
						if(cmdResultType.equals("success")) {
							resultCode = OTA_UPGRADE_RESULT_CODE.OTAERR_NOERROR;
						}else {
							resultCode = (modemCommandType == ModemCommandType.CLONE_ON ? OTA_UPGRADE_RESULT_CODE.OTAERR_CLONE_ON_FAIL : OTA_UPGRADE_RESULT_CODE.OTAERR_CLONE_OFF_FAIL);
						}
						
						
						// 이벤트 저장.
						// Event	            code	    Step
						// evtOTADownloadResult	200.66.0	OTA Result
						EV_SP_200_66_0_Action action2 = new EV_SP_200_66_0_Action();
						openTime = DateTimeUtil.getCurrentDateTimeByFormat("yyyyMMddHHmmss");
						action2.makeEvent(eventTargetClass, target.getDeviceId(), eventTargetClass, openTime
								, resultCode, eventMsg, otaExecuteType.name(), location);
						action2.updateOTAHistory(target.getDeviceId(), otaTargetDeviceType, openTime, resultCode, eventMsg);
						
						
						JSONObject jo = new JSONObject();
						jo.put("RESULT", cmdResultType);
						jo.put("RESULT_VALUE", eventMsg);
						
						resultMsgList.put("TargetId=" + target.getDeviceId(), jo);
					} catch (Exception e) {
						eventMsg = "Execute command error - " + e.getMessage();
						log.error(eventMsg, e);
						...
						openTime = DateTimeUtil.getCurrentDateTimeByFormat("yyyyMMddHHmmss");
						action3.makeEvent(eventTargetClass, target.getDeviceId(), eventTargetClass, openTime
								, (modemCommandType == ModemCommandType.CLONE_ON ? OTA_UPGRADE_RESULT_CODE.OTAERR_CLONE_ON_FAIL : OTA_UPGRADE_RESULT_CODE.OTAERR_CLONE_OFF_FAIL)
								, eventMsg, otaExecuteType.name(), location);
						action3.updateOTAHistory(target.getDeviceId(), otaTargetDeviceType, openTime
								, (modemCommandType == ModemCommandType.CLONE_ON ? OTA_UPGRADE_RESULT_CODE.OTAERR_CLONE_ON_FAIL : OTA_UPGRADE_RESULT_CODE.OTAERR_CLONE_OFF_FAIL)
								, eventMsg);
			...
			log.error("Can't execute cmdCloneOnOffViaSchedule command - " + e.getMessage(), e);
			
			JSONObject jo = new JSONObject();
			jo.put("RESULT", false);
			jo.put("RESULT_VALUE", "Can't execute cmdCloneOnOffViaSchedule command - " + e.getMessage());
			
			resultMsgList = jo;
		}

		
		elapseTime = DateTimeUtil.getElapseTimeToString(System.currentTimeMillis() - startTime);		
		log.info("#### cmdCloneOnOffViaSchedule process finished - Elapse Time : " + elapseTime + " ###");

		resultMsgList.put("Elapse Time", elapseTime);
		result.put("IssueDate = " + issueDate, resultMsgList.toString());
		
		return result;
	}
	
	
	public Map<String, String> 	executeClonOnOffByHES(String deviceId, HashMap<String, Object> niParams) throws Exception {
		log.debug("## executeClonOnOffByHES [" + deviceId + "], [" + StringUtil.objectToJsonString(niParams) +"]");
		...
		try {
			...
			if(niParams == null || niParams.size() == 0 
					|| niParams.get("cloneCode") == null || niParams.get("cloneCode").equals("")					
					|| niParams.get("count") == null || niParams.get("count").equals("")) {
				throw new Exception("Invalid parameter");
			}
			
			CommandNIProxy niProxy = new CommandNIProxy();
			
			resultObj = (CloneOnOff)niProxy.execute(modem, NIAttributeId.CloneOnOff, niParams);
			
			map.put("cmdResultType", "success");
			map.put("cmdResult", "Execution OK - " + resultObj.toString2());
		} catch (Exception e) {
			map.put("cmdResultType", "fail");
			map.put("cmdResult", "Execution Fail - " + e.getMessage());
	...
	public Map<String, String> executeClonOnOffByDCU(String modemDeviceSerial, byte[] param ){
		log.debug("executeClonOnOffByDCU ["+modemDeviceSerial+"],[" + Hex.decode(param) +"]");
		...
		String resultString = "";
 		...
 		try {
 			...
 			if(modem == null) {
 				throw new Exception("Can't find modem - " + modemDeviceSerial);
 			}
			...
			Vector<SMIValue> datas = new Vector<SMIValue>();
			datas.add(DataUtil.getSMIValue(target.getNameSpace(), new UINT(responseTimeout))); 			// timeout
			datas.add(DataUtil.getSMIValue(target.getNameSpace(), new BYTE(0x01)));  			// Attribute count
			datas.add(DataUtil.getSMIValue(target.getNameSpace(), new WORD(NIAttributeId.CloneOnOff.getCode())));	// Attribute ID
 			datas.add(DataUtil.getSMIValue(target.getNameSpace(), new STREAM(param)));  // Attribute params
			datas.add(DataUtil.getSMIValue(target.getNameSpace(), new INT(0x02)));  // FILTER_MODEMID
			datas.add(DataUtil.getSMIValueByObject(target.getNameSpace(), "stringEntry", modem.getDeviceSerial()));  // modemID

			Object[] params = new Object[] { target, "cmdDmdNiSet", datas };
 			String[] types = new String[] { TARGET_SRC, "java.lang.String", "java.util.Vector", };

 			log.debug("#### ====> [executeClonOnOffByDCU] invoke [" + modemDeviceSerial + "]");
 			Object obj = invoke(params, types);
 			log.debug("#### <==== [executeClonOnOffByDCU] invoke [" + modemDeviceSerial + "]");
				...
				if ((j==0) && (!smiValue.getVariable().toString().equals(modem.getDeviceSerial()))) {
					...
					if (res == null) {
						...
						for(int n=0; n < attrCount; n++) {
							...
							resultObj.setBx(attrDataVal);
							jo.put("Value", resultObj.createResponseStr(Hex.decode(NIAttributeId.CloneOnOff.getCode()), "SET"));
			...
			map.put("cmdResultType", "success");						
 			...
 			resultString="Execution failed -" + e.getMessage();
 			map.put("cmdResultType", "fail");
 		}
	...
	/**
     * OPF-982 Telit Module Upgrade 
     * @param targetModem	대상모뎀
     * @param ftpUrl		FTP Server URL
     * @param ftpPort		FTP Server Port
     * @param ftpDirectory	이미지파일 Directory
     * @param targetFile	이미지파일 이름
     * @param username		FTP 계정
     * @param password		FTP 계정 비밀번호
	 */
	@Override
	public Map<String, Object> cmdFOTA(
			MMIU targetModem,
			String ftpUrl,
			String ftpPort,
			String ftpDirectory,
		...
		log.debug("## cmdFOTA [" + targetModem + "] " + 
				...
				"username [" + username + "] " + 
				"password [" + password + "] ");
		InitiateModuleUpgrade result = null;
		...
		niParam.put("ftpUrl", ftpUrl);
		niParam.put("ftpPort", ftpPort);
		niParam.put("ftpDirectory", ftpDirectory);
		niParam.put("targetFile", targetFile);
		niParam.put("username", username);
		niParam.put("password", password);
		
		// Target , Command, Parameter
		result = (InitiateModuleUpgrade)commandNIProxy.execute(
				targetModem,
				NIAttributeId.InitiateModuleUpgrade,
				niParam
			);

		map.put("ftpUrl", result.getFtpUrl());
		map.put("ftpPort", result.getFtpPort());
		map.put("ftpDirectory", result.getFtpDirectory());
		map.put("targetFile", result.getTargetFile());
	...
	@Override
	public List<JSONObject> cmdGetSensorPath(String mcuId, String parserName) throws FMPMcuException, Exception {
