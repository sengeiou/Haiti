...
public class SendMeterEventLogWSTaskletOld implements Tasklet {

    private static Log log = LogFactory.getLog(SendMeterEventLogWSTaskletOld.class);

    @Autowired
    protected JdbcTemplate jdbcTemplate;

    @Autowired
    protected com.oracle.xmlns.ssys.nurieventsproxy.nurieventsproxy.ExecutePtt webServiceClient;
    ...
    private void sendData() throws Exception {
        ...
        String sql = "select ACTIVATOR_ID,METEREVENT_ID,EVENTOBIS_ID,OPEN_TIME,ACTIVATOR_TYPE,INTEGRATED,MESSAGE,SUPPLIER_ID,LOCATION_NAME,WRITETIME,YYYYMMDD from ip_ev_outbound where batch_id=?";
        ...
        for (Map<String, Object> batch_id : batch_ids) {
            ...
            XMLGregorianCalendar receivedDate = (XMLGregorianCalendar) sendWSMessage(message);
            log.debug("execute.result=" + receivedDate);

            jdbcTemplate.update(
                    "update ip_ev_batches set batch_status=3,delivered_date=current_date  where batch_id=?",
                    new Object[] { batch_id_val });

        }
    }

    private Object sendWSMessage(EndDeviceEventsEventMessageType message) throws Exception {
