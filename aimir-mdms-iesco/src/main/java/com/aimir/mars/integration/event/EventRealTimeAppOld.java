...
import com.oracle.xmlns.ssys.nurieventsproxy.nurieventsproxy.ExecutePtt;
...
public class EventRealTimeAppOld implements MessageListener {

    private static Log log = LogFactory.getLog(EventRealTimeAppOld.class);

    @Autowired
    protected ExecutePtt webServiceClient;
    @Autowired
    ...
    private boolean checkDeliveryAvailable(String CIMID) {
        ...
        List<Map<String, Object>> ipEvOption = jdbcTemplate
                .queryForList(
                        ...
                        "union all \n" +
                        "select attributename, attributevalue from ip_ev_option where codetype=? and attributename=? ", "OP", "CNF_DELIVERY", "EV", CIMID);
        ...
        return isDeliverData && isDeliverCIMData;
    }

    private void sendWSMessage(EndDeviceEventsEventMessageType message) {
        if(checkDeliveryAvailable(message.getPayload().getEndDeviceEvents().getEndDeviceEvent().get(0).getMRID())) {
            ...
            XMLGregorianCalendar timestamp =  webServiceClient.execute(message);
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 1) {
            log.info("Usage:");
            log.info(
                    "App  -configFile config/spring-ev-integration-realtime-old.xml");
            return;
        }
