...
public class MeterRetireNotificationService {
    ...
    public ArrayOfErrorObject execute(Meters removedMeters) throws Exception {
        ...
        Code deactiveStatus = codeDao.getCodeIdByCodeObject("1.3.3.11");
        ...
        if (removedMeters.getElectricMeters() != null && !removedMeters
            ...
            for (ElectricMeter cd : target) {
                ...
                if (isInvalidParameter == false) {
                    ...
                    if (removeMeter == null) {
                        obj = allocateAlreadyDeactivated(eventTime);
                    } else {
                        if (meterLocation != null
                                ...
                                && (removeMeter.getMeterStatus() != null
                                        && !removeMeter.getMeterStatus().getId()
                                                .equals(deactiveStatus
                                                        .getId()))) {
                            if (removeMeter.getMeterStatus() != null
                                    && !removeMeter.getMeterStatus().getId()
                                    .equals(deleteStatus.getId())) {
                                removeMeter.setDeleteDate(strRemovedDate);
                                try {
                                    retireMeter(removeMeter, deactiveStatus);
                                } catch (Exception e) {
                                    obj.setErrorString(
    ...
    private void retireMeter(Meter removeMeter,Code deactivationMeterStatus) throws Exception {
        removeMeter.setDeleteDate(
                DateTimeUtil.getCurrentDateTimeByFormat("yyyyMMddHHmmss"));
        removeMeter.setMeterStatus(deactivationMeterStatus);
        meterDao.update(removeMeter);
    }

    private ErrorObject allocateAlreadyDeactivated(
            XMLGregorianCalendar eventTime) {
        ErrorObject obj = new ErrorObject();
        obj.setErrorString(ValidationError.ALREADY_DEACTIVATED.getName());// already deleted meter
        obj.setEventTime(eventTime);
        obj.setNounType("");
        return obj;
    }
