...
public class SendMeterValueWSTasklet implements Tasklet {

    private static Log log = LogFactory.getLog(SendMeterValueWSTasklet.class);
    ...
    public RepeatStatus execute(StepContribution arg0, ChunkContext arg1)
        ...
        for (Map<String, Object> row : ipEvOption) {
            ...
            } else if(row.get("ATTRIBUTENAME").toString().toUpperCase().equals("CNF_READ_MAXBATCHCOUNT")) {
                batchcount = Integer.parseInt(row.get("ATTRIBUTEVALUE").toString());
            }
        }
    ...
    private void sendData(String tablename) throws Exception {
        List<Map<String, Object>> batch_ids = jdbcTemplate.queryForList(
                "select * from (select batch_id from ip_mv_batches where target_table=upper('" + tablename
                        + "') and batch_status=2 order by batch_id) where rownum<=" + batchcount );
        ...
        for (Map<String, Object> batch_id : batch_ids) {
            ...
            try {
                ...
                if(receivedDate != null) {
                    ...
                    jdbcTemplate.update(
                            "update ip_mv_batches set batch_status=?,delivered_date=? where batch_id=?",
