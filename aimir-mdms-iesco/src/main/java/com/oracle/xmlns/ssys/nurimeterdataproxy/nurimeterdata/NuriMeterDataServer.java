...
public class NuriMeterDataServer implements NuriMeterData {

    private static Log log = LogFactory.getLog(NuriMeterDataServer.class);
    ...
    @Override
    @Transactional
    public XMLGregorianCalendar nuriMeterDataBulk(MeterReadingsType part1) {
        log.debug(part1);
    ...
    public static void main(String[] args) {
        try {
            ...
            if (args.length < 3) {
                log.info("Usage:");
                log.info(
                        "ExecutePttServer -port CommunicationPort -config /config/spring-mv-integration-ws-test.xml");
                return;
            }
