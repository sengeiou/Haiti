...
@WSDLDocumentation("Electricity Energy Consumption Monitoring Service")
@WebService(name="ConsumptionMonitoringEmService", targetNamespace="http://aimir.com/services")
@SOAPBinding(style=Style.DOCUMENT, use=Use.LITERAL, parameterStyle=ParameterStyle.WRAPPED)
public interface ConsumptionMonitoringEmManager {
	...
	@WSDLDocumentationCollection(
		    {
		        ...
		        @WSDLDocumentation(value = "Return Map \r\n"
								...
								+ " ($key=grid, $valueType=List<Map<String, Object>>, @description=Energy Consumption Power of Location / Sum of carbon.) \r\n"
								+ " Description of 'grid' \r\n"
								...
								+ " ($key=CHANNEL, $valueType=Integer, @description= Energy Consumption Channel Number default 1 means usage) \r\n"
		...
		@WebMethod
		@WebResult(name="BuildingLookUpEmMap")	
	...
	public Map<String, Object> getBuildingLookUpMaxEm(Map<String, Object> condition);
	...
	@WSDLDocumentationCollection(
			{
			        ...
			        @WSDLDocumentation(value = "Return Map \r\n"
									...
									+ " ($key=EMMIN, $valueType=Double, @description=Energy Consumption Min) \r\n"
									+ " ($key=EMMAX, $valueType=Double, @description=Energy Consumption Max) \r\n"
									+ " ($key=EMSUM, $valueType=Double, @description=Energy Consumption Summation) \r\n"
			...
			@WebMethod
			@WebResult(name="BuildingLookUpMaxEmMap")
	...
	@WSDLDocumentationCollection(
			{
			        ...
			        @WSDLDocumentation(value = "Return Map \r\n"
									...
									+ " ($key=averageUsage, $valueType=Double, @description=Average Usage)v($key=averageCo2Usage, $valueType=Double, @description=Average co2 emission)",
		            					placement = WSDLDocumentation.Placement.BINDING_OPERATION_OUTPUT)
			}	)
			@WebMethod
    ...
    @WSDLDocumentationCollection(
            {@WSDLDocumentation(value="Returns the depth of the current location", 
            		placement=WSDLDocumentation.Placement.BINDING_OPERATION),
            @WSDLDocumentation(value="LocationLevel", 
            		placement=WSDLDocumentation.Placement.BINDING_OPERATION_OUTPUT)}
            )
	@WebMethod
	@WebResult(name="LocationLevel")
	public int thisLocationLevel(
