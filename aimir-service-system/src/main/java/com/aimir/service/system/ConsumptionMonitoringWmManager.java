...
@WSDLDocumentation("Water Energy Consumption Monitoring Service")
@WebService(name="ConsumptionMonitoringWmService", targetNamespace="http://aimir.com/services")
@SOAPBinding(style=Style.DOCUMENT, use=Use.LITERAL, parameterStyle=ParameterStyle.WRAPPED)
public interface ConsumptionMonitoringWmManager {
	...
	@WSDLDocumentationCollection(
			...
			@WebMethod
			@WebResult(name="BuildingLookUpWmMap")
	public Map<String, Object> getBuildingLookUpWmByParam(
			...
			@WebParam(name="detailLocationId") Integer detailLocationId
		);
	
	public Map<String, Object> getBuildingLookUpMaxWm(Map<String, Object> condition);
	...
	@WSDLDocumentationCollection(
			...
			@WebMethod
			@WebResult(name="BuildingLookUpMaxWmMap")
	...
	@WSDLDocumentationCollection(
			{
			        @WSDLDocumentation(value = "supplierId and cycle time, using information based on current usage and carbon emissions in the period return.",	
										placement = WSDLDocumentation.Placement.BINDING_OPERATION),
			        @WSDLDocumentation(value = "Parameter Map \r\n"
									+ " ($key=searchDateType, $valueType=String, @description=HOURLY('0'), DAILY('1'),PERIOD('2'),WEEKLY('3'), MONTHLY('4'), MONTHLYPERIOD('5'),WEEKDAILY('6'),SEASONAL('7'),YEARLY('8'),QUARTERLY('9')) \r\n"
									+ " ($key=supplierId, $valueType=String, @description=Supplier.id) \r\n"
