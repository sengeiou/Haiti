...
count=`/bin/ps -eaf | grep pom-throughput.xml | grep antrun:run | grep -v grep | wc -l`
if [ $count -gt 0 ]; then
    echo "process is running."
    exit 1
fi

mvn -e -f $AIMIR_TASK/pom-throughput.xml antrun:run -DtaskName=Throughput 2>&1 > /dev/null &
