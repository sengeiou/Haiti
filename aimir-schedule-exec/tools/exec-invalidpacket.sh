...
count=`/bin/ps -eaf | grep pom-invalidpacket.xml | grep antrun:run | grep -v grep | wc -l`
if [ $count -gt 0 ]; then
    echo "process is running."
    exit 1
fi

mvn -e -f $AIMIR_TASK/pom-invalidpacket.xml antrun:run -DtaskName=InvalidPacket 2>&1 > /dev/null &
