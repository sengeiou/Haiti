...
public class TariffWMDaoImpl extends AbstractJpaDao<TariffWM, Integer> implements TariffWMDao {
	...
	Log log = LogFactory.getLog(TariffWMDaoImpl.class);
	...
	public List<Map<String, Object>> getChargeMgmtList(Map<String, Object> condition){
		...
		Query query = em.createQuery(sb.toString(), TariffWM.class);
        ...
        List<TariffWM> result = query.getResultList();
        try {
            for (TariffWM t : result) {
	...
	public int updateData(TariffWM tariffWM) throws Exception {
		...
		sb.append("SET t.usageUnitPrice = :usageUnitPrice ");
		sb.append("   ,t.shareCost = :shareCost ");
		sb.append("   ,t.supplySizeMin = :supplySizeMin ");
		sb.append("   ,t.supplySizeMax = :supplySizeMax ");
		sb.append("   ,t.condition1 = :condition1 ");
		sb.append("   ,t.condition2 = :condition2 ");
		sb.append("WHERE t.id = :id ");

		Query query = em.createQuery(sb.toString());
		query.setParameter("usageUnitPrice", tariffWM.getUsageUnitPrice());
		query.setParameter("shareCost", tariffWM.getShareCost());
		query.setParameter("supplySizeMin", tariffWM.getSupplySizeMin());
		query.setParameter("supplySizeMax", tariffWM.getSupplySizeMax());
		query.setParameter("condition1", tariffWM.getCondition1());
		query.setParameter("condition2", tariffWM.getCondition2());
...
//	    			monthWM =monthWMDao.getMonthWM(tariffParam);
	...
	public List<TariffWM> getApplyedTariff(Map<String, Object> params){
		...
		Query query = em.createQuery(sb.toString(), TariffWM.class);
		query.setParameter("tariffTypeCode", tariffTypeCode);
...
//  선불계산로직은 스케줄러로 분리
    ...
    */
    public List<TariffWM> getNewestTariff(Contract contract) {
		...
		Query query = em.createQuery(sb.toString(), String.class);
		query.setParameter("tariffId", contract.getTariffIndexId());
		...
		Query query2 = em.createQuery(sb2.toString(), TariffWM.class);
		query2.setParameter("tariffId", contract.getTariffIndexId());
		query2.setParameter("yyyymmdd", yyyymmdd);
