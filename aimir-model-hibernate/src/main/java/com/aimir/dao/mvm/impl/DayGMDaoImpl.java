...
public class DayGMDaoImpl extends AbstractHibernateGenericDao<DayGM, Integer> implements DayGMDao {

	private static Log logger = LogFactory.getLog(DayGMDaoImpl.class);
    
	@Autowired
	protected DayGMDaoImpl(SessionFactory sessionFactory) {
		super(DayGM.class);
    ...
    public List<DayGM> getDayGMsByMap(Map map) {
	    ...
	    Criteria criteria = getSession().createCriteria(DayGM.class);
   ...
   public List<DayGM> getDayGMsByList(List<Map> list) {
    ...
    public List<Object> getDayGMsCount(Set<Condition> conditions,
            String div) {

        
        DetachedCriteria  criteriaSub = DetachedCriteria.forClass(DayGM.class);
        Criteria criteria = getSession().createCriteria(DayGM.class);
...
//			sb.append("\n  AND D.YYYYMMDD='20100517' ");
	 ...
	 */
	public List<Object> getConsumptionGmCo2SearchDayTypeTotal(Map<String, Object> condition) {
		
		logger.info("BEMS 주기에 따른 빌딩의  TOTAL 사용량 , TOTAL 탄소배출량  차트 데이터 조회.\n==== conditions ====\n"+condition);
		
		String searchDateType = ObjectUtils.defaultIfNull( condition.get("searchDateType") , CommonConstants.DateType.DAILY.getCode() ).toString(); // 일 , 주 , 월 , 분기
		Integer locationId = (Integer) condition.get("locationId");
//		String channelType  = (String)condition.get("channelType"); // 탄소일 경우만 0 , 전력/온도/습도의 사용량일때는 1
		String startDate = (String)condition.get("startDate");  
		String endDate   = (String)condition.get("endDate"); 
		...
		sb.append("\n 	SELECT LL.SUPPLIER_ID , SUM(D.TOTAL) TOTAL FROM DAY_GM D , ( ");
...
//			sb.append("\n  AND D.YYYYMMDD='20100517' ");
		...
		sb.append("\n LEFT JOIN ");
		sb.append("\n ( ");
		sb.append("\n 	SELECT LL2.SUPPLIER_ID , SUM(D2.TOTAL) AS CO2_TOTAL FROM DAY_GM D2 , ( ");
...
//			sb.append("\n  AND D2.YYYYMMDD='20100517' ");
    ...
    /**
     *  BEMS 동별   일(시간)전력사용량 , 탄소배출량  차트 데이터 조회.
...
//
//        logger.info("BEMS 동별   일(시간)전력사용량 , 탄소배출량  차트 데이터 조회.\n==== conditions ====\n"+condition);
//        
//		String searchDateType = ObjectUtils.defaultIfNull( condition.get("searchDateType") , CommonConstants.DateType.DAILY.getCode() ).toString(); // 일 , 주 , 월 , 분기
//		@SuppressWarnings("unused")
//        Integer supplierId = (Integer) condition.get("supplierId");
...
//		sb.append("\n 	SELECT LL.ID AS LOCATION_ID , LL.NAME , LL.MDS_ID , D.CHANNEL , SUM(D.TOTAL) TOTAL FROM DAY_GM AS D , ( ");
//		sb.append("\n 		SELECT M.MDS_ID , L.ID , L.NAME FROM METER AS M , (  ");
//		sb.append("\n 			SELECT ID , NAME FROM LOCATION WHERE PARENT_ID = :parentId ");
//		sb.append("\n 		) AS L WHERE M.LOCATION_ID = L.ID ");
////		sb.append("\n 		   AND M.METERTYPE_ID = (  ");
////		sb.append("\n 		   		SELECT CC.id FROM code AS CC WHERE CC.parent_id = ( ");
////		sb.append("\n 		   			SELECT C.id FROM code AS C WHERE C.CODE = '1.3.1' "); // MeterType
////		sb.append("\n 		   		) AND CC.CODE = '1.3.1.3' "); // GasMeter
////		sb.append("\n 		   	) ");
//		sb.append("\n 	) AS LL WHERE D.MDEV_ID = LL.MDS_ID AND D.CHANNEL=1 ");
//		if(CommonConstants.DateType.DAILY.getCode().equals(searchDateType)) {                // 일별	
//			sb.append("    AND D.YYYYMMDD = :startDate    ");
////			sb.append("\n  AND D.YYYYMMDD='20100517' ");
//		} else if (CommonConstants.DateType.WEEKLY.getCode().equals(searchDateType) || CommonConstants.DateType.MONTHLY.getCode().equals(searchDateType) || CommonConstants.DateType.QUARTERLY.getCode().equals(searchDateType)) {        // 주/월/분기별
...
//		sb.append("\n LEFT JOIN ");
//		sb.append("\n ( ");
//		sb.append("\n 	SELECT LL2.ID AS LOCATION_ID , SUM(D2.TOTAL) AS CO2_TOTAL FROM DAY_GM AS D2 , ( ");
//		sb.append("\n 		SELECT M2.MDS_ID , L2.ID , L2.NAME FROM METER AS M2 , (  ");
//		sb.append("\n 			SELECT ID , NAME FROM LOCATION WHERE PARENT_ID = :parentId ");
//		sb.append("\n 		) AS L2 WHERE M2.LOCATION_ID = L2.ID ");
////		sb.append("\n 		   AND M2.METERTYPE_ID = (  ");
////		sb.append("\n 		   		SELECT CC2.id FROM code AS CC2 WHERE CC2.parent_id = ( ");
////		sb.append("\n 		   			SELECT C2.id FROM code AS C2 WHERE C2.CODE = '1.3.1' "); // MeterType
////		sb.append("\n 		   		) AND CC2.CODE = '1.3.1.3' "); // GasMeter
////		sb.append("\n 		   	) ");
//		sb.append("\n 	) AS LL2 WHERE D2.MDEV_ID = LL2.MDS_ID AND D2.CHANNEL=0 ");
//		if(CommonConstants.DateType.DAILY.getCode().equals(searchDateType)) {                // 일별	
//			sb.append("    AND D2.YYYYMMDD = :startDate    ");
////			sb.append("\n  AND D2.YYYYMMDD='20100517' ");
//		} else if (CommonConstants.DateType.WEEKLY.getCode().equals(searchDateType) || CommonConstants.DateType.MONTHLY.getCode().equals(searchDateType) || CommonConstants.DateType.QUARTERLY.getCode().equals(searchDateType)) {        // 주/월/분기별
...
//		String channelType  = (String)condition.get("channelType"); // 탄소일 경우만 0 , 전력/온도/습도의 사용량일때는 1
		...
		sb.append("\n SELECT GM.LOCATION_ID , GM.NAME , GM.MDS_ID , GM.TOTAL , CO2.CO2_TOTAL FROM (  ");
		sb.append("\n 	SELECT LL.ID AS LOCATION_ID , LL.NAME , LL.MDS_ID , D.CHANNEL , SUM(D.TOTAL) TOTAL FROM DAY_GM D , ( ");
...
//			sb.append("\n  AND D.YYYYMMDD='20100517' ");
		...
		sb.append("\n LEFT JOIN ");
		sb.append("\n ( ");
		sb.append("\n 	SELECT LL2.ID AS LOCATION_ID , SUM(D2.TOTAL) AS CO2_TOTAL FROM DAY_GM D2 , ( ");
...
//			sb.append("\n  AND D2.YYYYMMDD='20100517' ");
	...
	/**
	 *  BEMS 전력사용량 , 탄소배출량  차트 최대/최소 데이터 조회.
	...
	public List<Object> getConsumptionGmCo2MonitoringSumMinMaxLocationId(Map<String, Object> condition) {
		...
		sb.append("\n SELECT GM.* , CO2.* FROM (  ");
		sb.append("\n 	SELECT LL.ID AS GM_PARENT_ID  ");
		sb.append("\n 	     , SUM(D.TOTAL) AS GM_TOTAL  ");
		for(int i=0; i<dataType.length; i++){

			sb.append("\n , " + dataType[i] + "(D.VALUE_00) AS GM_" + dataType[i] + "_00 , " + dataType[i] + "(D.VALUE_01) AS GM_" + dataType[i] + "_01 , " + dataType[i] + "(D.VALUE_02) AS GM_" + dataType[i] + "_02 , " + dataType[i] + "(D.VALUE_03) AS GM_" + dataType[i] + "_03 , " + dataType[i] + "(D.VALUE_04) AS GM_" + dataType[i] + "_04 " );
			sb.append("\n , " + dataType[i] + "(D.VALUE_05) AS GM_" + dataType[i] + "_05 , " + dataType[i] + "(D.VALUE_06) AS GM_" + dataType[i] + "_06 , " + dataType[i] + "(D.VALUE_07) AS GM_" + dataType[i] + "_07 , " + dataType[i] + "(D.VALUE_08) AS GM_" + dataType[i] + "_08 , " + dataType[i] + "(D.VALUE_09) AS GM_" + dataType[i] + "_09 " );
			sb.append("\n , " + dataType[i] + "(D.VALUE_10) AS GM_" + dataType[i] + "_10 , " + dataType[i] + "(D.VALUE_11) AS GM_" + dataType[i] + "_11 , " + dataType[i] + "(D.VALUE_12) AS GM_" + dataType[i] + "_12 , " + dataType[i] + "(D.VALUE_13) AS GM_" + dataType[i] + "_13 , " + dataType[i] + "(D.VALUE_14) AS GM_" + dataType[i] + "_14 " );
			sb.append("\n , " + dataType[i] + "(D.VALUE_15) AS GM_" + dataType[i] + "_15 , " + dataType[i] + "(D.VALUE_16) AS GM_" + dataType[i] + "_16 , " + dataType[i] + "(D.VALUE_17) AS GM_" + dataType[i] + "_17 , " + dataType[i] + "(D.VALUE_18) AS GM_" + dataType[i] + "_18 , " + dataType[i] + "(D.VALUE_19) AS GM_" + dataType[i] + "_19 " );
			sb.append("\n , " + dataType[i] + "(D.VALUE_20) AS GM_" + dataType[i] + "_20 , " + dataType[i] + "(D.VALUE_21) AS GM_" + dataType[i] + "_21 , " + dataType[i] + "(D.VALUE_22) AS GM_" + dataType[i] + "_22 , " + dataType[i] + "(D.VALUE_23) AS GM_" + dataType[i] + "_23 " );		
		}
...
//			sb.append("\n    AND D2.YYYYMMDD <= :endDate     ");
		...
		sb.append("\n 	GROUP BY LL2.ID ");
		sb.append("\n ) CO2 ");
		sb.append("\n ON GM.GM_PARENT_ID = CO2.CO2_PARENT_ID");
...
//			query.setString("endDate", endDate);
	...
	public List<Object> getConsumptionGmCo2MonitoringSumMinMaxPrentId(Map<String, Object> condition) {
		...
		logger.info("일(시간) 빌딩 전체 : 가스 사용량/탄솝출량 의 최대 ,최소,합\n==== conditions ====\n"+condition);
		...
		sb.append("\n SELECT GM.* , CO2.* FROM (  ");
		sb.append("\n 	SELECT LL.PARENT_ID AS GM_PARENT_ID  ");
		sb.append("\n 	     , SUM(D.TOTAL) AS GM_TOTAL  ");
		for(int i=0; i<dataType.length; i++){
...
//			query.setString("endDate", endDate);
	...
	/**
	 *  BEMS 일주일 전력사용량 , 탄소배출량  차트 데이터 조회.
	 *  일주일 : 빌딩 전체 전력사용량/탄솝출량 
	 */
...
//		String channelType  = (String)condition.get("channelType"); // 탄소일 경우만 0 , 전력/온도/습도의 사용량일때는 1
		...
		sb.append("\n    SELECT GM.YYYYMMDD AS YYYYMMDD , GM.TOTAL AS GM_TOTAL ,  CO2.TOTAL AS CO2_TOTAL  ");
		sb.append("\n      FROM (      ");
...
//		sb.append("\n 		   	) ");
		...
		sb.append("\n         ) CO2    ");
		sb.append("\n    ON GM.YYYYMMDD = CO2.YYYYMMDD ");
		sb.append("\n	ORDER BY GM.YYYYMMDD ASC  ");
...
//		sb.append("\n 		   	) ");
	...
	public List<Object> getDayGMsByNoSended(String date) {
		...
		sb.append("\n 	(select d.mdev_id,d.total,l.name,l.parent_id from day_gm d, ");
	...
	/* (non-Javadoc)
	 * @see com.aimir.dao.mvm.DayGMDao#getDayGMs(com.aimir.model.mvm.DayGM)
	 */
	public List<DayGM> getDayGMs(DayGM dayGM) {
		...
		if (dayGM != null) {
			...
			if (dayGM.getContract() != null) {
				
				if (dayGM.getContract().getId() != null) {
					
					criteria.add(Restrictions.eq("contract.id", dayGM.getContract().getId()));
				} 
			}
			
			if (dayGM.getChannel() != null) {
				
				criteria.add(Restrictions.eq("id.channel", dayGM.getChannel()));
			}
			
			if (dayGM.getYyyymmdd() != null) {
				
				
				criteria.add(Restrictions.eq("id.yyyymmdd", dayGM.getYyyymmdd()));
			}
			
			if (dayGM.getMDevType() != null) {
				
				criteria.add(Restrictions.eq("id.mdevType", dayGM.getMDevType()));
			}
		}
	...
	public List<Object> getDayGMsAvg(DayGM dayGM) {
		...
		if (dayGM != null) {
			...
			if (dayGM.getLocation() != null) {

				criteria.add(Restrictions.eq("location.id", dayGM.getLocation().getId()));
			}
		}
...
//        sb.append("\nGROUP BY c.location_id ");
	...
	public List<Object> getConsumptionGmValueSum(int supplierId, String startDate, String endDate, int starthour, int endhour) {
