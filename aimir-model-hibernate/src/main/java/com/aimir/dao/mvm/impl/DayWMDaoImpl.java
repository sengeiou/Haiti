...
public class DayWMDaoImpl extends AbstractHibernateGenericDao<DayWM, Integer> implements DayWMDao {

	private static Log logger = LogFactory.getLog(DayWMDaoImpl.class);
    
	@Autowired
	protected DayWMDaoImpl(SessionFactory sessionFactory) {
		super(DayWM.class);
    ...
    public List<DayWM> getDayWMsByMap(Map map) {
	    ...
	    Criteria criteria = getSession().createCriteria(DayWM.class);
   ...
   public List<DayWM> getDayWMsByList(List<Map<?,?>> list) {
    ...
    public List<Object> getDayWMsCount(Set<Condition> conditions,
            String div) {
        
        DetachedCriteria  criteriaSub = DetachedCriteria.forClass(DayWM.class);
        Criteria criteria = getSession().createCriteria(DayWM.class);
	...
	public List<Object[]> getDayBillingChartData(Map<String, String> conditionMap) {
		...
		query.setString("serviceTypeCode", MeterType.WaterMeter.getServiceType());
	...
	public List<Object> getConsumptionMonitoring(Map<String, Object> condition) {
		...
		String channelType  = "1"; // 탄소일 경우만 0 , 수도/온도/습도의 사용량일때는 1
...
//		sb.append("\n 		   		) AND CC.CODE = '1.3.1.2' "); // WaterMeter
//		sb.append("\n 		   	) ");
...
//			sb.append("\n  AND D.YYYYMMDD='20100517' ");
	 ...
	 */
	public List<Object> getConsumptionWmCo2SearchDayTypeTotal(Map<String, Object> condition) {
		
		logger.info("BEMS 주기에 따른 빌딩의  TOTAL 사용량 , TOTAL 탄소배출량  차트 데이터 조회.\n==== conditions ====\n"+condition);
		
		String searchDateType = ObjectUtils.defaultIfNull( condition.get("searchDateType") , CommonConstants.DateType.DAILY.getCode() ).toString(); // 일 , 주 , 월 , 분기
		Integer locationId = (Integer) condition.get("locationId");
//		String channelType  = (String)condition.get("channelType"); // 탄소일 경우만 0 , 수도/온도/습도의 사용량일때는 1
		String startDate = (String)condition.get("startDate");  
		String endDate   = (String)condition.get("endDate"); 
		...
		sb.append("\n SELECT WM.SUPPLIER_ID , WM.TOTAL , CO2.CO2_TOTAL FROM (  ");
		sb.append("\n 	SELECT LL.SUPPLIER_ID , SUM(D.TOTAL) TOTAL FROM DAY_WM D , ( ");
...
//			sb.append("\n  AND D.YYYYMMDD='20100517' ");
		...
		sb.append("\n LEFT JOIN ");
		sb.append("\n ( ");
		sb.append("\n 	SELECT LL2.SUPPLIER_ID , SUM(D2.TOTAL) AS CO2_TOTAL FROM DAY_WM D2 , ( ");
...
//		sb.append("\n 		   		) AND CC2.CODE = '1.3.1.2' "); // WaterMeter
//		sb.append("\n 		   	) ");
...
//			sb.append("\n  AND D2.YYYYMMDD='20100517' ");
		...
		sb.append("\n ) CO2 ");
		sb.append("\n ON WM.SUPPLIER_ID = CO2.SUPPLIER_ID ");
    ...
    /**
     *  BEMS 동별   일(시간)수도사용량 , 탄소배출량  차트 데이터 조회.
	...
	public List<Object> getConsumptionWmCo2DayMonitoringParentId(Map<String, Object> condition) {
        ...
        logger.info("BEMS 동별   일(시간)수도사용량 , 탄소배출량  차트 데이터 조회.\n==== conditions ====\n"+condition);
...
//		String channelType  = (String)condition.get("channelType"); // 탄소일 경우만 0 , 수도/온도/습도의 사용량일때는 1
		...
		sb.append("\n SELECT WM.LOCATION_ID , WM.NAME , WM.MDS_ID , WM.TOTAL , CO2.CO2_TOTAL FROM (  ");
		sb.append("\n 	SELECT LL.ID AS LOCATION_ID , LL.NAME , LL.MDS_ID , D.CHANNEL , SUM(D.TOTAL) TOTAL FROM DAY_WM D , ( ");
...
//			sb.append("\n  AND D.YYYYMMDD='20100517' ");
		...
		sb.append("\n LEFT JOIN ");
		sb.append("\n ( ");
		sb.append("\n 	SELECT LL2.ID AS LOCATION_ID , SUM(D2.TOTAL) AS CO2_TOTAL FROM DAY_WM D2 , ( ");
...
//			sb.append("\n  AND D2.YYYYMMDD='20100517' ");
		...
		sb.append("\n ) CO2 ");
		sb.append("\n ON WM.LOCATION_ID = CO2.LOCATION_ID ");
	...
	public List<Object> getConsumptionWmCo2MonitoringSumMinMaxLocationId(Map<String, Object> condition) {
		...
		sb.append("\n SELECT WM.* , CO2.* FROM (  ");
		sb.append("\n 	SELECT LL.ID AS WM_PARENT_ID  ");
		sb.append("\n 	     , SUM(D.TOTAL) AS WM_TOTAL  ");
		for(int i=0; i<dataType.length; i++){

			sb.append("\n , " + dataType[i] + "(D.VALUE_00) AS WM_" + dataType[i] + "_00 , " + dataType[i] + "(D.VALUE_01) AS WM_" + dataType[i] + "_01 , " + dataType[i] + "(D.VALUE_02) AS WM_" + dataType[i] + "_02 , " + dataType[i] + "(D.VALUE_03) AS WM_" + dataType[i] + "_03 , " + dataType[i] + "(D.VALUE_04) AS WM_" + dataType[i] + "_04 " );
			sb.append("\n , " + dataType[i] + "(D.VALUE_05) AS WM_" + dataType[i] + "_05 , " + dataType[i] + "(D.VALUE_06) AS WM_" + dataType[i] + "_06 , " + dataType[i] + "(D.VALUE_07) AS WM_" + dataType[i] + "_07 , " + dataType[i] + "(D.VALUE_08) AS WM_" + dataType[i] + "_08 , " + dataType[i] + "(D.VALUE_09) AS WM_" + dataType[i] + "_09 " );
			sb.append("\n , " + dataType[i] + "(D.VALUE_10) AS WM_" + dataType[i] + "_10 , " + dataType[i] + "(D.VALUE_11) AS WM_" + dataType[i] + "_11 , " + dataType[i] + "(D.VALUE_12) AS WM_" + dataType[i] + "_12 , " + dataType[i] + "(D.VALUE_13) AS WM_" + dataType[i] + "_13 , " + dataType[i] + "(D.VALUE_14) AS WM_" + dataType[i] + "_14 " );
			sb.append("\n , " + dataType[i] + "(D.VALUE_15) AS WM_" + dataType[i] + "_15 , " + dataType[i] + "(D.VALUE_16) AS WM_" + dataType[i] + "_16 , " + dataType[i] + "(D.VALUE_17) AS WM_" + dataType[i] + "_17 , " + dataType[i] + "(D.VALUE_18) AS WM_" + dataType[i] + "_18 , " + dataType[i] + "(D.VALUE_19) AS WM_" + dataType[i] + "_19 " );
			sb.append("\n , " + dataType[i] + "(D.VALUE_20) AS WM_" + dataType[i] + "_20 , " + dataType[i] + "(D.VALUE_21) AS WM_" + dataType[i] + "_21 , " + dataType[i] + "(D.VALUE_22) AS WM_" + dataType[i] + "_22 , " + dataType[i] + "(D.VALUE_23) AS WM_" + dataType[i] + "_23 " );		
		}
...
//			sb.append("\n    AND D2.YYYYMMDD <= :endDate     ");
		...
		sb.append("\n 	GROUP BY LL2.ID ");
		sb.append("\n ) CO2 ");
		sb.append("\n ON WM.WM_PARENT_ID = CO2.CO2_PARENT_ID");
...
//			query.setString("endDate", endDate);
	...
	public List<Object> getConsumptionWmCo2MonitoringSumMinMaxPrentId(Map<String, Object> condition) {
		...
		sb.append("\n SELECT WM.* , CO2.* FROM (  ");
		sb.append("\n 	SELECT LL.PARENT_ID AS WM_PARENT_ID  ");
		sb.append("\n 	     , SUM(D.TOTAL) AS WM_TOTAL  ");
		for(int i=0; i<dataType.length; i++){
...
//		String channelType  = (String)condition.get("channelType"); // 탄소일 경우만 0 , 수도/온도/습도의 사용량일때는 1
		...
		sb.append("\n    SELECT WM.YYYYMMDD AS YYYYMMDD , WM.TOTAL AS WM_TOTAL ,  CO2.TOTAL AS CO2_TOTAL  ");
		sb.append("\n      FROM (      ");
...
//		sb.append("\n 		   	) ");
		...
		sb.append("\n         ) CO2    ");
		sb.append("\n    ON WM.YYYYMMDD = CO2.YYYYMMDD ");
		sb.append("\n	ORDER BY WM.YYYYMMDD ASC  ");
...
//		sb.append("\n 		   	) ");
	...
	public List<Object> getDayWMsByNoSended(String date) {
		...
		sb.append("\n 	(select d.mdev_id,d.total,l.name,l.parent_id from day_wm d, ");
	...
	/* (non-Javadoc)
	 * @see com.aimir.dao.mvm.DayWMDao#getDayWMs(com.aimir.model.mvm.DayWM)
	 */
	public List<DayWM> getDayWMs(DayWM dayWM) {
		...
		if (dayWM != null) {
			...
			if (dayWM.getContract() != null) {
				
				if (dayWM.getContract().getId() != null) {
					
					criteria.add(Restrictions.eq("contract.id", dayWM.getContract().getId()));
				} 
			}
			
			if (dayWM.getChannel() != null) {
				
				criteria.add(Restrictions.eq("id.channel", dayWM.getChannel()));
			}
			
			if (dayWM.getYyyymmdd() != null) {
				
				
				criteria.add(Restrictions.eq("id.yyyymmdd", dayWM.getYyyymmdd()));
			}
			
			if (dayWM.getMDevType() != null) {
				
				criteria.add(Restrictions.eq("id.mdevType", dayWM.getMDevType()));
			}
		}
	...
	public List<Object> getDayWMsAvg(DayWM dayWM) {
		...
		if (dayWM != null) {
			...
			if (dayWM.getLocation() != null) {

				criteria.add(Restrictions.eq("location.id", dayWM.getLocation().getId()));
			}
		}
...
//        sb.append("\nGROUP BY c.location_id ");
	...
	public List<Object> getConsumptionWmValueSum(int supplierId, String startDate, String endDate, int starthour, int endhour) {
