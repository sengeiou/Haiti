...
public class MonthHUMDaoImpl extends AbstractHibernateGenericDao<MonthHUM, Integer> implements MonthHUMDao {

	private static Log logger = LogFactory.getLog(MonthHUMDaoImpl.class);
    
	@Autowired
	protected MonthHUMDaoImpl(SessionFactory sessionFactory) {
	...
	public List<Object> getMonthHUMsMaxMinAvg(Set<Condition> conditions, String div) {
		...
		Criteria criteria = getSession().createCriteria(MonthHUM.class);
    ...
    public List<Object> getUsageChartData(Set<Condition> conditions) {
		...
		while (cIterator.hasNext()) {
			...
			if ("date".equalsIgnoreCase(con.getField())) {
				maxStr = maxStr
						+ "(SELECT MAX(m.MAXIMUMVALUE) FROM MONTH_HUM m WHERE m.yyyymm >= :"
						+ con.getOperator() + "_startDate";
				maxStr = maxStr + " AND m.yyyymm <= :" + con.getOperator()
						+ "_endDate ";
				minStr = minStr
						+ "(SELECT MIN(m.MINIMUMVALUE) FROM MONTH_HUM m WHERE m.yyyymm >= :"
						+ con.getOperator() + "_startDate";
		...
		querySb.append(" from MONTH_HUM m where m.yyyymm >=:startDate ");
