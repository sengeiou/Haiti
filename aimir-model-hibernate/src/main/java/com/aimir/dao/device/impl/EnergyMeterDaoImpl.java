...
import com.aimir.constants.CommonConstants.PaymentStatus;
import com.aimir.constants.CommonConstants.PaymentType;
...
public class EnergyMeterDaoImpl extends AbstractHibernateGenericDao<EnergyMeter, Integer> implements EnergyMeterDao {
	...
	@Autowired
	protected EnergyMeterDaoImpl(SessionFactory sessionFactory) {
...
//		int firstIndex = page * pageSize;
//		
//		// 네이티브 쿼리시 '' 하나의글자만 나옴. 버그?. 그래서 한 짖긔릐임
//		// Location = L, Contract = C, MCU = U, METER = M, MODEM = D, ENDDEVICE = E
//		String groupTypeName = null;
//		if("Location".equals(groupType)) groupTypeName = "L";
//		else if("Contract".equals(groupType)) groupTypeName = "C";
//		else if("MCU".equals(groupType)) groupTypeName = "U";
//		else if("Modem".equals(groupType)) groupTypeName = "D";
//		else if("Meter".equals(groupType)) groupTypeName = "M";
//		else if("EndDevice".equals(groupType)) groupTypeName = "E";		
//		
//		int debit = codeDao.getCodeIdByCode(PaymentType.Debit.getCode());
//		int credit = codeDao.getCodeIdByCode(PaymentType.Credit.getCode());
//		int paid = codeDao.getCodeIdByCode(PaymentStatus.Paid.getCode());
//		int unpaid = codeDao.getCodeIdByCode(PaymentStatus.Unpaid.getCode());
//		
//		StringBuilder sb = new StringBuilder()
//		.append(" SELECT 'f' as checked, location.name as locationName, customer.name as customerName,                             \n")
//		.append("        meter.MDS_ID as meterId, mcu.SYS_ID as mcuId, contract.contractDemand as supplyCapacity,                      \n")
//		.append("        code.name as meterStatus, meter.id as id, contract.id as contractId, endDevice.SERIAL_NUMBER as endDeviceId,  \n")          
//		.append("        '" + groupTypeName + "' as groupType , ' ' as target , contract.CONTRACT_NUMBER as contractNumber, modem.DEVICE_SERIAL as modemId, \n")
//		.append("        CASE WHEN (contract.CREDITTYPE_ID = :debit AND contract.CREDITSTATUS_ID = :unpaid AND meter.SWITCH_STATUS = :activation ) THEN 1 ")
//		.append("             WHEN (contract.CREDITTYPE_ID = :debit AND contract.CURRENTCREDIT = 0  AND meter.SWITCH_STATUS = :activation ) THEN 1 ")
//		.append("             WHEN (contract.CREDITTYPE_ID = :debit AND contract.PREPAYMENTTHRESHOLD >= contract.CURRENTCREDIT AND meter.SWITCH_STATUS = :activation ) THEN 1 ")
////		.append("             WHEN (contract.CREDITTYPE_ID = :debit AND contract.CURRENTCREDIT = :unpaid AND meter.SWITCH_STATUS = :activation ) THEN 1 ")
////		.append("             WHEN (contract.CREDITTYPE_ID = :credit AND contract.CONTRACTDEMAND = :unpaid AND meter.SWITCH_STATUS = :activation ) THEN 1 ")
////		.append("             WHEN (contract.CREDITTYPE_ID = :debit AND contract.CREDITSTATUS_ID = :unpaid AND meter.SWITCH_STATUS = :activation ) THEN 1 ")
//		.append("             WHEN (contract.CREDITTYPE_ID = :debit AND contract.CREDITSTATUS_ID = :unpaid AND meter.SWITCH_STATUS = :activation ) THEN 2 ")
//		.append("             WHEN (contract.CREDITTYPE_ID = :debit AND contract.CREDITSTATUS_ID = :unpaid AND meter.SWITCH_STATUS = :activation ) THEN 2 ")
//		.append("             WHEN (contract.CREDITTYPE_ID = :debit AND contract.CREDITSTATUS_ID = :unpaid AND meter.SWITCH_STATUS = :activation ) THEN 2")
//		.append("             WHEN (contract.CREDITTYPE_ID = :debit AND contract.CREDITSTATUS_ID = :unpaid AND meter.SWITCH_STATUS = :activation ) THEN 2 ")
//		.append("             WHEN (contract.CREDITTYPE_ID = :debit AND contract.CREDITSTATUS_ID = :unpaid AND meter.SWITCH_STATUS = :activation ) THEN 2 ")				
//		.append("   FROM METER meter                                                                                                   \n")
//		.append("   LEFT OUTER JOIN LOCATION location ON meter.LOCATION_ID=location.id                                                 \n")
//		.append("   LEFT OUTER JOIN MODEM modem ON meter.MODEM_ID=modem.id                                                             \n")
//		.append("   LEFT OUTER JOIN MCU mcu ON modem.MCU_ID=mcu.id                                                                     \n")
//		.append("   LEFT OUTER JOIN Contract contract ON meter.CONTRACT_NUMBER=contract.id                                             \n")
//		.append("   LEFT OUTER JOIN Customer customer ON contract.customer_id=customer.id                                              \n")
//		.append("   LEFT OUTER JOIN EndDevice endDevice ON meter.ENDDEVICE_ID=endDevice.id                                             \n")
//		.append("   LEFT OUTER JOIN CODE code ON meter.METER_STATUS=code.id                                                            \n")
//		.append("  WHERE meter.METER='EnergyMeter'                                                                                     \n");
//		
//		if(!"".equals(switchStatus)) sb.append(" AND meter.SWITCH_STATUS = :switchStatus  ");
//		if(!"".equals(meterStatus)) sb.append(" AND meter.METER_STATUS = :meterStatus  ");
//		
//		if(!"".equals(target)) {
//			if("Location".equals(groupType))
//				sb.append(" AND location.NAME LIKE :target ");
//			else if("Contract".equals(groupType))
//				sb.append(" AND contract.CONTRACT_NUMBER LIKE :target ");
//			else if("MCU".equals(groupType)) 
//				sb.append(" AND mcu.SYS_ID LIKE :target ");
//			else if("Modem".equals(groupType)) 
//				sb.append(" AND modem.deviceSerial LIKE :target ");
//			else if("Meter".equals(groupType)) 
//				sb.append(" AND meter.MDS_ID LIKE :target ");
//			else if("EndDevice".equals(groupType)) 
//				sb.append(" AND endDevice.SERIAL_NUMBER like :target ");			
//		}
//
//		SQLQuery query = getSession().createSQLQuery(sb.toString());		
//		query.setFirstResult(firstIndex);		
//		query.setMaxResults(pageSize);
//		query.setInteger("debit", debit);
//		query.setInteger("unpaid", unpaid);
//		query.setInteger("activation", CircuitBreakerStatus.Activation.ordinal());
//		
//		if(!"".equals(switchStatus)) query.setInteger("switchStatus", CircuitBreakerStatus.valueOf(switchStatus).ordinal());
//		if(!"".equals(meterStatus)) query.setInteger("meterStatus", CommonConstants.getMeterStatusByName(meterStatus).getId());
//		
