...
public class ZeroConsumptionCustomerMakeExcel {

	private static Log logger = LogFactory.getLog(ZeroConsumptionCustomerMakeExcel.class);

	public ZeroConsumptionCustomerMakeExcel() {
...
//			DecimalFormat mdf = DecimalUtil.getDecimalFormat(supplier.getMd());
			...
			for (int i = 0; i < dataCount; i++) {
				...
				cell = row.createCell(2);
				// cell.setCellValue(resultMap.get("lastTokenDate")==null?"":TimeLocaleUtil.getLocaleDateByMediumFormat(((String)resultMap.get("lastTokenDate")),
				// lang, country));
				cell.setCellValue(resultMap.get("lastTokenDate") == null ? "" : resultMap.get("lastTokenDate").toString());
				cell.setCellStyle(dataCellStyle);

				cell = row.createCell(3);
				// cell.setCellValue(resultMap.get("currentCredit")==null?"":cdf.format(Double.parseDouble((String)
				// resultMap.get("currentCredit"))));
				...
				cell = row.createCell(8);
				// cell.setCellValue(resultMap.get("emergencyCreditMaxDate")==null?"":TimeLocaleUtil.getLocaleDateByMediumFormat(((String)resultMap.get("emergencyCreditMaxDate")),
				// lang, country));
