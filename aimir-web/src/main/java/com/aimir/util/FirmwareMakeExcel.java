...
//			DecimalFormat mdf = DecimalUtil.getDecimalFormat(supplier.getMd());
			...
			cell.setCellValue(msgMap.get("deviceIdParent"));
			cell.setCellStyle(titleCellStyle);

			cell = row.createCell(2);
			cell.setCellValue(msgMap.get("fmversionParent"));
			cell.setCellStyle(titleCellStyle);
			// Title End
			...
			for (int i = 0; i < dataCount; i++) {
				...
				cell = row.createCell(1);
				cell.setCellValue(resultMap.get("PARENT_ID") == null ? "" : resultMap.get("PARENT_ID").toString());
				cell.setCellStyle(dataCellStyle);

				cell = row.createCell(2);
				cell.setCellValue(resultMap.get("PARENT_VER") == null ? "" : resultMap.get("PARENT_VER").toString());
				cell.setCellStyle(dataCellStyle);
