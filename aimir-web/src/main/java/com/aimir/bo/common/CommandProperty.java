...
{
    private static Log log = LogFactory.getLog(CommandProperty.class);
    private static final Properties properties;

    static {
        String it = "/command.properties";

        Properties result = new Properties();
        try
        {
            InputStream is = 
                CommandProperty.class.getResourceAsStream(it);
            result.load(is);
            is.close();
    ...
    public static URL getPropertyURL(String key)
    {
        ...
        if(url == null)
        {
            url = CommandProperty.class.getResource(val);
        }
