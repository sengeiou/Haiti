...
<script type="text/javascript" charset="utf-8">
	...
	function commonDateTabInit() {
		...
		// 탭유형설정,조회일자설정
		$(function() {
			$('#_hourly').bind('click', function(event) {
				if (tabClickExec) {
		...
		$(function() {
			$('#_daily').bind('click', function(event) {
				if (tabClickExec) {
		...
		$(function() {
			$('#_period').bind('click', function(event) {
				if (tabClickExec) {
					$('#searchDateType').val(DateType.PERIOD);
		...
		$(function() {
			$('#_weekly').bind('click', function(event) {
				if (tabClickExec) {
					$('#searchDateType').val(DateType.WEEKLY);
		...
		$(function() {
			$('#_monthly').bind('click', function(event) {
				if (tabClickExec) {
					$('#searchDateType').val(DateType.MONTHLY);
		...
		$(function() {
			$('#_monthlyPeriod').bind('click', function(event) {
				if (tabClickExec) {
					$('#searchDateType').val(DateType.MONTHLYPERIOD);
		...
		$(function() {
			$('#_weekdaily').bind('click', function(event) {
				if (tabClickExec) {
					$('#searchDateType').val(DateType.WEEKDAILY);
		...
		$(function() {
			$('#_seasonal').bind('click', function(event) {
				if (tabClickExec) {
					$('#searchDateType').val(DateType.SEASONAL);
		...
		$(function() {
			$('#_yearly').bind('click', function(event) {
				if (tabClickExec) {
					$('#searchDateType').val(DateType.YEARLY);
		...
		//조회버튼클릭 이벤트 생성
		$(function() {
			$('#hourlySearch').bind('click', function(event) {
				sendRequest(DateType.HOURLY);
			});
		});
		$(function() {
			$('#dailySearch').bind('click', function(event) {
				sendRequest(DateType.DAILY);
			});
		});
		$(function() {
			$('#periodSearch').bind('click', function(event) {
				sendRequest(DateType.PERIOD);
			});
		});
		$(function() {
			$('#weeklySearch').bind('click', function(event) {
				sendRequest(DateType.WEEKLY);
			});
		});
		$(function() {
			$('#monthlySearch').bind('click', function(event) {
				sendRequest(DateType.MONTHLY);
			});
		});
		$(function() {
			$('#monthlyPeriodSearch').bind('click', function(event) {
				sendRequest(DateType.MONTHLYPERIOD);
			});
		});
		$(function() {
			$('#weekDailySearch').bind('click', function(event) {
				sendRequest(DateType.WEEKDAILY);
			});
		});
		$(function() {
	...
	function getSeasonalSeasonCombo(flag) {
		$.getJSON("${ctx}/common/getSeason.do", {
			...
			// Metering Data 가젯 Seasonal 검색시 오류 수정 전 원본
	...
	function getSearchDate(_dateType, flag) {
		...
		if (flag == false) {
			...
			if (DefaultDate == null || DefaultDate == 'undefined') {
				$('div#datetab > ul:first > li:first > a:first').trigger(
						'click');
			} else {
				tabClickExec = false;
				...
				if (DateType.HOURLY == DefaultDate._dateType) {
					...
					$('#hourlyStartHourCombo').option(
							DefaultDate.hourlyStartHourCombo);
					$('#hourlyEndHourCombo').option(
							DefaultDate.hourlyEndHourCombo);
					...
					$('#monthlyPeriodStartYearCombo').val(
							DefaultDate.monthlyPeriodStartYearCombo);
					$('#monthlyPeriodStartYearCombo').trigger('change');
					$('#monthlyPeriodStartMonthCombo').val(
							DefaultDate.monthlyPeriodStartMonthCombo);
					$('#monthlyPeriodEndYearCombo').val(
							DefaultDate.monthlyPeriodEndYearCombo);
					$('#monthlyPeriodEndYearCombo').trigger('change');
					$('#monthlyPeriodEndMonthCombo').val(
							DefaultDate.monthlyPeriodEndMonthCombo);
					...
					$('#weekDailyYearCombo')
							.val(DefaultDate.weekDailyYearCombo);
					$('#weekDailyYearCombo').trigger('change');
					$('#weekDailyMonthCombo').val(
							DefaultDate.weekDailyMonthCombo);
					$('#weekDailyMonthCombo').trigger('change');
					$('#weekDailyWeekCombo')
							.val(DefaultDate.weekDailyWeekCombo);
					$('#weekDailyWeekCombo').trigger('change');
					$('#weekDailyWeekDayCombo').val(
							DefaultDate.weekDailyWeekDayCombo);
					...
					$('#seasonalSeasonCombo').val(
							DefaultDate.seasonalSeasonCombo);
					$('#seasonalYearCombo').selectbox();
					$('#seasonalSeasonCombo').selectbox();
		...
		if (DateType.HOURLY == _dateType) {
			...
			$.getJSON("${ctx}/common/getMonthPeriod.do", {
				...
				$('#searchStartDate').val(json.startDate);
				if ($('#monthlyPeriodEndMonthCombo').val() == null)
					return;
...
<div id="datetab" style="display: none">
	<ul>
		<li><a href="#hourly" id="_hourly"><fmt:message
					key="aimir.hourly" /></a></li>
		<li><a href="#daily" id="_daily"><fmt:message
					key="aimir.daily" /></a></li>
		<li><a href="#period" id="_period"><fmt:message
					key="aimir.period" /></a></li>
		<li><a href="#weekly" id="_weekly"><fmt:message
					key="aimir.weekly" /></a></li>
		<li><a href="#monthly" id="_monthly"><fmt:message
					key="aimir.monthly" /></a></li>
		<li><a href="#monthlyPeriod" id="_monthlyPeriod"><fmt:message
					key="aimir.monthly" />(<fmt:message key="aimir.period" />)</a></li>
		<li><a href="#weekdaily" id="_weekdaily"><fmt:message
					key="aimir.weekdaily" /></a></li>
		<li><a href="#seasonal" id="_seasonal"><fmt:message
					key="aimir.seasonal" /></a></li>
		<li><a href="#yearly" id="_yearly"><fmt:message
					key="aimir.yearly" /></a></li>
	</ul>

	<div id="hourly">
		<!-- <label id="hourlyLabel"><fmt:message key="aimir.hourly"/></label> -->
		<ul>
			<li><input id="hourlyStartDate" class="day" type="text"
				readonly="readonly"></li>
			<li class="date-space"></li>
			<li><select id="hourlyStartHourCombo" class="sm"></select></li>
			<li><input value="~" class="between" type="text"></li>
			<li><input id="hourlyEndDate" class="day" type="text"
				readonly="readonly"></li>
			<li class="date-space"></li>
		...
		<div id="btn">
			<ul>
				<li><a href="#;" id="hourlySearch" class="on"><fmt:message
	...
	<div id="daily">
		...
		<ul>
			...
			<li><input id="dailyStartDate" type="text" class="day"
				readonly="readonly"></li>
			<li><button id="dailyRight" type="button" class="next"></button></li>
		</ul>
		<div id="btn">
			<ul>
				<li><a href="#;" id="dailySearch" class="on"><fmt:message
							key="aimir.button.search" /></a></li>
			</ul>
		</div>
	</div>
	...
	<div id="period">
		...
		<ul>
			...
			<li class="date-space"></li>
			<li><input id="periodStartDate" class="day" type="text"
				readonly="readonly"></li>
			<li><input value="~" class="between" type="text"></li>
			<li><input id="periodEndDate" class="day" type="text"
				readonly="readonly"></li>
		</ul>
		<div id="btn">
			<ul>
				<li><a href="#;" id="periodSearch" class="on"><fmt:message
							key="aimir.button.search" /></a></li>
			</ul>
		</div>
	</div>
	...
	<div id="weekly">
		...
		<ul>
			...
			<li><label class="descr"><fmt:message
						key="aimir.day.mon" /></label></li>
			<li class="date-space"></li>
		...
		<div id="btn">
			<ul>
				<li><a href="#;" id="weeklySearch" class="on"><fmt:message
							key="aimir.button.search" /></a></li>
			</ul>
		</div>
	</div>
	...
	<div id="monthly">
		...
		<div id="btn">
			<ul>
				<li><a href="#;" id="monthlySearch" class="on"><fmt:message
							key="aimir.button.search" /></a></li>
			</ul>
		</div>
	</div>
	...
	<div id="monthlyPeriod">
		...
		<div id="btn">
			<ul>
				<li><a href="#;" id="monthlyPeriodSearch" class="on"><fmt:message
							key="aimir.button.search" /></a></li>
			</ul>
		</div>
	</div>
	...
	<div id="weekdaily">
		...
		<ul>
			...
			<li><label class="descr"><fmt:message key="aimir.week" /></label></li>
			<li class="date-space"></li>
			<li><select id="weekDailyWeekDayCombo" class="date-w50"></select></li>
		</ul>
		<div id="btn">
			<ul>
				<li><a href="#;" id="weekDailySearch" class="on"><fmt:message
							key="aimir.button.search" /></a></li>
			</ul>
		</div>
	</div>
	...
	<div id="seasonal">
		...
		<div id="btn">
			...
			<ul>
				<li><a href="#;" id="seasonalSearch" class="on"><fmt:message
key="aimir.button.search" /></a></li>
			</ul>
			<div>&nbsp;</div>
	...
	<div id="yearly">
		...
		<div id="btn">
			<ul>
				<li><a href="#;" id="yearlySearch" class="on"><fmt:message
							key="aimir.button.search" /></a></li>
			</ul>
		</div>
	</div>
