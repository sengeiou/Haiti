...
<script type="text/javascript" charset="utf-8">/*<![CDATA[*/
    ...
    function tabClickSet(dateType) {
    	dateType = dateType.trim();
    	if(tabClickExec){
    		$('#searchDateType').val(dateType);
    		getSearchDate(dateType);
    	}
    	tabClickExec=true;
    	if(meteringDataColModel != null && ("WM" == mvmMiniType.toString() || "GM" == mvmMiniType.toString()) ){
    		if(dateType == DateType.HOURLY || dateType == DateType.MONTHLY) {
    ...
    function commonDateTabInit(){
        ...
        // 탭클릭이벤트 생성
        // 탭유형설정,조회일자설정
        $(function() { $('#_hourly')        .bind('click',function(event) {tabClickSet(DateType.HOURLY			);} ); });
        $(function() { $('#_daily')         .bind('click',function(event) {tabClickSet(DateType.DAILY 			);} ); });
        $(function() { $('#_period')        .bind('click',function(event) {tabClickSet(DateType.PERIOD 			);} ); });
        $(function() { $('#_weekly')        .bind('click',function(event) {tabClickSet(DateType.WEEKLY			);} ); });
        $(function() { $('#_monthly')       .bind('click',function(event) {tabClickSet(DateType.MONTHLY			)}  ); });
        $(function() { $('#_monthlyPeriod') .bind('click',function(event) {tabClickSet(DateType.MONTHLYPERIOD	);} ); });
        $(function() { $('#_weekdaily')     .bind('click',function(event) {tabClickSet(DateType.WEEKDAILY 		);} ); });
        $(function() { $('#_seasonal')      .bind('click',function(event) {tabClickSet(DateType.SEASONAL 		);} ); });
        $(function() { $('#_yearly')        .bind('click',function(event) {tabClickSet(DateType.YEARLY	 		);} ); });
