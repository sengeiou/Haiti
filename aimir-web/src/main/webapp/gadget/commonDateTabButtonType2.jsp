...
<script type="text/javascript" charset="utf-8">/*<![CDATA[*/
    ...
    function commonDateTabButton2Init(){
        ...
        $(function() { $('#btn_hourlySearch')       .bind('click',function(event) { btnSendRequest(DateType.HOURLY      ); } ); });
        ...
        $.getJSON("${ctx}/common/getYear.do"
                ...
                ,function(json) {
                     ...
                     //탭별 현재일자 설정
                     if(tabs.btn_daily!=0)btnDailyArrow('',0,false);
                     if(tabs.btn_period!=0)setBtnPeriod(0,false);

                     if(tabs.btn_weekly!=0){
        ...
        //탭숨김처리
        if(tabs.btn_yearly==0)btnHideTab(DateType.YEARLY);
        if(tabs.btn_seasonal==0)btnHideTab(DateType.SEASONAL);
        if(tabs.btn_weekDaily==0)btnHideTab(DateType.WEEKDAILY);
        if(tabs.btn_monthlyPeriod==0)btnHideTab(DateType.MONTHLYPERIOD);
        if(tabs.btn_monthly==0)btnHideTab(DateType.MONTHLY);
        if(tabs.btn_weekly==0)btnHideTab(DateType.WEEKLY);
        if(tabs.btn_period==0)btnHideTab(DateType.PERIOD);
        if(tabs.btn_daily==0)btnHideTab(DateType.DAILY);
        if(tabs.btn_hourly==0)btnHideTab(DateType.HOURLY);

        if(tabNames.hourly!=null&&tabNames.hourly!=''&&tabNames.hourly!='undefined')$('#btn_hourly').html(tabNames.hourly);
        if(tabNames.daily!=null&&tabNames.daily!=''&&tabNames.daily!='undefined')$('#btn_daily').html(tabNames.daily);
        if(tabNames.period!=null&&tabNames.period!=''&&tabNames.period!='undefined')$('#btn_period').html(tabNames.period);
        if(tabNames.weekly!=null&&tabNames.weekly!=''&&tabNames.weekly!='undefined')$('#btn_weekly').html(tabNames.weekly);
        if(tabNames.monthly!=null&&tabNames.monthly!=''&&tabNames.monthly!='undefined')$('#btn_monthly').html(tabNames.monthly);
        if(tabNames.monthlyPeriod!=null&&tabNames.monthlyPeriod!=''&&tabNames.monthlyPeriod!='undefined')$('#btn_monthlyPeriod').html(tabNames.monthlyPeriod);
        if(tabNames.weekDaily!=null&&tabNames.weekDaily!=''&&tabNames.weekDaily!='undefined')$('#btn_weekDaily').html(tabNames.weekDaily);
        if(tabNames.seasonal!=null&&tabNames.seasonal!=''&&tabNames.seasonal!='undefined')$('#btn_seasonal').html(tabNames.seasonal);
        if(tabNames.yearly!=null&&tabNames.yearly!=''&&tabNames.yearly!='undefined')$('#btn_yearly').html(tabNames.yearly);

     // 검색버튼 표시
        if(tabs.btn_search_yearly==1)        showSearchBtn2('btn_yearlySearch');
        if(tabs.btn_search_seasonal==1)      showSearchBtn2('btn_seasonalSearch');
        if(tabs.btn_search_weekDaily==1)     showSearchBtn2('btn_weekDailySearch');
        if(tabs.btn_search_monthlyPeriod==1) showSearchBtn2('btn_monthlyPeriodSearch');
        if(tabs.btn_search_monthly==1)       showSearchBtn2('btn_monthlySearch');
        if(tabs.btn_search_weekly==1)        showSearchBtn2('btn_weeklySearch');
        if(tabs.btn_search_period==1)        showSearchBtn2('btn_periodSearch');
        if(tabs.btn_search_daily==1)         showSearchBtn2('btn_dailySearch');
        if(tabs.btn_search_hourly==1)        showSearchBtn2('btn_hourlySearch');
    ...
    function getBtnSearchDate(_dateType,flag){
        ...
        if(DateType.HOURLY == _dateType){
            ...
            if(tabs.btn_monthlyPeriod==0)return;
        	...
        	if(tabs.btn_weekDaily==0)return;
...
<div id="btn_datetab" class="buttontype" style="display:none">
    ...
    <div id="div_period">
        ...
        <ul>
            ...
            <li class="date-space"></li>
            <li><input id="btn_periodStartDate" class="day" type="text" readonly="readonly"></li>
            <li><input value="~" class="between" type="text"></li>
            <li><input id="btn_periodEndDate" class="day" type="text" readonly="readonly"></li>
        </ul>
        <div id="btn">
