...
<script type="text/javascript" charset="utf-8">
        ...
        $(document).ready(function() {
            ...
            $('#topologyDivTabId').click(function() { 
           ...
           /*  $(".tooltipBox").hide();
            ...
            new Ext.ToolTip({
            	target:'mcuGroupAddBtn',
            	width:200,
            	html:'You can create a new DCU Group of searched list.',
            	trackMouse:true
            });
        });
        ...
        // 집중기 모델에 따라 표시될 버튼을 결정한다.
           var enableButton = function(modelId) {
               ...
               $('#tracerouteDiv').hide();
               $('#otaDiv').hide();
               $('#scheduleDivTabId').hide();
               $('#MCUSetScheduleBtn').hide();
               $('#MCUDeleteScheduleBtn').hide();
               $('#MCUExecuteScheduleBtn').hide();
                       ...
                       for ( var i = 0; i < json.namesOfContain.length; i++) {
                           switch (json.namesOfContain[i]) {
                               ...
                               case 'DCU OTA':
                                   $('#otaDiv').show();
                                   break;
                               case 'DCU Schedule Get':
                            	   $('#scheduleDivTabId').show();
                            	   break;
                              case 'DCU Schedule Set':
                            	   if(editAuth == "true")
                                       $('#MCUSetScheduleBtn').show();
                                   break;
                              case 'DCU Schedule Delete':
                            	   if(editAuth == "true")
                                       $('#MCUDeleteScheduleBtn').show();
                                   break;
                              case 'DCU Schedule Execute':
                           	  	   if(editAuth == "true")
	                           	       $('#MCUExecuteScheduleBtn').show();
	                           	   break;
                           }
                       }
                   }
        ...
        var displayDiv = function() {
            ...
            var modelId = $('#MCUDeviceModelId').val();
...
//----------------- 지도정보
        ...
        function mcuStatusMonitoring_PKS() {
            $('#commandResult').val("Request MCU Status....");
            $.getJSON('${ctx}/gadget/device/command/getMCUStatus_PKS.do'
                    , {'target' : mcuId
                    , 'loginId' : loginId}
                    , function (returnData){
                        if(!returnData.status){
                        ...
                        if(returnData.status.length>0 && returnData.status!='SUCCESS'){
                            ...
                            $('#commandResult').val("MCU Status Monitoring: "+returnData.status);
                            var resultmsg ="";
                        	resultmsg += ("<b>DCU ID: </b>"+returnData.sysID+"<br>");
                        	resultmsg += ("<b>DCU Type: </b>"+returnData.sysType+"<br>");
                        	resultmsg += ("<b>DCU Name: </b>"+returnData.sysName+"<br>");
                        	resultmsg += ("<b>DCU HW Model Name: </b>"+returnData.sysModel+"<br>");
                        	resultmsg += ("<b>DCU HW Version: </b>"+returnData.sysHwVersion+"<br>");
                        	resultmsg += ("<b>DCU HW Build Number: </b>"+returnData.sysHwBuild+"<br>");
                        	resultmsg += ("<b>DCU SW Version: </b>"+returnData.sysSwVersion+"<br>");
                        	resultmsg += ("<b>SW Build Number: </b>"+returnData.sysSwBuild+"<br>");
                        	resultmsg += ("<b>DCU Listen port number: </b>"+returnData.sysPort+"<br>");
                        	resultmsg += ("<b>DCU Serial Number: </b>"+returnData.sysSerialNumber+ "<br>");
                        	resultmsg += ("<b>Sytem Uptimes(secs): </b>"+returnData.sysUptime+"<br>");
                        	resultmsg += ("<b>DCU Current Time: </b>"+returnData.sysTime+"<br>");
                        	resultmsg += ("<b>DCU Timezone: </b>"+returnData.sysTimeZone+"<br>");
                        	resultmsg += ("<b>Current Temp.: </b>"+returnData.sysTemperature+"<br>");
                        	resultmsg += ("<b>Sytem Network Uptimes(secs): </b>"+returnData.sysNetworkUptime);
                        
                            Ext.Msg.show ({
                                title: 'DCU Status Monitoring',
                                msg: resultmsg,
                                minWidth:150,
                                closable:false,
                                buttons: Ext.Msg.OK,
        ...
        function commandCODIPing(){

        	
        	doCODIPing('64', '3');
        }
        
	    function doCODIPing(packetSize, count) {
	    	$('#commandResult').val("Request Coordinator Server Ping....");
	    	$.ajax({
	    	  url: '${ctx}/gadget/device/command/cmdCODIPing.do',
	    	  dataType: 'json',
	    	  async: true,
	    	  data: {'target' : $('#coordinatorId').val()
	              , 'loginId' : loginId
	              , 'packetSize' : packetSize
	              , 'count' : count
	              },
	    	  success: function(returnData) {
		             ...
		             if(returnData.status.length>0 && returnData.status!='SUCCESS'){
		                 $('#commandResult').val("Coordinator Ping: "+returnData.status);
		             } else {
		             	$('#commandResult').val(""); 
		                 $('#commandResult').val(returnData.jsonString); 
		             }
     ...
     // Get Log [Get Method Only]
        ...
        /* SP-809 */
        var forceUploadFormPanel;
        function commandForceUpload(){
           // Close the current window
            if(Ext.getCmp('forceUploadWindow')){
                Ext.getCmp('forceUploadWindow').close();
            }
            // Form Panel (item of window)
            forceUploadFormPanel = new Ext.FormPanel({
                id : 'forceUploadForm',
                defaultType : 'fieldset',
                bodyStyle:'padding:1px 1px 1px 1px',
                frame : true,
                ...
                buttons : [
                    {
                        ...
                        listeners: {
                            click: function(btn,e){
		                        var serverName = Ext.getCmp('forceUpLoadServer').getValue();
		                        var dataCount = Ext.getCmp('forceUpLoadData').getValue();
		                        var fromDate = Ext.getCmp('forceUpLoadFromDate').getValue();
		                        var toDate = Ext.getCmp('forceUpLoadFromDate').getValue();
                                //submit action
					    		Ext.Msg.wait('Waiting for response.', 'Wait !');
					    		$('#commandResult').val("Request MCU Server COAP-Ping....");
					    		
					    		if(!(dataCount >= 1 && dataCount <= 32)){
					    		    Ext.Msg.alert('<fmt:message key='aimir.message'/>','<fmt:message key="aimir.msg.confirm.datacount" />');
	                                return;
	                            }	        
        			            Ext.define('forceUploadEventAjax', {
        			                extend: 'Ext.data.Connection',
        			                singleton: true,
        			                constructor : function(config){
        			                    this.callParent([config]);
        			            ...
        			            forceUploadEventAjax.request({
						        	url :  '${ctx}/gadget/device/command/cmdForceUpload.do',
					                method : 'POST',
					                timeout : extAjaxTimeout,
					                params : {
        			        			target        : mcuId,
                                    ...
                                    function (returnData){
                                        ...
                                        if(returnData.status.length>0 && returnData.status!='SUCCESS'){
                                            $('#commandResult').val("Force Upload: "+returnData.status);
                                        }else{
                                            $('#commandResult').val("Force Upload: "+returnData.status);
                                        }
                                    }             					                		                					                
            ...
            var forceUploadWin = new Ext.Window({
                id     : 'forceUploadWindow',
                title : 'Force Upload',
				pageX : $("#dcuGridDiv").width()/2-100,
				pageY : 300,
                height : 180,
	    ...
	    function execNICommand(){
			...
			var obj = new Object();
			obj.target = $('#coordinatorId').val();
			obj.loginId = loginId;
	
			if (niWin){
				niWin.close();
			}
			niWin = window.open("${ctx}/gadget/device/codiNIPopup.jsp", 
        ...
        function commandMCUGetSchedule() {
        	...
        	Ext.Ajax.timeout = 4*1000*60;
            timeoutPeriod = 4*1000*60;
        	
        	Ext.Msg.wait('Waiting for response.', 'Wait !',{
        		text:'<fmt:message key="aimir.maximum"/>' + timeoutPeriod/1000 + 's...',
        		scope: this,
        	});
        ...
        function commandMCUSetSchedule() {
            ...
            $.post('${ctx}/gadget/device/command/cmdMcuSetSchedule_.do',
                    ...
                    function(data) {
            			Ext.Msg.hide();
                    	Ext.Msg.alert('<fmt:message key='aimir.message'/>',data.resultList+"//"+data.status);  
                    	}
                   );
        };
        ...
        var win_schedule;
        function mcuGroupSchedulePopup(){
        	var opts="width=1300px, height=700px, left=100px, top=100px,scrollbars=No,  resizable=no, status=no";
            var objs = new Object();
            objs.operatorId			 = operatorId;
            objs.supplierId          = supplierId;
            objs.loginId         	 = loginId;
            

            if(win_schedule)
            	win_schedule.close();
            win_schedule = window.open("${ctx}/gadget/device/mcuGroupSchedulePopup.do", "MCUGroupSchedule;", opts);
            win_schedule.opener.objs = objs;
        }
        
        <!-- Group Name 중복확인 -->
        function dupCheckGroupName(gName) {
            ...
            if(result.result == "Y") {
            	Ext.Msg.alert("<fmt:message key="aimir.message"/>",
            			"<fmt:message key="aimir.duplicateGroupName"/>");
                hide();
                return false;
            } else {
            	return true;
        ...
        function saveGroup(gName) {
            emergePre();

            if (!dupCheckGroupName(gName)) {
                return;
            }
            
            var conditionArray = getConditionArray();
            
            $.getJSON("${ctx}/gadget/device/addMcuGroup.do"
                    ,{supplierId : conditionArray[12],
                        mcuId : conditionArray[0],
                        mcuType : conditionArray[1],
                        locationId : conditionArray[2],
                    	...
                    	mcuStatus : conditionArray[13],
                    	mcuSerial : conditionArray[14], // sMcuSerial
                    	// firmware max gadget에서 사용하는 condition
                        swRevison : "",
                        modelId :"",
        ...
        <!-- Add Group Btn Start -->
        var mcuGroupAdd = function(){
        	var gname;
                Ext.MessageBox.prompt('Add Group',
                		'You can create a new DCU Group of searched list.<br>Please enter the group name.',
                		function(btn, groupName){
        			gName = groupName.trim();
        			if(btn=='ok'){
        				if(gname ==''){
                			Ext.Msg.alert('<fmt:message key='aimir.message'/>','Please enter the group name.');                                
                		}else{
                			saveGroup(gName);
                		}
        			}
...
<body onLoad="init();">
	...
	<div style="float:right;">
		<!-- Group Btn 추가 SP-779 -->
		<div id="groupBtn" class="btn_right_top2 margin-t10px" style="float:left;">
			<span style="float:left;">
				<!-- Group Add 버튼 -->
				<ul id="mcuGroupAddBtn">
					<li><a href="JavaScript:mcuGroupAdd();" class="btn_blue">
					<span><fmt:message key="aimir.add" />&nbsp;<fmt:message key="aimir.group" /></span></a></li>
				</ul>
			</span>
			<span style="float:right;">
				<!-- Group Command 버튼 green11pt -->
				<ul id="mcuGroupScheduleBtn">
					<li><a href="JavaScript:mcuGroupSchedulePopup();" class="btn_green">
					<span><fmt:message key="aimir.group" />&nbsp;<fmt:message key="aimir.schedule" /></span></a></li>
				</ul>
			</span>
		</div>
	
		<!-- 기존 Btn -->
		<div id="btn" style="float:right;" class="btn_right_top2 margin-t10px">
			<ul id="mcuAddBtn">
	...
	<div id="mcuInfoDiv" class="width_auto margin10px"
		...
		<div id="mcuDetailTabs" class="am_sub_tab">
			<ul>
				...
				<li><a id="topologyDivTabId" style="display: none;"><fmt:message key="aimir.topology" /></a></li>
			</ul>
		</div>
		<!--// sub tab -->
		...
		<div class="box_blu padding20px">
			...
			<div style="display:none;" id ="appendSchedule"></div>
             <div style="display:none;" id="appendSchedule2">
             	...
             	<div class="width-auto margin-t10px">
                        ...
                        <span style="padding:2px 10px 2px 2px" ></span>
						<em class="am_button margin-t1px" style="margin-right: 5px;" id='MCUSetScheduleBtn'>
							<a href="#" onClick="commandMCUSetSchedule()">Update</a>
						</em>
						<em class='am_button margin-t1px' style='margin-right: 5px;' id='MCUDeleteScheduleBtn'>
						<a href='#' onClick='commandMCUDeleteSchedule()'>Delete</a>
						</em>
						<em class='am_button margin-t1px' style='margin-right: 5px;' id='MCUExecuteScheduleBtn'>
			...
			<div id="topologyDivTab" class = "max">
				...
				<li><div id="btn">
					...
					<ul><li><a href="JavaScript:getNMSInformation();" class="on"><fmt:message key='aimir.topology.update'/></a></li></ul>
			 		<ul><li><a href="JavaScript:labelOnOff();" class="on"><fmt:message key='aimir.topology.label'/></a></li></ul>
			 		<ul><li><a href="JavaScript:unpin();" class="on"><fmt:message key='aimir.topology.unpin'/></a></li></ul>
			 		<ul><li><a href="JavaScript:content_print();" class="on"><fmt:message key='aimir.topology.snapshot'/></a></li></ul>
			 		<ul><li><a href="JavaScript:NMSPopup();" class="on"><fmt:message key='aimir.topology.nms'/></a></li></ul>
				</div>
				</li>
				</ul>
				</div>
