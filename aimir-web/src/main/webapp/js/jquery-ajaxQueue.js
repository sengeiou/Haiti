...
(function($) {
	$.ajaxQueueStop = function(queueName){
		$(document).clearQueue(queueName);
    	if(window.ajaxQueueCount[queueName] != undefined)
		{
    		window.ajaxQueueCount[queueName] = 0;
    		window.ajaxSuccessCallbacks[queueName] = {};
		}
	}
	$.ajaxQueue = function(ajaxOptions,queueName,queueMode){
		...
		//console.log('adding to queue: '+queueName);
