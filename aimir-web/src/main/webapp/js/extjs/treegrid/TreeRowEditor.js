    ...
    stopEditing: function(saveChanges) {
        ...
        for (var i = 0, len = cm.length; i < len; i++) {
            ...
            if (!c.hidden && !c.buttons) {
                ...
                /*Origin Source 
                기존값과 비교시 값의 변화만 있는 경우의 값을 changes의 입력. 
                -> 문제점 발생! 
                 이유 : 두번째 저장 버튼 클릭시 기존 입력값과 차이가 없어 changes의 값 입력이 없으며
                 hasChange가 false가 되어 서버 호출 없이 화면에 저장해버린다. 
                 if (String(oldValue) !== String(value)) {
                    changes[dindex] = value;
                    hasChange = true;
                }
                */
               if ((String(oldValue) !== String(value))|| (String(value)!==null))  {
                    changes[dindex] = value;
                    hasChange = true;
                }
            }
