	...
	$.tree = {
		...
		defaults	: {
			...
			ui		: {
				...
				//selected_delete			: "select_previous" // false,
			...
			types	: {
				"default" : {
					...
					max_children	: -1, // -1 - not set, 0 - no children, 1 - one child, etc // can
																								// be
																								// function
					max_depth		: -1, // -1 - not set, 0 - no children, 1 - one level of children, etc // can
																											// be
																											// function
					valid_children	: "all", // all, none, array of values // can
																			// be
																			// function
					icon : {
						image : false,
	...
	function tree_component () {
		...
		return {
			...
			scroll_check : function (x,y) { 
				...
				if(y - off.top < 20)						cnt.scrollTop(Math.max( (st - _this.settings.ui.scroll_spd) ,0));	// NEAR
																																// TOP
				if(cnt.height() - (y - off.top) < h_cor)	cnt.scrollTop(st + _this.settings.ui.scroll_spd);					// NEAR
																																// BOTTOM
				if(x - off.left < 20)						cnt.scrollLeft(Math.max( (sl - _this.settings.ui.scroll_spd),0));	// NEAR
																																// LEFT
				if(cnt.width() - (x - off.left) < 40)		cnt.scrollLeft(sl + _this.settings.ui.scroll_spd);					// NEAR
																																// RIGHT
				...
				if(this.settings.rules.use_max_depth) {
					...
					if(mx !== false) { 
						...
						if(typeof nod != "string") {
							var t = nod;
							// possible async problem - when nodes are not all
							// loaded down the chain
							while(t.size() > 0) {
			...
			open_branch : function (obj, disable_animation, callback) {
				...
				if(this.settings.data.async && obj.find("li").size() == 0) {
					...
					_datastore.load(this.callback("beforedata",[obj,this]),this,this.settings.data.opts,function(data){
						...
						_datastore.parse(data,_this,_this.settings.data.opts,function(str){
							str = _this.callback("onparse", [str, _this]);
							// if(obj.children('ul:eq(0)').children('li').size()
							// > 1)
							// obj.children("ul").find('.loaading').parent().replaceWith(str);
							// else
			...
			callback : function (cb, args) {
				...
				for(var i in this.settings.plugins) {
					...
					if(typeof r !== "undefined" && r !== null) {
						if(cb == "ondata" || cb == "onparse") args[0] = r; // keep
																			// the
																			// chain
																			// if
																			// data
			...
			moved : function (what, where, how, is_new, is_copy, rb) {
				...
				if(!is_new) {
					...
					if(tmp.get(0) != this.container.get(0)) {
						...
						// if there are languages - otherwise - no cleanup
						// needed
						if(tmp.settings.languages.length) {
							var res = [];
							// if new tree has no languages - use current
							// visible
