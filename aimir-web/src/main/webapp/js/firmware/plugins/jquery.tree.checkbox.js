	...
	$.extend($.tree.plugins, {
		"checkbox" : {
			...
			check : function (n) {
            	...
            	/*if(n.attr("id").indexOf("top_")>-1){
            		var_locationsID = n.attr("id");
            		subLocation = $("#"+var_locationsID+" li");
                	for(i=0 ; i<subLocation.length ; i++){
                       	$("#"+var_locationsID).children("a").removeClass("unchecked undetermined").addClass("checked");
                       	$(subLocation[i]).children("a").removeClass("unchecked undetermined").addClass("checked");
                	}
            	}else{
            		var_locationsID = n.attr("id");
            		subLocation = $("#"+var_locationsID+" li");
			...
			uncheck : function (n) {
            	...
            	//if(n.attr("id").indexOf("top_")>-1){
            		...
            		subLocation = $("#"+var_locationsID+" li");
                	for(i=0 ; i<subLocation.length ; i++){
                		$("#"+var_locationsID).children("a").removeClass("checked undetermined").addClass("unchecked");
                		$(subLocation[i]).children("a").removeClass("checked undetermined").addClass("unchecked");
                	}
            	//}
